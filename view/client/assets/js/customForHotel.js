                       ...
                       <div class="box-div-data-hotel">
                               <div class='divided-list divided-list-1'>
                                        <div class='divided-list-item'>
                                            ...
                                            <span class="parent-icon-person">
                                                <i class='fa-regular fa-user-group'></i>
                                             </span>
                                             <span class="parent-span-person">
                                                   ${Room.MaxCapacity} ${useXmltag('People')}           
                                             </span>
                                           </span>
                                        </div>
                            ...
                            <div class='hotel-rooms-local-content-col'>
                                ...
                                <div class='nuumbrtRoom d-none'>
                                    ...
                                    <input id='remainingCapacity${Rate.RoomToken}' name='remainingCapacity${Rate.RoomToken}' type='hidden' value='1'>
                                          ...
                                          <div class="parent-box-discount--new">
                                    `
...
<span class='price_number'>  
                                             <i class='site-main-text-color'>${number_format(Rate.Prices[0].CalculatedOnline)}</i>
                                    ...
                                    <div class="parent-box-room-selection">
                                    ...
                                    <i class='fa-solid fa-plus plus_room site-bg-main-color' data-type_application='api' data-room_token='${Rate.RoomToken}'></i>
                                    ...
                                    <i class='fa-solid fa-minus minus_room site-bg-main-color' data-type_application='api' data-room_token='${Rate.RoomToken}'></i>
                            </div>   
                        </div>
                                        </div>
                                    </div>
...
`
            ...
            if (Rate.Prices.length > 0) {
               $.each((Rate.Prices), function(index, Price) {
                  roomHtml += `
                        <div class='details'>
                            <div class='AvailableSeprate background__new--hotel site-bg-color-border-right-b '>
                            <span class="title-mobile--new">قیمت در تاریخ</span>
                                 ${Price.Date}
                            </div>
                            <div class='seprate'>
                                <span>
                                   <span class=''>${useXmltag('Adult')}:</span>
                                   <h6>${number_format(Price.CalculatedOnline)} ${Price.currency}</h6>
                                   <i class='fa fa-male checkIcon'></i>
                                </span>
                            </span>`
                  if (Price.ExtraBed > 0 && Room.ExtraCapacity > 0 && availabilityBeds > 0) {
                     roomHtml += `<span>
                      <span class=''>${useXmltag('Extrabed')}:</span>
                     <h6>${number_format(Price.ExtraBed)} ${Price.currency}</h6>
                     <i data-availabilityBeds='${availabilityBeds}' class='fa fa-bed checkIcon'></i>
                     </span>`
                  }
                  if (Price.Child) {
                     roomHtml += `
                     <span>
                        <span class=''>${useXmltag('Child')}:</span>   
                        <h6>${number_format(Price.Child)} ${Price.currency}</h6> 
...
`
            ...
            success: function(response) {
               ...
               if (result.Fees.length > 0) {
                  ...
                  $.each(result.Fees, function(index, Fee) {
                     boxHtml += `<li>${useXmltag('From')}<span style='direction: ltr; display:inline-block'>${Fee.FromDate}</span> ${useXmltag('To')} <span style='direction: ltr; display:inline-block'>${Fee.ToDate}</span> ${useXmltag('CancelationAmount')} <span style='direction: ltr; display: inline-block'>${Fee.Amount}</span></li>`
                  })
                  boxHtml += `<ul>`
      ...
      let generateHtmlForSearchHotel = function(data, parseJson) {
         ...
         let starsHtml = '<i class=\'fa fa-star\' aria-hidden=\'true\'></i>'
        ...
        <div class="mBox mBox-owl">
            <div class="parent-owl-tour-image">
               ...
               galleryHtml += `<div class='mBox parent-tour-image parent-gallery-image-mbox'>`;
            }
      ...
      <div class="more-image"> 
         ...
         */
         ...
         const iconsArray = value.Facilities && value.Facilities.HotelWithIcons
            ? Object.values(value.Facilities.HotelWithIcons)
            : null;
         ...
         hotelDetailContainer += `
            ...
            <div class="parent-navigation-list-hotel">
                    <ul>
                        <li><a onclick="clickScroll('RoomsContainerId')" href="javascript:">انتخاب اتاق</a></li>
                              ${
            ( typeof value.Facilities.HotelWithIcons != 'undefined')
               ? `<li><a onclick="clickScroll('hotel-feaId')" href="javascript:">امکانات اتاق</a></li>`
               : ``
         }
                        <li><a onclick="clickScroll('hotelPanelId')" href="javascript:">معرفی هتل</a></li>
                        <li><a onclick="clickScroll('hotelPanelId')" href="javascript:">قوانین و مقررات</a></li>
                        <li><a onclick="clickScroll('hotelFeaInnerId')" href="javascript:">مشاهده نقشه</a></li>
                    </ul>
                </div>
...
`
         ...
         hotelDetailContainer +=`<div id="RoomsContainerId" class=' RoomsContainer'>
            ...
            <div id="hotelPanelId" class='hotel-panel ${parseJson.lang == 'fa' ? 'txtRight' : 'txtLeft'}'>
         ...
         hotelDetailContainer += `<div id="hotel-feaId" class='hotel-fea'>
     ${
            ...
            ( typeof value.Facilities.HotelWithIcons != 'undefined')
               ...
               : ``
                        ...
                        <div id="hotelFeaInnerId" class='hotel-fea-inner'>

`

         let roomIcons = hotelIcons = ''
         if (typeof value.Facilities.HotelWithIcons != 'undefined') {
            if (iconsArray.length > 0 && iconsArray != null) {
      ...
      $(document).on('click', '.DetailRoom_external.showCancelRule', function(e) {
         ...
         if (_this.hasClass('isHide')) {
            _this.find('.detail_room_hotel').toggleClass('active_detail')
               ...
               if (data.Count > 0) {
                  $.each(value, function(index, item) {
                     ...
                     if (item.type_application == 'reservation') {
                        ...
                        if (typeof item.facilities != 'undefined') {
                           let html_li = '<div class="internal-hotel-facilities">'
                           if (item?.facilities && item.facilities.length > 1) {
                              $.each(item.facilities, function(j, facility) {
         ...
         if (data.book == 'PreReserve') {
            ...
            if(typeApplication == 'externalApi'){
               ...
               $(element).off('click').on('click', function(e) {
                  ...
                  content.style.cssText += 'text-align:center !important;';
...
function hotelDetail(typeApplication, hotelId, hotelName, RequestNumber, sourceId,_this=null,section=null) {
   ...
   // $('#formHotel').attr('action', href).submit()
   $('#formHotel').attr('action', href).attr('target', '_blank').submit();
}
...
function leaderRoomForHotel(typeApplication) {
   ...
   const requireExtraFields = $('#require_extra_fields').val() === 'true';
   ...
   if ($('#passenger_leader_room_fullName').val() == '') {
      ...
      if (requireExtraFields==true) {

         if ($('#passenger_leader_room_email').val() == '') {
            $('#messagePassengerLeader').html(useXmltag('PleaseEnterEmailPassengers'))
            return false
         } else if ($('#passenger_leader_room_postalcode').val() == '') {
            $('#messagePassengerLeader').html(useXmltag('PleaseEnterPostalcodePassengers'))
            return false
         } else if ($('#passenger_leader_room_address').val() == '') {
            $('#messagePassengerLeader').html(useXmltag('PleaseEnterAddressPassengers'))
            return false
         }
...
function HotelPopular(e){
   if(e === 'hotel'){
      e = 'popularCityForInternalHotel'
   }

   const $list = $('#listSearchCity');
   $list.empty();
   ...
   $.post(amadeusPath + 'hotel_ajax.php',{
         ...
         if(lang === 'fa'){
            $list.append(`
               <li onclick="selectCity_internal('${index.id}','${index.city_name}')">
                  <i class="div_c_sr_i">📍</i>
                  <div class="div_c_sr">
                     <span class="c-text">${index.city_name}</span>
                  </div>
               </li>
            `);
         } else {
      ...
      $list.removeClass('displayiN').show();
   });
}
   ...
   $.post(amadeusPath + 'hotel_ajax.php',
      ...
      function(data) {
         ...
         if (data.indexOf('successLoginHotel') > -1) {
            ...
            console.log('hi no log')

            if (isShowLoginPopup == '' || isShowLoginPopup == '1') {
               $('#login-popup').trigger('click')
               console.log('hi login-popup')
...
function checkForReserve(idRoom) {
   ...
   $.post(amadeusPath + 'hotel_ajax.php',
      ...
      function(data) {
         ...
         if (data.indexOf('success') > -1) {
            // فقط دکمه قیمت کودک همین اتاق را فعال کن
            enableECHDButtonForRoom(idRoom);
   ...
   if (RoomCount == 0) {
      // غیرفعال کردن دکمه قیمت کودک همین اتاق
      disableECHDButtonForRoom(idRoom);
   }
}
function enableAllECHDButtons() {
   $('.showECHDPrice').each(function() {
...
function enableECHDButtonForRoom(roomId) {
   $('#btnECHDPrice-' + roomId).removeClass('disabled');
   $('#btnECHDPrice-' + roomId).css({
      'opacity': '1',
      'pointer-events': 'auto'
   });
}

function disableECHDButtonForRoom(roomId) {
   $('#btnECHDPrice-' + roomId).addClass('disabled');
   $('#btnECHDPrice-' + roomId).css({
      'opacity': '0.5',
      'pointer-events': 'none'
   });
   // بستن بخش قیمت کودک اگر باز است
   $('#echd-section-' + roomId).addClass('displayN');
   $('#btnECHDPrice-' + roomId).find('i').removeClass('fa-angle-up').addClass('fa-angle-down');
}
function checkForReserveECHD(idRoom) {
   var idHotel = $('#idHotel_reserve').val()
   var startDate = $('#startDate_reserve').val()
   var endDate = $('#endDate_reserve').val()
   ...
   $.post(amadeusPath + 'hotel_ajax.php',
      {
         ...
         startDate: startDate,
         endDate: endDate,
         flag: 'checkForReserveECHD', // تغییر فلگ برای تشخیص نوع درخواست
      },
      function(data) {

         if (data.indexOf('success') > -1) {
            calculateRoomPricesForReservation(idRoom) // محاسبه مجدد قیمت‌ها
         } else {
            $.alert({
               title: useXmltag('Reservationhotel'),
               icon: 'fa fa-cart-plus',
            ...
            $('#RoomCountECHD' + idRoom).val(0) // بازگرداندن مقدار به صفر
         }
      })
}

function disableAllECHDButtons() {
   $('.showECHDPrice').each(function() {
      ...
      // بستن بخش‌های باز شده
      const roomId = $(this).data('roomindex');
      $('#echd-section-' + roomId).addClass('displayN');
...
function calculateRoomPricesForReservation(idRoom) {
   if (isResettingCapacity) {
      return;
   }

   // فقط بررسی ظرفیت - اگر مشکل داشت هشدار بده اما محاسبات را متوقف نکن
   checkRoomCapacity(idRoom);

   // بقیه کد محاسبات بدون تغییر...
   var RoomCount = $('#RoomCount' + idRoom).val();
   var TypeRoomHotel = $('#TypeRoomHotel').val();

   // بررسی آیا حداقل یک اتاق انتخاب شده است
   var hasSelectedRoom = false;
   var resultIdRoom = TypeRoomHotel.split('/');

   for (i = 0; i < resultIdRoom.length; i++) {
      if ($('#RoomCount' + resultIdRoom[i]).val() > 0) {
         hasSelectedRoom = true;
         enableECHDButtonForRoom(resultIdRoom[i]);
      } else {
         disableECHDButtonForRoom(resultIdRoom[i]);
      }
   }

   if (!hasSelectedRoom) {
      disableAllECHDButtons();
   }
   ...
   // محاسبه قیمت بزرگسال
   var priceAdult = parseInt(RoomCount) * parseInt(CostkolHotelRoomDBL);
   var priceAdultToShow = parseInt(RoomCount) * CostkolHotelRoomDBLToShow;
   var countAdult = parseInt(RoomCount);

   // محاسبه قیمت کودک (ECHD)
   var ecdhData = calculateECHDPriceForReservation(idRoom);
   var priceECHD = ecdhData.priceECHD;
   var priceECHDToShow = ecdhData.priceECHDToShow;
   var countECHD = ecdhData.countECHD;

   // محاسبه قیمت تخت اضافه بزرگسال (EXT) - بدون فراخوانی تابع اصلی
   var RoomCountEXT = $('#RoomCountEXT' + idRoom).val() || 0;
   var stayingTime = parseInt($('#stayingTime').val()) || 1;
   var priceEXT = parseInt(RoomCountEXT) * parseInt(CostkolHotelRoomEXT) * stayingTime;
   var priceEXTToShow = parseInt(RoomCountEXT) * CostkolHotelRoomEXTToShow * stayingTime;
   //177 var priceEXT = parseInt(RoomCountEXT) * parseInt(CostkolHotelRoomEXT);
   //177var priceEXTToShow = parseInt(RoomCountEXT) * CostkolHotelRoomEXTToShow;
   ...
   if (ExtraBed > 0) {
      priceAdult = parseInt(priceAdult) + (parseInt(ExtraBed) * parseInt(CostkolHotelRoomEXT));
      priceAdultToShow = priceAdultToShow + (parseInt(ExtraBed) * CostkolHotelRoomEXTToShow);
   }

   if (ExtraChildBed > 0) {
      priceAdult = parseInt(priceAdult) + (parseInt(ExtraChildBed) * parseInt(CostkolHotelRoomCHD));
      priceAdultToShow = priceAdultToShow + (parseInt(ExtraChildBed) * CostkolHotelRoomCHDToShow);
   }

   // ذخیره مقادیر بزرگسال
   $('#FinalRoomCount_Reserve' + idRoom).val(countAdult);
   $('#FinalPriceRoom_Reserve' + idRoom).val(priceAdult);
   $('#FinalPriceRoom_Reserve' + idRoom).data('amount', priceAdultToShow);

   // ذخیره مقادیر کودک
   $('#FinalECHDCount_Reserve' + idRoom).val(countECHD);
   $('#FinalECHDPrice_Reserve' + idRoom).val(priceECHD);
   $('#FinalECHDPrice_Reserve' + idRoom).data('amount', priceECHDToShow);

   // ذخیره مقادیر تخت اضافه بزرگسال
   $('#FinalEXTCount_Reserve' + idRoom).val(RoomCountEXT);
   $('#FinalEXTPrice_Reserve' + idRoom).val(priceEXT);
   $('#FinalEXTPrice_Reserve' + idRoom).data('amount', priceEXTToShow);

   // به‌روزرسانی قیمت کل
   updateTotalPrice();
   // به‌روزرسانی نمایش ظرفیت
   updateRemainingCapacity(idRoom);
}


function calculateECHDPriceForReservation(idRoom) {
   // بررسی ظرفیت
   if (!checkRoomCapacity(idRoom)) {
      resetToMaxCapacity(idRoom);
      return {
         priceECHD: 0,
         priceECHDToShow: 0,
         countECHD: 0
   ...
   var RoomCountECHD = $('#RoomCountECHD' + idRoom).val()
   var CostkolHotelRoomECHD = $('#CostkolHotelRoom_ECHD' + idRoom).val()
   var CostkolHotelRoomECHDToShow = $('#CostkolHotelRoom_ECHD' + idRoom).data('amount')
   var currencyUnit = $('#CostkolHotelRoom_ECHD' + idRoom).data('unit')

   var stayingTime = parseInt($('#stayingTime').val()) || 1;
   var priceECHD = parseInt(RoomCountECHD) * parseInt(CostkolHotelRoomECHD) * stayingTime;
   var priceECHDToShow = parseInt(RoomCountECHD) * CostkolHotelRoomECHDToShow * stayingTime;
   //177 var priceECHD = parseInt(RoomCountECHD) * parseInt(CostkolHotelRoomECHD)
   //177var priceECHDToShow = parseInt(RoomCountECHD) * CostkolHotelRoomECHDToShow

   $('#FinalECHDCount_Reserve' + idRoom).val(RoomCountECHD)
   $('#FinalECHDPrice_Reserve' + idRoom).val(priceECHD)
   $('#FinalECHDPrice_Reserve' + idRoom).data('amount', priceECHDToShow)

   return {
      priceECHD: priceECHD,
      priceECHDToShow: priceECHDToShow,
      countECHD: parseInt(RoomCountECHD)
   }
}
...
function toggleECHDPriceSection(element) {
   const roomId = element.getAttribute('data-roomindex');
   const $section = $('#echd-section-' + roomId);
   const $icon = $(element).find('i');

   if ($section.hasClass('displayN') || $section.hasClass('display-none')) {
      $section.removeClass('displayN display-none');
      $icon.removeClass('fa-angle-down').addClass('fa-angle-up');
      $(element).find('span').text(useXmltag('Extrabed'));
   } else {
      $section.addClass('displayN');
      $icon.removeClass('fa-angle-up').addClass('fa-angle-down');
      $(element).find('span').text(useXmltag('Extrabed'));
   }
}

// توابع جدید برای مدیریت تخت اضافه بزرگسال

function changeECHDCount(roomId, change) {
   if (isResettingCapacity) {
      return;
   }

   var currentVal = parseInt($('#RoomCountECHD' + roomId).val());
   var maxVal = parseInt($('#RoomCountECHD' + roomId).attr('max'));
   var newVal = currentVal + change;

   if (newVal >= 0 && newVal <= maxVal) {
      var roomCount = parseInt($('#RoomCount' + roomId).val()) || 0;
      var extCount = parseInt($('#RoomCountEXT' + roomId).val()) || 0;

      var totalExtraBedsAfterChange = extCount + newVal;
      // بررسی شرط: مجموع تخت‌های اضافه نباید از تعداد اتاق بیشتر شود
      if (totalExtraBedsAfterChange > roomCount) {
         //var extraBedsMessage = useXmltag('ExtraBedsLimit').replace('@@', roomCount);
         $.alert({
            title: useXmltag('Warning'),
            icon: 'fa fa-exclamation-triangle',
            //content: useXmltag('ExtraBedsExceeded') + '<br>' + extraBedsMessage,
            content: useXmltag('ExtraBedsExceeded').replace(/@@/g, roomCount),
            rtl: true,
            type: 'orange'
         });
         return false; // از تغییر مقدار جلوگیری می‌کند
      }

      // فقط در صورت عدم نقض شرط، مقدار را تغییر دهید
      $('#RoomCountECHD' + roomId).val(newVal);
      calculateECHDPriceForReservation(roomId);
      return true;
   }
   return false;
}

function changeEXTCount(roomId, change) {
   if (isResettingCapacity) {
      return;
   }

   var currentVal = parseInt($('#RoomCountEXT' + roomId).val());
   var maxVal = parseInt($('#RoomCountEXT' + roomId).attr('max'));
   var newVal = currentVal + change;

   if (newVal >= 0 && newVal <= maxVal) {
      var roomCount = parseInt($('#RoomCount' + roomId).val()) || 0;
      var ecdhCount = parseInt($('#RoomCountECHD' + roomId).val()) || 0;

      var totalExtraBedsAfterChange = newVal + ecdhCount;

      if (totalExtraBedsAfterChange > roomCount) {
         // فقط هشدار بده و از تغییر مقدار جلوگیری کن
         //var extraBedsMessage = useXmltag('ExtraBedsLimit').replace('@@', roomCount);
         $.alert({
            title: useXmltag('Warning'),
            icon: 'fa fa-exclamation-triangle',
      ...
      // فقط در صورت عدم نقض شرط، مقدار را تغییر دهید
      $('#RoomCountEXT' + roomId).val(newVal);
      calculateEXTPriceForReservation(roomId);
      return true;
   }
}


function calculateEXTPriceForReservation(idRoom) {
   // بررسی ظرفیت
   if (!checkRoomCapacity(idRoom)) {
      resetToMaxCapacity(idRoom);
      return {
   ...
   var RoomCountEXT = $('#RoomCountEXT' + idRoom).val();
   var CostkolHotelRoomEXT = $('#CostkolHotelRoom_EXT' + idRoom).val();
   var CostkolHotelRoomEXTToShow = $('#CostkolHotelRoom_EXT' + idRoom).data('amount');
   var currencyUnit = $('#CostkolHotelRoom_EXT' + idRoom).data('unit');

   var priceEXT = parseInt(RoomCountEXT) * parseInt(CostkolHotelRoomEXT);
   var priceEXTToShow = parseInt(RoomCountEXT) * CostkolHotelRoomEXTToShow;

   $('#FinalEXTCount_Reserve' + idRoom).val(RoomCountEXT);
   $('#FinalEXTPrice_Reserve' + idRoom).val(priceEXT);
   $('#FinalEXTPrice_Reserve' + idRoom).data('amount', priceEXTToShow);

   // محاسبه مجدد قیمت کل - بدون فراخوانی مجدد calculateRoomPricesForReservation
   updateTotalPrice();

   return {
      priceEXT: priceEXT,
      priceEXTToShow: priceEXTToShow,
      countEXT: parseInt(RoomCountEXT)
   }
}

// توابع برای مدیریت تخت کودک (ECHD)
...
function updateTotalPrice() {
   ...
   for (i = 0; i < resultIdRoom.length; i++) {
      ...
      if ($('#FinalRoomCount_Reserve' + resultIdRoom[i]).val() > 0) {
         ...
         priceHotelToShow = priceHotelToShow + ($('#FinalPriceRoom_Reserve' + resultIdRoom[i]).data('amount') || 0);
         countHotel = parseInt(countHotel) + parseInt($('#FinalRoomCount_Reserve' + resultIdRoom[i]).val());
      }

      if ($('#FinalECHDCount_Reserve' + resultIdRoom[i]).val() > 0) {
         priceHotel = parseInt(priceHotel) + parseInt($('#FinalECHDPrice_Reserve' + resultIdRoom[i]).val());
         priceHotelToShow = priceHotelToShow + ($('#FinalECHDPrice_Reserve' + resultIdRoom[i]).data('amount') || 0);
         countECHDTotal = parseInt(countECHDTotal) + parseInt($('#FinalECHDCount_Reserve' + resultIdRoom[i]).val());
      }

      if ($('#FinalEXTCount_Reserve' + resultIdRoom[i]).val() > 0) {
         priceHotel = parseInt(priceHotel) + parseInt($('#FinalEXTPrice_Reserve' + resultIdRoom[i]).val());
         priceHotelToShow = priceHotelToShow + ($('#FinalEXTPrice_Reserve' + resultIdRoom[i]).data('amount') || 0);
         countEXTTotal = parseInt(countEXTTotal) + parseInt($('#FinalEXTCount_Reserve' + resultIdRoom[i]).val());
      }
   }
   ...
   if (countECHDTotal > 0) {
      finalText += ' + ' + countECHDTotal + ' ' + useXmltag('HotelExtraChild');
   }
   if (countEXTTotal > 0) {
      finalText += ' + ' + countEXTTotal + ' ' + useXmltag('ExtrabedAdult');
   }

   $('.roomFinalTxt').html(finalText);
   $('.roomFinalPrice').html(number_format(priceHotelToShow) + ' ' + ($('#CostkolHotelRoom_EXT' + resultIdRoom[0]).data('unit') || 'ریال'));

   var prepayment_percentage = $('#prepaymentPercentage').data('amount');
   var final_prepayment_package_price = ((priceHotel * prepayment_percentage) / 100);
   $('.roomFinalPrepaymentPackagePrice').html(number_format(final_prepayment_package_price) + ' ' + ($('#CostkolHotelRoom_EXT' + resultIdRoom[0]).data('unit') || 'ریال'));
   ...
   // قرار دادن قیمت کل در فیلد hidden
   $('#total_price_reserve').val(priceHotel);
}

// در تابع checkForReserve یا event listener مربوط به RoomCount اصلی
function handleRoomCountChange(roomId, newVal) {
   if (isResettingCapacity) {
      return false;
   }
   ...
   var totalExtraBeds = extCount + ecdhCount;

   if (totalExtraBeds > newVal) {
      resetToMaxCapacity(roomId);
      return false;
   }
...
function checkRoomCapacity(roomId) {
   ...
   // محاسبه کل تخت‌های اضافه (کودک + بزرگسال)
   var totalExtraBeds = extCount + ecdhCount;

   // بررسی آیا مجموع تخت‌های اضافه از تعداد اتاق بیشتر شده است
   if (totalExtraBeds > roomCount) {
      $('#capacity_error_' + roomId).removeClass('displayN');
      //var extraBedsMessage = useXmltag('ExtraBedsLimit').replace('@@', roomCount);
      var ExtraBedsExceeded = useXmltag('ExtraBedsExceeded').replace(/@@/g, roomCount);
      //$('#capacity_error_' + roomId).html(useXmltag('ExtraBedsExceeded') + '<br>' + extraBedsMessage);
      $('#capacity_error_' + roomId).html(ExtraBedsExceeded);
      return false;
   } else {
      $('#capacity_error_' + roomId).addClass('displayN');
      return true;
   }
}

function resetToMaxCapacity(roomId, callback) {
   if (isResettingCapacity) {
      return;
   }

   isResettingCapacity = true;
   var roomCount = parseInt($('#RoomCount' + roomId).val()) || 0;
   //var extraBedsMessage = useXmltag('ExtraBedsLimit').replace('@@', roomCount);
   var ExtraBedsExceeded = useXmltag('ExtraBedsExceeded').replace(/@@/g, roomCount);
   $.alert({
      ...
      icon: 'fa fa-exclamation-triangle',
      //content: useXmltag('ExtraBedsExceeded') + '<br>' + extraBedsMessage,
      content: ExtraBedsExceeded ,
      rtl: true,
      type: 'orange',
      onClose: function() {
         isResettingCapacity = false;
         if (typeof callback === 'function') callback();
      }
   });
}

// تابع برای به‌روزرسانی ظرفیت باقیمانده
function updateRemainingCapacity(roomId) {
   var roomCount = parseInt($('#RoomCount' + roomId).val()) || 0;
   var extCount = parseInt($('#RoomCountEXT' + roomId).val()) || 0;
   var ecdhCount = parseInt($('#RoomCountECHD' + roomId).val()) || 0;

   var totalExtraBeds = extCount + ecdhCount;
   var remainingExtraBeds = roomCount - totalExtraBeds;
...
$(document).ready(function () {
   $(document).on('click', '.showECHDPrice:not(.disabled)', function(e) {
      toggleECHDPriceSection(e.currentTarget);
   });
   // اضافه کردن event listener برای تغییرات مستقیم در inputها

   $(document).on('input change', 'input[class*="val_number_room"]', function() {
      var inputId = $(this).attr('id');
      var roomId = inputId.replace('RoomCount', '').replace('ECHD', '').replace('EXT', '');
      ...
      // بررسی اینکه آیا اتاق اصلی انتخاب شده است (برای تخت‌های اضافه)
      var roomCount = parseInt($('#RoomCount' + roomId).val()) || 0;
      if (roomCount === 0 && (inputId.indexOf('ECHD') > -1 || inputId.indexOf('EXT') > -1)) {
         $(this).val(0);
         $.alert({
            title: useXmltag('Warning'),
            icon: 'fa fa-exclamation-triangle',
            content: useXmltag('PleaseSelectMainRoomFirst'),
            rtl: true,
            type: 'orange'
         });
         return;
      ...
      // بررسی شرط مجموع تخت‌های اضافه
      if (inputId.indexOf('ECHD') > -1 || inputId.indexOf('EXT') > -1) {
         var extCount = parseInt($('#RoomCountEXT' + roomId).val()) || 0;
         var ecdhCount = parseInt($('#RoomCountECHD' + roomId).val()) || 0;

         // محاسبه مجموع جدید
         var newExtCount = inputId.indexOf('EXT') > -1 ? currentValue : extCount;
         var newEcdhCount = inputId.indexOf('ECHD') > -1 ? currentValue : ecdhCount;
         var totalExtraBeds = newExtCount + newEcdhCount;

         if (totalExtraBeds > roomCount) {
            $(this).val(inputId.indexOf('EXT') > -1 ? extCount : ecdhCount); // بازگشت به مقدار قبلی
            //var extraBedsMessage = useXmltag('ExtraBedsLimit').replace('@@', roomCount);
            var ExtraBedsExceeded = useXmltag('ExtraBedsExceeded').replace(/@@/g, roomCount);
            $.alert({
               title: useXmltag('Warning'),
      ...
      // اگر همه شرایط برقرار بود، محاسبات را انجام بده
      setTimeout(function() {
         if (inputId.indexOf('ECHD') > -1) {
            calculateECHDPriceForReservation(roomId);
         } else if (inputId.indexOf('EXT') > -1) {
            calculateEXTPriceForReservation(roomId);
         } else {
   ...
   // اضافه کردن event listener برای دکمه‌های plus/minus
   $(document).on('click', '.plus_room, .minus_room', function() {
      if (isResettingCapacity) {
         return;
      }

      var roomId = $(this).data('room_token');
      var bedType = $(this).data('bed_type');
      var isPlus = $(this).hasClass('plus_room');

      // بررسی اینکه آیا اتاق اصلی انتخاب شده است
      var roomCount = parseInt($('#RoomCount' + roomId).val()) || 0;
      if (roomCount === 0 && (bedType === 'echd' || bedType === 'ext')) {
         $.alert({
            title: useXmltag('Warning'),
            icon: 'fa fa-exclamation-triangle',
            content: useXmltag('PleaseSelectMainRoomFirst'),
      ...
      // اجرای توابع تغییر با توجه به نوع تخت
      var success = false;
      if (bedType === 'echd') {
         success = changeECHDCount(roomId, isPlus ? 1 : -1);
      } else if (bedType === 'ext') {
         success = changeEXTCount(roomId, isPlus ? 1 : -1);
      }

      // اگر تغییر موفقیت‌آمیز نبود، هیچ کاری نکن (مقادیر قبلی حفظ می‌شوند)
   });
