...
function openCountPassengerNew(element) {
   // پیدا کردن المان کلیک شده
   let $clickedElement = $(element);

   // پیدا کردن فرم والد
   let $form = $clickedElement.closest('form');

   // تشخیص نوع از روی id فرم
   let formId = $form.attr('id');
   let type = '';

   if (formId === 'hotel_search_form') {
      // تشخیص از روی selected_type یا destination-country-js
      let selectedType = $("#selected_type").val();
      let hasExternalCountry = $(".destination-country-js").val();

      if (selectedType === 'external' || hasExternalCountry) {
         type = 'international';
      } else {
         type = 'internal';
      }
   } else if (formId === 'internal_hotel_form') {
      type = 'internal';
   } else if (formId === 'international_hotel_form') {
      type = 'international';
   } else {
      // تشخیص از روی کلاس‌های موجود در والد
      if ($clickedElement.closest('.internal-hotel-js').length) {
         type = 'internal';
      } else if ($clickedElement.closest('.international-hotel-js').length) {
         type = 'international';
      }
   }

   // پیدا کردن پنل مربوطه
   let $targetPanel;
   if (type === 'internal') {
      $targetPanel = $('.internal-my-hotels-rooms-js');
   } else {
      $targetPanel = $('.international-my-hotels-rooms-js');
   }
   ...
   // باز کردن پنل مربوطه
   $targetPanel.toggleClass('active_p');
...
function searchCityInternalAndExternal(e) {
   // ====== مهمترین خط: ریست کردن نوع انتخاب در هر بار تایپ ======
   $("#selected_type").val('');
   $("#no_select_city").val('');

   let searchValue = $('#autoComplateSearchIN_2').val();
   ...
   if (searchValue.length >= 2) {
      ...
      //          flag: 'searchCityHotelForInternalAndExternalHotel',
      //          json: true,
      //       },
      //       dataType: 'json',
      //       success: function(response) {
...
function resetAllSearchStates() {
   $("#selected_type").val('');
   $("#no_select_city").val('');
   $("#autoComplateSearchIN_hidden").val('');
   $("#autoComplateSearchIN_hidden_en").val('');
   $(".destination-country-js").val('');
   $(".destination-city-js").val('');
   $("#destination_country").val('');
   $("#destination_city_foreign").val('');
   $("#destination_city").val('');
}

// ریست کردن وضعیت جستجو
...
function displaySearchResults(data) {
   let listElement = document.querySelector('#listSearchCity_2');
   if (!listElement) {
      console.error('المان #listSearchCity_2 یافت نشد');
      return;
   }
   ...
   // 1. نمایش شهرهای داخلی (Cities)
   if (data.Cities && Array.isArray(data.Cities) && data.Cities.length > 0) {
      hasData = true;
      data.Cities.forEach(function(item) {
         let cityName = (lang == 'fa' ? item.CityName : item.CityNameEn) || item.CityName || item.CityNameEn || '';
         listElement.innerHTML += createCityItem(item.CityId, cityName, item.CityNameEn);
      });
   }

   // 2. نمایش هتل‌های API
   if (data.ApiHotels && Array.isArray(data.ApiHotels) && data.ApiHotels.length > 0) {
      hasData = true;
      data.ApiHotels.forEach(function(item) {
         let hotelName = (lang == 'fa' ? item.HotelName : item.HotelNameEn) || item.HotelName || item.HotelNameEn || '';
         listElement.innerHTML += createApiHotelItem(item.HotelId, hotelName, item.HotelNameEn);
      });
   }

   // 3. نمایش هتل‌های رزرویشن
   if (data.ReservationHotels && Array.isArray(data.ReservationHotels) && data.ReservationHotels.length > 0) {
      hasData = true;
      data.ReservationHotels.forEach(function(item) {
         let hotelName = (lang == 'fa' ? item.HotelName : item.HotelNameEn) || item.HotelName || item.HotelNameEn || '';
         listElement.innerHTML += createReservationHotelItem(item.HotelId, hotelName, item.HotelNameEn);
      });
   }

   // 4. نمایش شهرهای خارجی
   if (data.external && Array.isArray(data.external) && data.external.length > 0) {
      hasData = true;
      data.external.forEach(function(item) {
         let countryName = (lang == 'fa') ? (item.CountryFa || item.CountryTitleFa || '') : (item.CountryEn || item.CountryTitleEn || '');
         let cityName = (lang == 'fa') ? (item.DepartureCityFa || '') : (item.DepartureCityEn || '');
         listElement.innerHTML += createExternalCityItem(item, cityName, countryName);
      });
   }
...
// ساخت آیتم شهر داخلی
function createCityItem(cityId, cityName, cityNameEn) {
   return `
      <li onclick="selectItem('city', '${cityId}', '${escapeHtml(cityName)}', '${escapeHtml(cityNameEn)}', null, null, null, null)">
         <!--        ^^^^^^
         اینجا cityId ارسال می‌شود -->
         <i class="div_c_sr_i">
         ...
         <div class="div_c_sr">
            <span class="c-text">${escapeHtml(cityName)}</span>
         </div>
      </li>
   `;
}
// ساخت آیتم هتل API
function createApiHotelItem(hotelId, hotelName, hotelNameEn) {
   return `
      <li class="div_c_sr_blue" onclick="selectItem('api', '${hotelId}', '${escapeHtml(hotelName)}', '${escapeHtml(hotelNameEn)}', null, null, null, null)">
         <i class="div_c_sr_i">
         ...
         <div class="div_c_sr">
            <span class="c-text">${escapeHtml(hotelName)}</span>
         </div>
      </li>
   `;
}
// ساخت آیتم هتل رزرویشن
function createReservationHotelItem(hotelId, hotelName, hotelNameEn) {
   return `
      <li class="div_c_sr_blue" onclick="selectItem('reservation', '${hotelId}', '${escapeHtml(hotelName)}', '${escapeHtml(hotelNameEn)}', null, null, null, null)">
         <i class="div_c_sr_i">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512"><path d="M336 488C336 501.3 325.3 512 312 512C298.7 512 288 501.3 288 488V56C288 25.07 313.1 0 344 0H456C486.9 0 512 25.07 512 56V192H544V120C544 106.7 554.7 96 568 96C581.3 96 592 106.7 592 120V192.6C619.1 196.4 640 219.8 640 248V488C640 501.3 629.3 512 616 512C602.7 512 592 501.3 592 488V248C592 243.6 588.4 240 584 240H464V56C464 51.58 460.4 48 456 48H344C339.6 48 336 51.58 336 56V488zM368 96C368 87.16 375.2 80 384 80H416C424.8 80 432 87.16 432 96V128C432 136.8 424.8 144 416 144H384C375.2 144 368 136.8 368 128V96zM416 176C424.8 176 432 183.2 432 192V224C432 232.8 424.8 240 416 240H384C375.2 240 368 232.8 368 224V192C368 183.2 375.2 176 384 176H416zM368 288C368 279.2 375.2 272 384 272H416C424.8 272 432 279.2 432 288V320C432 328.8 424.8 336 416 336H384C375.2 336 368 328.8 368 320V288zM544 272C552.8 272 560 279.2 560 288V320C560 328.8 552.8 336 544 336H512C503.2 336 496 328.8 496 320V288C496 279.2 503.2 272 512 272H544zM496 384C496 375.2 503.2 368 512 368H544C552.8 368 560 375.2 560 384V416C560 424.8 552.8 432 544 432H512C503.2 432 496 424.8 496 416V384zM224 160C224 166 223 171 222 176C242 190 256 214 256 240C256 285 220 320 176 320H160V480C160 498 145 512 128 512C110 512 96 498 96 480V320H80C35 320 0 285 0 240C0 214 13 190 33 176C32 171 32 166 32 160C32 107 74 64 128 64C181 64 224 107 224 160z"/></svg>
         </i>
         <div class="div_c_sr">
            <span class="c-text">${escapeHtml(hotelName)}</span>
         </div>
...
// ساخت آیتم شهر خارجی
function createExternalCityItem(item, cityName, countryName) {
   let displayText = cityName + (countryName ? '، ' + countryName : '');
   return `
      <li onclick="selectItem('external', null, '${escapeHtml(displayText)}', null, '${escapeHtml(item.CountryEn || '')}', '${escapeHtml(item.CountryFa || '')}', '${escapeHtml(item.DepartureCityEn || '')}', '${escapeHtml(item.DepartureCityFa || '')}')">
         <i class="div_c_sr_i">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M272 192C272 236.2 236.2 272 192 272C147.8 272 112 236.2 112 192C112 147.8 147.8 112 192 112C236.2 112 272 147.8 272 192zM192 160C174.3 160 160 174.3 160 192C160 209.7 174.3 224 192 224C209.7 224 224 209.7 224 192C224 174.3 209.7 160 192 160zM384 192C384 279.4 267 435 215.7 499.2C203.4 514.5 180.6 514.5 168.3 499.2C116.1 435 0 279.4 0 192C0 85.96 85.96 0 192 0C298 0 384 85.96 384 192H384zM192 48C112.5 48 48 112.5 48 192C48 204.4 52.49 223.6 63.3 249.2C73.78 274 88.66 301.4 105.8 329.1C134.2 375.3 167.2 419.1 192 451.7C216.8 419.1 249.8 375.3 278.2 329.1C295.3 301.4 310.2 274 320.7 249.2C331.5 223.6 336 204.4 336 192C336 112.5 271.5 48 192 48V48z"/></svg>
         </i>
         <div class="div_c_sr">
            <span class="c-text">${escapeHtml(cityName)}</span>
            <div class="yata_gdemo"><i>${escapeHtml(countryName)}</i></div>
         </div>
      </li>
...
function selectItem(type, id, name, nameEn, countryEn, countryFa, cityEn, cityFa) {
   console.log('انتخاب شد:', type, id, name);

   // ذخیره نوع انتخاب
   $("#selected_type").val(type);
   $("#no_select_city").val(type);

   if (type === 'city') {
      $("#autoComplateSearchIN_hidden").val(id);
      $("#autoComplateSearchIN_hidden_en").val(nameEn);
      $("#autoComplateSearchIN_2").val(name);

      // پاک کردن فیلدهای خارجی
      $(".destination-country-js").val('');
      $(".destination-city-js").val('');
      $("#destination_country").val('');
      $("#destination_city_foreign").val('');
   ...
   } else if (type === 'api' || type === 'reservation') {
      $("#autoComplateSearchIN_hidden").val(id);
      $("#autoComplateSearchIN_hidden_en").val(nameEn);
      $("#autoComplateSearchIN_2").val(name);
      ...
      $(".destination-country-js").val(countryEn);
      $(".destination-city-js").val(cityEn);
      $("#destination_country").val(countryEn);
      $("#destination_city_foreign").val(cityEn);
      $("#destination_city").val(cityEn);
   ...
   // $('#listSearchCity_2').hide();

   // مهم: focus روی input و آماده برای جستجوی بعدی
   $('#autoComplateSearchIN_2').focus();

   $("#startDateForHotelLocal").focus();
   // اگر روی موبایل هستیم
   if (window.innerWidth <= 576) {
      if (typeof closeMobileHotelDrawer === 'function') {
...
function searchHotel() {
   let selectedType = $("#selected_type").val();
   let search = $("#autoComplateSearchIN_2");
   let searchValue = $("#autoComplateSearchIN_2").val();
   let check_in_date = $(".check-in-date-js-new")
   let nights_hotel = $(".nights-hotel-js").val()
   checkSearchFields(search,check_in_date)

   // if (!searchValue || searchValue === '') {
   //    // alert('لطفاً ابتدا یک شهر یا هتل را انتخاب کنید');
   //
   //    return false;
   // }

   // همیشه در تب جدید باز شود
   let isNewTab = $("#hotel_search_form").attr('target') === '_blank';

   // اگر نوع خارجی است
   if (selectedType === 'external') {
      let country = $(".destination-country-js").val();
      let city = $(".destination-city-js").val();
      let checkIn = $(".check-in-date-js").val();
      let checkOut = $(".check-out-date-internal-js").val();
      ...
      if (!country || !city) {
         // alert('لطفاً یک شهر خارجی را انتخاب کنید');
         return false;
      }

      let rooms = buildRoomsForSearch();
      let url = amadeusPathByLang + `resultExternalHotel/${country}/${city}/${checkIn}/${checkOut}/${nights_hotel}/${rooms}`;
      ...
      // نوع داخلی
      let id = $("#autoComplateSearchIN_hidden").val();  // ← اینجا cityId یا hotelId است
      let nameEn = $("#autoComplateSearchIN_hidden_en").val();
      let checkIn = $(".check-in-date-js").val();
      let checkOut = $(".check-out-date-internal-js").val();
      ...
      if (!id) {
         // alert('لطفاً یک شهر یا هتل را انتخاب کنید');
      ...
      if (selectedType === 'api') {
         // جستجوی هتل API
         url = amadeusPathByLang + `detailHotel/api/${id}/?startDate=${checkIn}&nights=${nights_hotel}&rooms=${rooms}`;

      } else if (selectedType === 'reservation') {
         // جستجوی هتل رزرویشن
         url = amadeusPathByLang + `roomHotelLocal/reservation/${id}/${nameEn}?startDate=${checkIn}&nights=${nights_hotel}`;

      } else {
         // جستجوی شهر (city) - ارسال cityId به جای اسم شهر
         url = amadeusPathByLang + `searchHotel&type=new&city=${id}&startDate=${checkIn}&nights=${nights_hotel}&rooms=${rooms}`;
         //                                                    ^^^^
         //                                                    اینجا id همان cityId است
      }
...
function hotelHideLoader() {
   $('.loader-container').hide();
}
// تابع ساخت اتاق‌ها برای جستجو
function buildRoomsForSearch() {
   let rooms = "";
   let roomItems = $(".international-my-room-hotel-item-js");

   if (roomItems.length === 0) {
      roomItems = $(".internal-my-room-hotel-item-js");
   }
   ...
   roomItems.each(function() {
      const adult = parseInt($(this).find(".international-count-parent-js").val()) ||
          parseInt($(this).find(".internal-count-parent-js").val()) || 2;
      const child = parseInt($(this).find(".international-count-child-js").val()) ||
          parseInt($(this).find(".internal-count-child-js").val()) || 0;

      rooms = rooms + "R:" + adult;
      if (child > 0) {
         rooms = rooms + "-" + child;
         let birthDays = $(this).find(".birth-days-item-js select");
         birthDays.each(function(index) {
            let age = $(this).val();
            if (index === 0 && age) {
               rooms = rooms + "-" + age;
            } else if (age) {
...
function handleMobileHotelSearchInternalAndExternal(keyword) {
   const listContainer = $("#mobilePopularDestinationsHotel");

   // اگر سرچ خالی شد → دوباره لیست محبوب را برگردان
   if (!keyword.trim()) {
      renderMobilePopularCities();
      return;
   }

   // اگر حداقل 2 حرف وارد شده، جستجو کن
   if (keyword.length >= 2) {
      ...
      $.post(amadeusPath + 'hotel_ajax.php', {
         ...
         try {
            ...
            // ========== 1. شهرهای داخلی (Cities) ==========
            if (response.Cities && Array.isArray(response.Cities) && response.Cities.length > 0) {
               hasData = true;
               response.Cities.forEach((item) => {
                  let cityName = (lang == 'fa' ? item.CityName : item.CityNameEn) || item.CityName || item.CityNameEn || '';
                  html_items.push(`
                     <li onclick="selectItemMobile('city', '${item.CityId}', '${escapeHtml(cityName)}', '${escapeHtml(item.CityNameEn)}', null, null, null, null)">
                        <div class="mobile-city-item">
                           ...
                           <span class="city-text">${escapeHtml(cityName)}</span>
                        </div>
                     </li>
            ...
            // ========== 2. هتل‌های ApiHotels ==========
            if (response.ApiHotels && Array.isArray(response.ApiHotels) && response.ApiHotels.length > 0) {
               hasData = true;
               response.ApiHotels.forEach((item) => {
                  let hotelName = (lang == 'fa' ? item.HotelName : item.HotelNameEn) || item.HotelName || item.HotelNameEn || '';
                  html_items.push(`
                     <li onclick="selectItemMobile('api', '${item.HotelId}', '${escapeHtml(hotelName)}', '${escapeHtml(item.HotelNameEn)}', null, null, null, null)">
                        <div class="mobile-city-item">
                           <svg viewBox="0 0 24 24" width="24px" height="24px" fill="currentColor" class="mobile-drawer-svg-map shrink-0">
                              <path d="M11.28 1.534c4.437-.419 8.22 3.11 8.22 7.59 0 4.053-1.89 7.941-6.398 12.888-.593.65-1.62.651-2.212 0-4.219-4.628-6.14-8.33-6.374-12.09-.263-4.237 2.701-8.005 6.765-8.388ZM18 9.124c0-3.604-3.031-6.432-6.579-6.097C8.192 3.332 5.8 6.374 6.013 9.83c.21 3.37 1.977 6.775 5.982 11.17l.531-.59c3.803-4.306 5.402-7.66 5.471-11.054L18 9.124ZM12 5.25a3.75 3.75 0 1 1 0 7.5 3.75 3.75 0 0 1 0-7.5Zm0 1.5a2.25 2.25 0 1 0 0 4.5 2.25 2.25 0 0 0 0-4.5Z" fill-rule="evenodd"></path>
                           </svg>
                           <span class="city-text">${escapeHtml(hotelName)}</span>
                        </div>
                     </li>
                     <div class="mobile-divider"></div>
                  `);
            ...
            // ========== 3. هتل‌های ReservationHotels ==========
            if (response.ReservationHotels && Array.isArray(response.ReservationHotels) && response.ReservationHotels.length > 0) {
               hasData = true;
               response.ReservationHotels.forEach((item) => {
                  let hotelName = (lang == 'fa' ? item.HotelName : item.HotelNameEn) || item.HotelName || item.HotelNameEn || '';
                  html_items.push(`
                     <li onclick="selectItemMobile('reservation', '${item.HotelId}', '${escapeHtml(hotelName)}', '${escapeHtml(item.HotelNameEn)}', null, null, null, null)">
                        <div class="mobile-city-item">
                           <svg viewBox="0 0 24 24" width="24px" height="24px" fill="currentColor" class="mobile-drawer-svg-map shrink-0">
                              <path d="M11.28 1.534c4.437-.419 8.22 3.11 8.22 7.59 0 4.053-1.89 7.941-6.398 12.888-.593.65-1.62.651-2.212 0-4.219-4.628-6.14-8.33-6.374-12.09-.263-4.237 2.701-8.005 6.765-8.388ZM18 9.124c0-3.604-3.031-6.432-6.579-6.097C8.192 3.332 5.8 6.374 6.013 9.83c.21 3.37 1.977 6.775 5.982 11.17l.531-.59c3.803-4.306 5.402-7.66 5.471-11.054L18 9.124ZM12 5.25a3.75 3.75 0 1 1 0 7.5 3.75 3.75 0 0 1 0-7.5Zm0 1.5a2.25 2.25 0 1 0 0 4.5 2.25 2.25 0 0 0 0-4.5Z" fill-rule="evenodd"></path>
                           </svg>
            ...
            // ========== 4. شهرهای خارجی (external) ==========
            if (response.external && Array.isArray(response.external) && response.external.length > 0) {
               hasData = true;
               response.external.forEach((item) => {
                  let countryName = (lang == 'fa') ? (item.CountryFa || item.CountryTitleFa || '') : (item.CountryEn || item.CountryTitleEn || '');
                  let cityName = (lang == 'fa') ? (item.DepartureCityFa || '') : (item.DepartureCityEn || '');
                  let displayName = cityName + (countryName ? '، ' + countryName : '');

                  html_items.push(`
                     <li onclick="selectItemMobile('external', null, '${escapeHtml(displayName)}', null, '${escapeHtml(item.CountryEn || '')}', '${escapeHtml(item.CountryFa || '')}', '${escapeHtml(item.DepartureCityEn || '')}', '${escapeHtml(item.DepartureCityFa || '')}')">
                        <div class="mobile-city-item">
                           <svg viewBox="0 0 24 24" width="24px" height="24px" fill="currentColor" class="mobile-drawer-svg-map shrink-0">
                              <path d="M11.28 1.534c4.437-.419 8.22 3.11 8.22 7.59 0 4.053-1.89 7.941-6.398 12.888-.593.65-1.62.651-2.212 0-4.219-4.628-6.14-8.33-6.374-12.09-.263-4.237 2.701-8.005 6.765-8.388ZM18 9.124c0-3.604-3.031-6.432-6.579-6.097C8.192 3.332 5.8 6.374 6.013 9.83c.21 3.37 1.977 6.775 5.982 11.17l.531-.59c3.803-4.306 5.402-7.66 5.471-11.054L18 9.124ZM12 5.25a3.75 3.75 0 1 1 0 7.5 3.75 3.75 0 0 1 0-7.5Zm0 1.5a2.25 2.25 0 1 0 0 4.5 2.25 2.25 0 0 0 0-4.5Z" fill-rule="evenodd"></path>
                           </svg>
                           <div class="city-text">
                              ...
                              <div style="font-size:12px">${escapeHtml(countryName)}</div>
                           </div>
                        </div>
                     </li>
                     <div class="mobile-divider"></div>
         ...
         console.error('خطا:', error);
         listContainer.html(`<div class='not_found text-center mt-4'>${useXmltag("ErrorHappened")}</div>`);
      });
   }
}
setupMobileHotelDrawerElementsDatePicker('dept')
// تابع انتخاب برای موبایل
function selectItemMobile(type, id, name, nameEn, countryEn, countryFa, cityEn, cityFa) {
   console.log('موبایل - انتخاب شد:', type, id, name);
   ...
   // فوکوس روی تاریخ ورود
   setTimeout(function() {
      openMobileHotelDrawerDatePicker()
      $("#startDateForHotelLocal").focus();
   }, 300);
}
// رندر شهرهای محبوب (داخلی و خارجی با هم)
function renderMobilePopularCities() {
   ...
   // دریافت هر دو نوع داده
   $.when(
       $.post(amadeusPath + 'hotel_ajax.php', { flag: 'popularCityForInternalHotel', self_Db: true }),
       $.post(amadeusPath + 'hotel_ajax.php', { flag: 'flightExternalRoutesDefault', self_Db: true })
   ).done(function(internalRes, externalRes) {
      let internalData = typeof internalRes[0] === 'string' ? JSON.parse(internalRes[0]) : internalRes[0];
      let externalData = typeof externalRes[0] === 'string' ? JSON.parse(externalRes[0]) : externalRes[0];
      ...
      //       let cityName = (lang == 'fa') ? item.city_name : item.city_name_en;
      //       html_items.push(`
      //          <li onclick="selectItemMobile('city', '${item.id}', '${escapeHtml(cityName)}', '${escapeHtml(item.city_name_en)}', null, null, null, null)">
      //             <div class="mobile-city-item">
      //                <svg viewBox="0 0 24 24" width="24px" height="24px" fill="currentColor" class="mobile-drawer-svg-map shrink-0">
      //                   <path d="M11.28 1.534c4.437-.419 8.22 3.11 8.22 7.59 0 4.053-1.89 7.941-6.398 12.888-.593.65-1.62.651-2.212 0-4.219-4.628-6.14-8.33-6.374-12.09-.263-4.237 2.701-8.005 6.765-8.388ZM18 9.124c0-3.604-3.031-6.432-6.579-6.097C8.192 3.332 5.8 6.374 6.013 9.83c.21 3.37 1.977 6.775 5.982 11.17l.531-.59c3.803-4.306 5.402-7.66 5.471-11.054L18 9.124ZM12 5.25a3.75 3.75 0 1 1 0 7.5 3.75 3.75 0 0 1 0-7.5Zm0 1.5a2.25 2.25 0 1 0 0 4.5 2.25 2.25 0 0 0 0-4.5Z" fill-rule="evenodd"></path>
      //                </svg>
      ...
      if (externalData && Array.isArray(externalData)) {
         externalData.forEach(function(item) {
            let countryName = (lang == 'fa') ? (item.CountryFa || '') : (item.CountryEn || '');
            let cityName = (lang == 'fa') ? (item.DepartureCityFa || '') : (item.DepartureCityEn || '');
            let displayName = cityName + (countryName ? '، ' + countryName : '');
      ...
      if (html_items.length === 0) {
         container.html(`<div class='not_found text-center mt-4'>${useXmltag("Noresult")}</div>`);
      } else {
         container.html(`
      ...
      container.html(`<div class='not_found text-center mt-4'>${useXmltag("ErrorHappened")}</div>`);
   });
}
...
function setupMobileHotelDrawerElements(search_type) {
   ...
   const isHotelOneInputSearch = document.getElementById('searchbox_internal_external');
   if (drawer && overlay) {
      const searchInput = document.getElementById('mobileSearchInputHotel');  // تغییر: id جدید
      if (searchInput) {
         searchInput.placeholder = 'انتخاب مقصد یا هتل';
         searchInput.oninput = () => {
            if (isHotelOneInputSearch && isHotelOneInputSearch.value === '1') {
               handleMobileHotelSearchInternalAndExternal(searchInput.value);
            } else {
               handleMobileHotelSearch(searchInput.value, search_type);
            }
         };
...
function openBoxPopularInternalAndExternal(e) {
   if(window.innerWidth <= 576) {
      if(e === 'hotel') {
         setupMobileHotelDrawerElements('internal');
         ...
         items.forEach((index) => {
            if (lang === 'en' || lang === 'ar') {
               if (e === 'hotel' || e === 'residence') {
                  // استفاده از selectItem برای شهر داخلی
                  $(targetUl).append(`
                     <li onclick="selectItem('city', '${index.id}', '${index.city_name_en}', '${index.city_name_en}', null, null, null, null)">
                        <i class="div_c_sr_i">
                           <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M272 192C272 236.2 236.2 272 192 272C147.8 272 112 236.2 112 192C112 147.8 147.8 112 192 112C236.2 112 272 147.8 272 192zM192 160C174.3 160 160 174.3 160 192C160 209.7 174.3 224 192 224C209.7 224 224 209.7 224 192C224 174.3 209.7 160 192 160zM384 192C384 279.4 267 435 215.7 499.2C203.4 514.5 180.6 514.5 168.3 499.2C116.1 435 0 279.4 0 192C0 85.96 85.96 0 192 0C298 0 384 85.96 384 192H384zM192 48C112.5 48 48 112.5 48 192C48 204.4 52.49 223.6 63.3 249.2C73.78 274 88.66 301.4 105.8 329.1C134.2 375.3 167.2 419.1 192 451.7C216.8 419.1 249.8 375.3 278.2 329.1C295.3 301.4 310.2 274 320.7 249.2C331.5 223.6 336 204.4 336 192C336 112.5 271.5 48 192 48V48z"/></svg>
                        </i>
                        <div class="div_c_sr">
                  ...
                  // استفاده از selectItem برای شهر خارجی
                  $(targetUl).append(`
                     <li onclick="selectItem('external', null, '${index.DepartureCityEn}', null, '${index.CountryEn}', '${index.CountryFa}', '${index.DepartureCityEn}', '${index.DepartureCityFa}')">
                        <i class="div_c_sr_i">
                           <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M272 192C272 236.2 236.2 272 192 272C147.8 272 112 236.2 112 192C112 147.8 147.8 112 192 112C236.2 112 272 147.8 272 192zM192 160C174.3 160 160 174.3 160 192C160 209.7 174.3 224 192 224C209.7 224 224 209.7 224 192C224 174.3 209.7 160 192 160zM384 192C384 279.4 267 435 215.7 499.2C203.4 514.5 180.6 514.5 168.3 499.2C116.1 435 0 279.4 0 192C0 85.96 85.96 0 192 0C298 0 384 85.96 384 192H384zM192 48C112.5 48 48 112.5 48 192C48 204.4 52.49 223.6 63.3 249.2C73.78 274 88.66 301.4 105.8 329.1C134.2 375.3 167.2 419.1 192 451.7C216.8 419.1 249.8 375.3 278.2 329.1C295.3 301.4 310.2 274 320.7 249.2C331.5 223.6 336 204.4 336 192C336 112.5 271.5 48 192 48V48z"/></svg>
                        </i>
                        <div class="div_c_sr">
                           <span class="c-text">${index.DepartureCityEn}</span>
                           <div class="yata_gdemo"><i>${index.CountryEn}</i></div>
                        </div>
                     </li>
                  `);
               }
               ...
               if (e === 'externalHotel' && index.CountryEn !== 'Iran') {
                  let indexCountry = index.CountryFa || index.CountryEn || " ";
                  let cityName = index.DepartureCityFa || index.DepartureCityEn || '';
                  let countryName = index.CountryFa || index.CountryEn || '';
                  let displayName = cityName + '، ' + countryName;

                  // استفاده از selectItem برای شهر خارجی
                  $(targetUl).append(`
                     <li onclick="selectItem('external', null, '${displayName}', null, '${index.CountryEn}', '${index.CountryFa}', '${index.DepartureCityEn}', '${index.DepartureCityFa}')">
                        <i class="div_c_sr_i">
                           <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M272 192C272 236.2 236.2 272 192 272C147.8 272 112 236.2 112 192C112 147.8 147.8 112 192 112C236.2 112 272 147.8 272 192zM192 160C174.3 160 160 174.3 160 192C160 209.7 174.3 224 192 224C209.7 224 224 209.7 224 192C224 174.3 209.7 160 192 160zM384 192C384 279.4 267 435 215.7 499.2C203.4 514.5 180.6 514.5 168.3 499.2C116.1 435 0 279.4 0 192C0 85.96 85.96 0 192 0C298 0 384 85.96 384 192H384zM192 48C112.5 48 48 112.5 48 192C48 204.4 52.49 223.6 63.3 249.2C73.78 274 88.66 301.4 105.8 329.1C134.2 375.3 167.2 419.1 192 451.7C216.8 419.1 249.8 375.3 278.2 329.1C295.3 301.4 310.2 274 320.7 249.2C331.5 223.6 336 204.4 336 192C336 112.5 271.5 48 192 48V48z"/></svg>
                        </i>
                        <div class="div_c_sr">
                           <span class="c-text">${cityName}</span>
                           <div class="yata_gdemo"><i>${indexCountry}</i></div>
                        </div>
                     </li>
                     
                  `);
               } else if (e === 'hotel' || e === 'residence') {
                  let cityName = index.city_name || index.city_name_en || '';

                  // استفاده از selectItem برای شهر داخلی
                  $(targetUl).append(`
                     <li onclick="selectItem('city', '${index.id}', '${cityName}', '${index.city_name_en}', null, null, null, null)">
                        <i class="div_c_sr_i">
                           <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M272 192C272 236.2 236.2 272 192 272C147.8 272 112 236.2 112 192C112 147.8 147.8 112 192 112C236.2 112 272 147.8 272 192zM192 160C174.3 160 160 174.3 160 192C160 209.7 174.3 224 192 224C209.7 224 224 209.7 224 192C224 174.3 209.7 160 192 160zM384 192C384 279.4 267 435 215.7 499.2C203.4 514.5 180.6 514.5 168.3 499.2C116.1 435 0 279.4 0 192C0 85.96 85.96 0 192 0C298 0 384 85.96 384 192H384zM192 48C112.5 48 48 112.5 48 192C48 204.4 52.49 223.6 63.3 249.2C73.78 274 88.66 301.4 105.8 329.1C134.2 375.3 167.2 419.1 192 451.7C216.8 419.1 249.8 375.3 278.2 329.1C295.3 301.4 310.2 274 320.7 249.2C331.5 223.6 336 204.4 336 192C336 112.5 271.5 48 192 48V48z"/></svg>
                        </i>
                        <div class="div_c_sr">
