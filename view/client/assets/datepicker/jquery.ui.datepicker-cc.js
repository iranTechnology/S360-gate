...
;(function ($, undefined) {
    ...
    $.extend(Datepicker.prototype, {
        ...
        // Add these methods to your datepicker object
// Add these methods to your datepicker object

        _onDateHover: function(event, targetId, year, month, day) {
            console.log('_onDateHover called with:', targetId, year, month, day);

            try {
                var inst = this._getInst(document.getElementById(targetId));
                console.log('Got inst:', inst);

                if (!inst || !this._get(inst, 'showRange')) {
                    console.log('No inst or showRange is false');
                    return;
                }

                var hoveredDate = this._daylightSavingAdjust(new this.CDate(year, month, day));
                var minDate = this._getMinMaxDate(inst, 'min');

                console.log('hoveredDate:', hoveredDate, 'minDate:', minDate);

                if (!minDate) {
                    console.log('No minDate set');
                    return;
                }

                // Clear existing range highlights
                this._clearRangeHighlight(inst);

                // Only show range if hovered date is after or equal to min date
                if (this._compareDate(hoveredDate, '>=', minDate)) {
                    console.log('Highlighting range from', minDate, 'to', hoveredDate);
                    this._highlightRange(inst, minDate, hoveredDate);
                }
            } catch (e) {
                console.error('Error in _onDateHover:', e);
            }
        },

// Method to clear range highlights
        _clearRangeHighlight: function(inst) {
            console.log('_clearRangeHighlight called with inst:', inst);

            try {
                if (!inst || !inst.id) {
                    console.log('No inst or inst.id');
                    return;
                }
                $('.ui-datepicker-calendar td').removeClass('ui-datepicker-range-highlight');
                console.log('Cleared highlights for:', inst.id);
            } catch (e) {
                console.error('Error in _clearRangeHighlight:', e);
            }
        },

// Method to highlight date range
        _highlightRange: function(inst, startDate, endDate) {
            console.log('_highlightRange called:', startDate, endDate);
            console.log('startDate details:', startDate.getFullYear(), startDate.getMonth(), startDate.getDate());
            console.log('endDate details:', endDate.getFullYear(), endDate.getMonth(), endDate.getDate());

            try {
                if (!inst || !inst.id) {
                    console.log('No inst or inst.id in _highlightRange');
                    return;
                }

                var datesHighlighted = 0;

                // Debug: Check if jQuery is working and the container exists
                var $container = $('body');
                console.log('Container found:', $container.length);

                var $allTds = $container.find('.ui-datepicker-calendar td');
                console.log('All td elements found:', $allTds.length);

                // Simple approach: iterate through days numerically
                var startDay = startDate.getDate();
                var endDay = endDate.getDate();
                var targetMonth = startDate.getMonth();
                var targetYear = startDate.getFullYear();

                console.log('Looking for days from', startDay, 'to', endDay, 'in month', targetMonth, 'year', targetYear);

                // Find all matching cells
                $allTds.each(function() {
                    var $td = $(this);
                    var cellYear = $td.attr('data-year');
                    var cellMonth = $td.attr('data-month');
                    var cellDay = $td.attr('data-day');

                    console.log('cellYear',cellYear)
                    console.log('cellMonth',cellMonth)
                    console.log('cellDay',cellDay)

                    if (cellYear && cellMonth && cellDay) {
                        var numYear = parseInt(cellYear);
                        var numMonth = parseInt(cellMonth);
                        var numDay = parseInt(cellDay);

                        // Check if this cell is in our target range
                        if (numYear === targetYear && numMonth === targetMonth && numDay >= startDay && numDay <= endDay) {
                            console.log('Found matching cell:', numYear, numMonth, numDay,'hamed');
                            console.log(numYear === targetYear,'numYear === targetYear','hamed')
                            console.log(numMonth === targetMonth,'numMonth === targetMonth','hamed')
                            console.log(numDay >= startDay,'numDay >= startDay','hamed')
                            console.log(numDay <= endDay,'numDay <= endDay','hamed')
                            $td.addClass('ui-datepicker-range-highlight');
                            datesHighlighted++;
                        }
                    }
                });

                console.log('Highlighted', datesHighlighted, 'dates using direct iteration');

                // Alternative approach using direct selector (debugging)
                for (var day = startDay; day <= endDay; day++) {
                    var testSelector = 'td[data-year="' + targetYear + '"][data-month="' + targetMonth + '"][data-day="' + day + '"]';
                    var $testCell = $(testSelector);
                    console.log('Testing selector:', testSelector, 'found:', $testCell.length);

                    if ($testCell.length > 0) {
                        console.log('Would highlight day', day);
                    }
                }

            } catch (e) {
                console.error('Error in _highlightRange:', e);
            }
        },

// Modified _generateHTML method with hover events
        _generateHTML: function (inst) {
            ...
            var showRange = this._get(inst, 'showRange');
            ...
            if (maxDate) {
                ...
                maxDraw =
                   minDate && this._compareDate(maxDraw, '<', minDate)
                      ? minDate
                      : maxDraw;
                while (
                ...
                for (var col = 0; col < numMonths[1]; col++) {
                    ...
                    for (var dRow = 0; dRow < numRows; dRow++) {
                        ...
                        for (var dow = 0; dow < 7; dow++) {
                            ...
                            // Add hover events for show range feature
                            var hoverEvents = '';
                            if (showRange && !unselectable && !otherMonth) {
                                hoverEvents = ' onmouseenter="DP_jQuery_' + dpuuid +
                                   '.datepicker._onDateHover(event, \'' + inst.id +
                                   '\', ' + printDate.getFullYear() +
                                   ', ' + printDate.getMonth() + ', ' + printDate.getDate() + ');"' +
                                   ' onmouseleave="DP_jQuery_' + dpuuid +
                                   '.datepicker._clearRangeHighlight(DP_jQuery_' + dpuuid +
                                   '.datepicker._getInst(document.getElementById(\'' + inst.id + '\')));"';
                            }
...
function gregorian_to_jd(b, c, a) {
    return (
      ...
      Math.floor((b - 1) / 400) +
      Math.floor(
        (367 * c - 362) / 12 + (c <= 2 ? 0 : leap_gregorian(b) ? -1 : -2) + a
      )
    )
}
...
function islamic_to_jd(b, c, a) {
    console.log('islamic_to_jd', b, c, a)
    return (
      a +
      Math.ceil(29.5 * (c - 1)) +
      (b - 1) * 354 +
...
function persian_to_jd(b, e, a) {
    ...
    return (
      a +
      (e <= 7 ? (e - 1) * 31 : (e - 1) * 30 + 6) +
      Math.floor((c * 682 - 110) / 2816) +
      (c - 1) * 365 +
      Math.floor(d / 2820) * 1029983 +
      (PERSIAN_EPOCH - 1)
    )
}
    ...
    a.datepicker.regional.ar = {
        ...
        calculateWeek: function (b) {
            var c = new HijriDate(
              b.getFullYear(),
              b.getMonth(),
              b.getDate() + (b.getDay() || 7) - 3
            )
            return (
              Math.floor(
                Math.round(
                  (c.getTime() - new HijriDate(c.getFullYear(), 0, 1).getTime()) /
                  86400000
                ) / 7
              ) + 1
            )
    ...
    a.datepicker.regional.fa = {
        ...
        calculateWeek: function (b) {
            ...
            return (
              Math.floor(
                Math.round(
                  (c.getTime() - new JalaliDate(c.getFullYear(), 0, 1).getTime()) /
                  86400000
                ) / 7
              ) + 1
            )
...
function JalaliDate(i, h, f) {
    ...
    function j(l) {
        ...
        const g2 = jalaali.toGregorian(l[0], l[1] + 1, l[2])
    ...
    function b(k) {
        ...
        const jalali = jalaali.toJalaali(new Date(k[0], k[1], k[2]))
        var jdate = [jalali.jy, jalali.jm, jalali.jd]
