        ...
        <div class="main-bank-box">
            ...
            {if $smarty.post.bankType neq 'credit'}
                ...
                <div class="txtAlertBank">
                    ...
                    ##Waiteroncepaymentreferringyourbankbacksite##
...
{load_presentation_object filename="BookingTourLocal" assign="objBook"}
{if $smarty.post.bankType neq 'credit'}
    {$objBook->setPortBankForTour($smarty.post.bankType, $smarty.post.factorNumber , $smarty.post.paymentStatus)}
{/if}
{if $redirectBank}
{literal}
...
{$objBank->calculateAmount('reservationTourLocal', $smarty.post.factorNumber )}
{$objBank->executeBank('go')}

{if $objBank->failMessage eq ''}
    {$objBook->sendUserToBankForTour($smarty.post.factorNumber, $smarty.post.paymentStatus)}
{else}
