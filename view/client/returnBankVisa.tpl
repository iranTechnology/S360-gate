{load_presentation_object filename="bookingVisa" assign="objBookingLocal"}
...
{if $smarty.post.flag eq 'credit'}		{*پرداخت از طریق اعتبار*}
	...
	{if $smarty.post.factorNumber neq ''}
        {$successPayment = 'true'}
		{$objBookingLocal->visaBook($smarty.post.factorNumber, 'credit')} {*در جدول book مشخص میکند که پرداخت از اعتبار صورت گرفته*}
    {else}
...
{elseif $smarty.post.flag eq 'currencyPayment'}     {* پرداخت ارزی *}
    ...
    {if $smarty.post.trackingCode neq ''}
        ...
        {$objBookingLocal->visaBook($smarty.post.factorNumber)}
...
{else}		{*پرداخت از طریق بانکها*}
	...
	{if $objBank->transactionStatus neq 'failed' && $objBank->trackingCode neq ''}
		...
		{$objBookingLocal->visaBook($objBank->factorNumber)}
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padr0 padl0 marb40 Clr">
			<div class="main-bank-box">
				...
				<div class="mbb-detail">
					<div class="message-box-car">##uploadingVisaDocumentsDiscriptions##</div>
				</div>
				...
				<div class="s-u-update-popup-change">
					<div class="s-u-bank-item s-u-bank-item-tiket-print">
						<a  id="myBtn" onclick="modalListVisa('{$objBookingLocal->factorNumber}');" class="btn btn-primary-green fa fa-eye site-bg-tsxt-color-b site-bg-color-border-right site-border-main-color">
							<p class="s-u-bank-title"> ##Showvisa##</p>
						</a>
					</div>
					<div class="s-u-bank-item s-u-bank-item-tiket-print">
					...
					<div class="s-u-bank-item s-u-bank-item-tiket-print">
						<a href="{$smarty.const.ROOT_ADDRESS}/UserTracking&type=visa&id={$objBookingLocal->factorNumber}"
