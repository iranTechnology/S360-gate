...
<input type="hidden" name="total_price_reserve" id="total_price_reserve" value="">
...
{*{$objResult->reservationHotelRoom|var_dump}*}
    ...
    {if ($objResult->RoomPrices[$room.id_room]['DBL'][$startDate]['TotalCapacity'] > 0  && $smarty.const.CLIENT_ID == '327') || $smarty.const.CLIENT_ID != '327'}
        ...
        <div class="hotel-detail-room-list special_list_room">
            ...
            <div class="hotel-rooms-item">
                                ...
                                <div class="hotel-rooms-price-col">
                                    <div class="divided-list">
                                        <div class="parent-box-discount--new">
                                            ...
                                            {/if}
                                            {if $objResult->RoomPrices[$room.id_room]['DBL'][$startDate]['Discount'] neq 0}
                                                <span class="discount-percentage">{$objResult->RoomPrices[$room.id_room]['DBL'][$startDate]['Discount']} % </span>
                                            {/if}
                                        </div>

                                        {* --- main DBL price (use ForView if exists) --- *}
                                        {assign var="priceDBLForCalc" value=$objResult->RoomPrices[$room.id_room]['DBL'][$startDate]['PriceOnlineForView']}
                                        {if !$priceDBLForCalc}
                                            {assign var="priceDBLForCalc" value=$objResult->RoomPrices[$room.id_room]['DBL'][$startDate]['PriceOnline']}
                                        {/if}
                                        {assign var="mainCurrency" value=$objFunctions->CurrencyCalculate($priceDBLForCalc, $CurrencyCode, $CurrencyEquivalent)}
                                        ...
                                        <input type="hidden" name="priceRoom{$room.id_room}" id="priceRoom{$room.id_room}" value="{$priceDBLForCalc}">
                                        ...
                                        <span class="title_price">##Priceforanynight## ##Room## </span>
                                        ...
                                        <div class="number_room number_room-reservation">
                                            <i class="plus_room site-bg-main-color" data-type_application="reservation" data-room_token="{$room.id_room}"> + </i>
                                            ...
                                            <i class="minus_room site-bg-main-color" data-type_application="reservation" data-room_token="{$room.id_room}" > - </i>
                                        </div>
                ...
                <input type="hidden" name="total_capacity_{$room.id_room}" id="total_capacity_{$room.id_room}"
                       value="{$objResult->RoomPrices[$room.id_room]['DBL'][$startDate]['TotalCapacity']}">


                {*                <div class="capacity-error-message displayN " id="capacity_error_{$room.id_room}" style="color: red; margin-top: 10px; font-size: 12px;">*}
                {*                    ##CapictyroomFull## {$objResult->RoomPrices[$room.id_room]['DBL'][$startDate]['TotalCapacity'] }*}
                {*                </div>*}
                {*                <div class="capacity-info" style="font-size: 12px; color: #666; margin-top: 5px;">*}
                {*                    <span>##RemainingCapacity##: </span>*}
                {*                    <span id="capacity_display_{$room.id_room}">{$objResult->RoomPrices[$room.id_room]['DBL'][$startDate]['TotalCapacity']}</span>*}
                {*                </div>*}

                {if (isset($objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]) && $objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['PriceOnline'] > 0) ||
                (isset($objResult->RoomPrices[$room.id_room]['EXT'][$startDate]) && $objResult->RoomPrices[$room.id_room]['EXT'][$startDate]['PriceOnline'] > 0)}


                    <div class="divided-list-item">
                        <div class="DetailRoom showECHDPrice disabled" id="btnECHDPrice-{$room.id_room}" data-roomindex="{$room.id_room}" style="opacity: 0.5; pointer-events: none;">
                            <span>##Extrabed##</span>
                            <i class="fa fa-angle-down"></i>
                        </div>
                    </div>

                    <div class="hotel-rooms-row ecdh-price-section displayN" id="echd-section-{$room.id_room}" >

                        <div class="hotel-rooms-local-content-col " id="EchdPriceRoom">
                            <div class="hotel-rooms-content-local w-100">
                                <div class="parent-col--new">
                                    <div class="hotel-rooms-price-col">
                                        <div class="divided-list">
                                            <div class="parent-box-discount--new">
                                                {if $objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['Discount'] neq 0}
                                                    {assign var="disabledCurrencyECHD" value=$objFunctions->CurrencyCalculate($objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['PriceBoardForView'], $CurrencyCode, $CurrencyEquivalent)}
                                                    <strike class="currency priceOff">{$objFunctions->numberFormat($disabledCurrencyECHD.AmountCurrency)}</strike>
                                                {/if}
                                                {if $objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['Discount'] neq 0}
                                                    <span class="discount-percentage">{$objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['Discount']} % </span>
                                                {/if}
                                            </div>

                                            {* --- ECHD price: prefer ForView else fallback to PriceOnline --- *}
                                            {assign var="priceECHDForCalc" value=$objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['PriceOnlineForView']}
                                            {if !$priceECHDForCalc}
                                                {assign var="priceECHDForCalc" value=$objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['PriceOnline']}
                                            {/if}
                                            {assign var="mainCurrencyECHD" value=$objFunctions->CurrencyCalculate($priceECHDForCalc, $CurrencyCode, $CurrencyEquivalent)}
                                            {if $mainCurrencyECHD.AmountCurrency > 0}
                                                <span class="price_number">
                                                    <i class="site-main-text-color">{$objFunctions->numberFormat($mainCurrencyECHD.AmountCurrency)}</i>
                                                    {$mainCurrencyECHD.TypeCurrency}
                                                </span>
                                                <span class="title_price">
                                                    {if $objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['fromAge'] neq 0 || $objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['toAge'] neq 0}
                                                        ##Exterabed##
                                                        {$objFunctions->StrReplaceInXml(['@@from@@'=>$objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['fromAge'], '@@to@@'=>$objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['toAge']],'HotelExtraChildBed')}
                                                    {else}
                                                        ##HotelExtraChild##
                                                    {/if}
                                                </span>
                                                <div class="selsect-room-reserve s-u-bank-title">{if $smarty.const.SOFTWARE_LANG neq 'en' } ##CountExtraChildBed##{/if}</div>
                                                <div class="number_room number_room-reservation">
                                                    <i class="plus_room site-bg-main-color" data-type_application="reservation" data-room_token="{$room.id_room}" data-bed_type="echd" > + </i>
                                                    <input min="0" max="{$room.maximum_extra_chd_beds}" type="number"
                                                           class="val_number_room" value="0"
                                                           id="RoomCountECHD{$room.id_room}"
                                                           name="RoomCountECHD{$room.id_room}"
                                                           onchange="calculateECHDPriceForReservation({$room.id_room})"/>
                                                    <i class="minus_room site-bg-main-color" data-type_application="reservation" data-room_token="{$room.id_room}" data-bed_type="echd"> - </i>
                                                </div>
                                            {/if}
                                            <input type="hidden" name="priceRoomECHD{$room.id_room}" id="priceRoomECHD{$room.id_room}" value="{$priceECHDForCalc}">
                                            <input type="hidden" id="CostkolHotelRoom_ECHD{$room.id_room}"
                                                   value="{$priceECHDForCalc}"
                                                   data-amount="{$mainCurrencyECHD.AmountCurrency}"
                                                   data-unit="{$mainCurrencyECHD.TypeCurrency}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="hotel-rooms-local-content-col" id="ExtPriceRoom">
                            <div class="hotel-rooms-content-local w-100">
                                <div class="parent-col--new">
                                    <div class="hotel-rooms-price-col">
                                        <div class="divided-list">
                                            <div class="parent-box-discount--new">
                                                {if $objResult->RoomPrices[$room.id_room]['EXT'][$startDate]['Discount'] neq 0}
                                                    {assign var="disabledCurrencyEXT" value=$objFunctions->CurrencyCalculate($objResult->RoomPrices[$room.id_room]['EXT'][$startDate]['PriceBoardForView'], $CurrencyCode, $CurrencyEquivalent)}
                                                    <strike class="currency priceOff">{$objFunctions->numberFormat($disabledCurrencyEXT.AmountCurrency)}</strike>
                                                {/if}
                                                {if $objResult->RoomPrices[$room.id_room]['EXT'][$startDate]['Discount'] neq 0}
                                                    <span class="discount-percentage">{$objResult->RoomPrices[$room.id_room]['EXT'][$startDate]['Discount']} % </span>
                                                {/if}
                                            </div>

                                            {* --- EXT price: prefer ForView else fallback to PriceOnline --- *}
                                            {assign var="priceEXTForCalc" value=$objResult->RoomPrices[$room.id_room]['EXT'][$startDate]['PriceOnlineForView']}
                                            {if !$priceEXTForCalc}
                                                {assign var="priceEXTForCalc" value=$objResult->RoomPrices[$room.id_room]['EXT'][$startDate]['PriceOnline']}
                                            {/if}
                                            {assign var="mainCurrencyEXT" value=$objFunctions->CurrencyCalculate($priceEXTForCalc, $CurrencyCode, $CurrencyEquivalent)}

                                            {if $mainCurrencyEXT.AmountCurrency > 0}
                                                <span class="price_number">
                                                    <i class="site-main-text-color">{$objFunctions->numberFormat($mainCurrencyEXT.AmountCurrency)}</i>
                                                    {$mainCurrencyEXT.TypeCurrency}
                                                </span>
                                                <span class="title_price">
                                                    ##HotelExtraBed##
                                                </span>
                                            <div class="selsect-room-reserve s-u-bank-title">{if $smarty.const.SOFTWARE_LANG neq 'en' } ##CountExtraBed##{/if}</div>
                                            <div class="number_room number_room-reservation">
                                                    <i class="plus_room site-bg-main-color" data-type_application="reservation" data-room_token="{$room.id_room}" data-bed_type="ext"> + </i>
                                                    <input min="0" max="{$room.maximum_extra_beds}" type="number"
                                                           class="val_number_room" value="0"
                                                           id="RoomCountEXT{$room.id_room}"
                                                           name="RoomCountEXT{$room.id_room}"
                                                           onchange="calculateEXTPriceForReservation({$room.id_room})"/>
                                                    <i class="minus_room site-bg-main-color" data-type_application="reservation" data-room_token="{$room.id_room}" data-bed_type="ext" > - </i>
                                                </div>
                                            {/if}

                                            <input type="hidden" name="priceRoomEXT{$room.id_room}" id="priceRoomEXT{$room.id_room}" value="{$priceEXTForCalc}">
                                            <input type="hidden" id="CostkolHotelRoom_EXT{$room.id_room}"
                                                   value="{$priceEXTForCalc}"
                                                   data-amount="{$mainCurrencyEXT.AmountCurrency}"
                                                   data-unit="{$mainCurrencyEXT.TypeCurrency}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <input type="hidden" id="FinalECHDCount_Reserve{$room.id_room}" value="" data-amount="">
                <input type="hidden" id="FinalECHDPrice_Reserve{$room.id_room}" value="" data-amount="">
                <input type="hidden" id="FinalEXTCount_Reserve{$room.id_room}" value="" data-amount="">
                <input type="hidden" id="FinalEXTPrice_Reserve{$room.id_room}" value="" data-amount="">
                            ...
                            <div class="box-cancel-rule-col displayN" id="boxCancelRule-{$room.id_room}">
                                <div>
                                    <ul class="nav nav-pills" id="pills-tab-{$room.id_room}" role="tablist">
                                        {* show price tab if at least DBL ForView > 0 (fallback handled above in display) *}
                                        {assign var="dbCheckPrice" value=$objResult->RoomPrices[$room.id_room]['DBL'][$startDate]['PriceOnlineForView']}
                                        {if !$dbCheckPrice}
                                            {assign var="dbCheckPrice" value=$objResult->RoomPrices[$room.id_room]['DBL'][$startDate]['PriceOnline']}
                                        {/if}
                                        {if $dbCheckPrice gt 0}
                                            ...
                                            <div class="tab-pane fade show active" id="price-hotel-details-{$room.id_room}" role="tabpanel" aria-labelledby="price-hotel-details-{$room.id_room}-tab">
                                                ...
                                                <div class='price-hotel-details-grid'>
                                                    {foreach $objResult->RoomPrices[$room.id_room]['DBL'] as $roomPrice}
                                                        ...
                                                        {* --- Use ForView when present, else fallback to PriceOnline --- *}
                                                        {assign var="rPriceForCalc" value=$roomPrice.PriceOnlineForView}
                                                        {if !$rPriceForCalc}
                                                            {assign var="rPriceForCalc" value=$roomPrice.PriceOnline}
                                                        {/if}

                                                        {assign var="extPriceForCalc" value=$objResult->RoomPrices[$room.id_room]['EXT'][$roomPrice.Date]['PriceOnlineForView']}
                                                        {if !$extPriceForCalc}
                                                            {assign var="extPriceForCalc" value=$objResult->RoomPrices[$room.id_room]['EXT'][$roomPrice.Date]['PriceOnline']}
                                                        {/if}

                                                        {assign var="echdPriceForCalc" value=$objResult->RoomPrices[$room.id_room]['ECHD'][$roomPrice.Date]['PriceOnlineForView']}
                                                        {if !$echdPriceForCalc}
                                                            {assign var="echdPriceForCalc" value=$objResult->RoomPrices[$room.id_room]['ECHD'][$roomPrice.Date]['PriceOnline']}
                                                        {/if}

                                                        {$CostRoom = $CostRoom + $rPriceForCalc}
                                                        {$CostBedEXT = $CostBedEXT + $extPriceForCalc}
                                                        {$CostBedCHD = $CostBedCHD + $echdPriceForCalc}
                                                        ...
                                                        {if $roomPrice.RemainingCapacity neq 0}
                                                            <div>
                                                                ...
                                                                {assign var="everyPriceCurrency" value=$objFunctions->CurrencyCalculate($rPriceForCalc, $CurrencyCode, $CurrencyEquivalent)}
                                                                {assign var="childCurrency" value=$objFunctions->CurrencyCalculate($echdPriceForCalc, $CurrencyCode, $CurrencyEquivalent)}
                                                                {assign var="extraBedCurrency" value=$objFunctions->CurrencyCalculate($extPriceForCalc, $CurrencyCode, $CurrencyEquivalent)}
                                                                ...
                                                                <div>
                                                                    ...
                                                                    {if  $objFunctions->numberFormat($extraBedCurrency.AmountCurrency) >0}
                                                                        <h2>
                                                                            ##HotelExtraBed##:
                                                                    ...
                                                                    {/if}
                                                                    {if  $objResult->RoomPrices[$room.id_room]['CHD'][$startDate]['toAge'] >0}
                                                                        <h2 style="background-color: #6BCC6B;">

                                                                            {$objFunctions->StrReplaceInXml(['@@age@@'=>$objResult->RoomPrices[$room.id_room]['CHD'][$startDate]['toAge']],'FreeHotelChildFree')} :
                                                                            ##Free##
                                                                        </h2>
                                                                    {/if}
                                                                </div>
