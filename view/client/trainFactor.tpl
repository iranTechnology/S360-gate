...
{load_presentation_object filename="trainResult" assign="objResult"}
{load_presentation_object filename="trainFactor" assign="objFactor"}
...
{*<code style="color:rebeccapurple;">{$smarty.post|json_encode:256}</code>*}
{assign var="InfoTrain" value=$objDetail->getTrain()}
{assign var="check" value=$objDetail->checkExists($smarty.post.request_number_dept,'trainFactor','train')}
{assign var="doubleCheckPrice" value=$objFactor->doubleCheckPrice($smarty.post.request_number_dept)}
{assign var="check_tpl_factor_doubl_price"  value=true}

{*{if $smarty.server.REMOTE_ADDR == '84.241.4.20'}*}
{*    doubleCheckPrice :  {$smarty.post|json_encode}*}
{*{/if}*}

{if  $doubleCheckPrice eq false}
    {$check_tpl_factor_doubl_price=false}
{/if}

{if $check eq NULL && $check_tpl_factor_doubl_price}
    {assign var="insertRequest" value=['code'=>$smarty.post.request_number_dept,'service'=>'train','method'=>'trainFactor']}
    {assign var='insertCode' value=$apiCodeExist->insertCode($insertRequest)}
    {assign var="InfoTrain" value=$objDetail->getTrain()}
    {*<code style="color #bdae0a;">{$InfoTrain|json_encode:256}</code>*}
    {$objMember->get()}
    ...
    {$objFactor->registerPassengersTrain($smarty.post.SaleId_dept,$InfoTrain[0],'ONEWAY')}


    {*    <code style=" color: pink">after Register</code>*}
    {assign var="list" value=$objFactor->getPassengerList($smarty.post.request_number_dept,$smarty.post.ServiceCode)} {**گرفتن اطلاعات مربوط به مسافران**}

    {*    <code style="color: lightgreen">{$list|json_encode:256}</code>*}
    {*    {$objFunctions->exitCode()}*}

    {if isset($InfoTrain[1]) AND $InfoTrain[1] neq ''}

        {$objFactor->registerPassengersTrain($smarty.post.SaleId_return,$InfoTrain[1],'TOWEWAY')}
        {assign var="listReturn" value=$objFactor->getPassengerList($smarty.post.request_number_return,$smarty.post.ServiceCode)} {**گرفتن اطلاعات مربوط به مسافران**}
        {assign var="doubleCheckPriceReturn" value=$objFactor->doubleCheckPrice($smarty.post.request_number_return)}
        {if $doubleCheckPriceReturn eq false}
            {$check_tpl_factor_doubl_price = false}
        {/if}
    {/if}
    {if $check_tpl_factor_doubl_price}
        <div id="steps">
    ...
    <path d="m384.8 136.04c1.613 3.934 5.718 6.476 9.964 6.148 4.173-.323 7.775-3.265 8.896-7.304 2.362-8.512-7.182-15.758-14.768-11.235-4.197 2.503-5.97 7.884-4.092 12.391z"/>
            ...
            <i class="zmdi zmdi-account-box-mail zmdi-hc-fw mart10"></i> ##Invoice##
                ...
                <div class="s-u-result-wrapper">
                    <ul>
                        ...
                        <!-- result item -->
                        {if $InfoTrain[0]['is_specific'] eq 'yes'}
                        ...
                        <li class="s-u-result-item s-u-result-item-change wow blit-flight-passenger fadeInDown">
                            ...
                            <div class="s-u-result-item-wrapper s-u-result-item-wrapper-change col-xs-9 col-sm-10">
                                    ...
                                    <div class="s-u-result-item-div s-u-result-items-div-change displayib padl15 fltr">
                                        ...
                                        <div class="left-Cell-change">
                                            ...
                                            <i>    {if $objSession->IsLogin()}
                                                    ...
                                                    <i>{$objFunctions->numberFormat({$objFactor->deptAdultPrice} - {$discount['costOff']})}</i>
                                            {else}
                                                <i>{$objFunctions->numberFormat($objFactor->deptAdultPrice)}</i>
                                                {/if}</i>
                                            ##Rial##
                                        </span>
                                ...
                                {if $smarty.post.SellStatus_dept eq '0' && $smarty.post.CheckCoupeReturn eq '1'}
                                ...
                                {elseif $smarty.post.SellStatus_dept eq '1' && $smarty.post.CheckCoupeReturn eq '0'}
                                    {assign var="WagonNumbers" value=$smarty.post.WagonNumbers_dept }
                                    {assign var="CompartmentNumbers" value=$smarty.post.CompartmentNumbers_dept}
                                    <div class="train-info">

                                        -                                    <span>   سالن  {$WagonNumbers} - کوپه  {$CompartmentNumbers} </span>
                        ...
                        {if $InfoTrain[1] neq ''}
                            {if $InfoTrain[1]['is_specific'] eq 'yes'}
                                <div class="ribbon"><span> قطار چارتری  </span></div>
                            {/if}
                            <li class="s-u-result-item s-u-result-item-change wow blit-flight-passenger fadeInDown">
                                ...
                                <div class="s-u-result-item-wrapper s-u-result-item-wrapper-change col-xs-9 col-sm-10">
                                    ...
                                    <div class="details-wrapper-change">
                                        ...
                                        <div class="s-u-result-raft first-row-change">
                                            <div class="s-u-result-item-div  s-u-result-items-div-change right-Cell-change fltr padb5  ">
                                                ...
                                                <div class="s-u-result-item-div s-u-result-items-div-change displayib padl15 fltr">
                                        <span class="s-u-result-item-date-format s-u-result-item-date-format-change iranB">{$objResult->DateJalali($InfoTrain[1]['ExitDate'])}
                                            {$objResult->day},{$objResult->date_now}</span>
                                            ...
                                            <div class="left-Cell-change">
                                                <div class="s-u-bozorg s-u-bozorg-change">
                                                  ...
                                                  <i>    {if $objSession->IsLogin()}
                                                          ...
                                                          <i>{$objFunctions->numberFormat({$objFactor->returnAdultPrice} - {$discountReturn['costOff']})}</i>
                                            {else}
                                                <i>{$objFunctions->numberFormat($objFactor->returnAdultPrice)}</i>
                                                      {/if}</i>
                                            ##Rial##
                                        </span>
                                                </div>
                                    ...
                                    {if $smarty.post.SellStatus_return eq '0' && $smarty.post.CheckCoupeReturn eq '1'}
                                        <div style="padding: 5px;background-color:brown;color:#fff;">به دلیل پراکندگی صندلی ها امکان رزرو کوپه دربست برای این مسیر موجود نمی باشد لطفا مجددا جستجو نمایید</div>
                                    {elseif $smarty.post.SellStatus_dept eq '1' && $smarty.post.CheckCoupe eq '0'}
                                        {assign var="WagonNumbers" value=$smarty.post.WagonNumbers_return }
                                        {assign var="CompartmentNumbers" value=$smarty.post.CompartmentNumbers_return}
                                        <div class="train-info">
                                            <span>   سالن  {$WagonNumbers} - کوپه  {$CompartmentNumbers} </span>
                                        </div>
                                    {else}
        ...
        {if ($smarty.post.SaleId_dept gt '0' && $InfoTrain[1] eq '') || ($InfoTrain[1] neq '' && $smarty.post.SaleId_dept gt '0' && $smarty.post.SaleId_return gt '0')}
                    ...
                    <div class="table-responsive">
                        <table id="passengers" class="display" cellspacing="0" width="100%">
                            ...
                            {foreach $list as $i=>$passenger}
                                <tr>
                                    <td>{if $passenger['passenger_age'] eq 'Adt'} ##Adult## {elseif $passenger['passenger_age'] eq 'Chd'} ##Child## {else} ##Baby## {/if} </td>
                                    <td>
                                    ...
                                    <td>
                                        <p>{if $passenger['passenger_national_code'] eq '' || $passenger['passenger_national_code'] eq '0000000000'}{$passenger['passportNumber']}{else}{$passenger['passenger_national_code']}{/if}</p>
                                    </td>
                                    <td>
...
{*                                    <label class="FilterHoteltypeName site-main-text-color-a  " for="discount_code" >##Ihavediscountcodewantuse##</label>*}
            ...
            <div class="btns_factors_n">
                ...
                <div class="btn_research__">
                    <button type="button" class="loading_on_click  cancel-passenger" onclick="BackToHome('{$objDetail->reSearchAddress}'); return false">##Repeatsearch## <i class="fa fa-refresh"></i> </button>
                </div>
                {if ({$lockseat.Result.SellStatus} eq '0' && $smarty.post.CheckCoupe eq '1')|| ({$lockseatreturn.Result.SellStatus} eq '0' && $smarty.post.CheckCoupeReturn eq '1')}
                    <div></div>
                {else}
                    ...
                    <div class="passengersDetailLocal_next">
                        ...
                        <a class="s-u-check-step s-u-select-flight-change s-u-submit-passenger-Buyer txt15 lh40 site-bg-main-color factorLocal-btn"
                           id="final_ok_and_insert_passenger" data-json='{$objFactor|json_encode}' onclick="bookTrain('{$objFactor->factorNumber}','{$objFactor->UniqueId}')">##Approvefinal##</a>
                    </div>
                {/if}
            </div>
            ...
            <div class="main-pay-content">
                ...
                {assign var="bankInputs" value=['type_service'=>'train','flag' => 'check_credit_train', 'factorNumber' => $objFactor->factorNumber,'ticket_number' => $objFactor->ticket_number,'ticket_number_return' => $objFactor->ticket_number_return, 'serviceType' => $serviceType,'serviceSessionId' => $objFactor->serviceSessionId]}
                ...
                {assign var="creditAction" value="`$smarty.const.ROOT_ADDRESS`/trainReturnBank"}
                ...
                {*        {assign var="currencyAction" value="`$smarty.const.ROOT_ADDRESS`/trainReturnBank"}*}
                {*    {/if}*}
        ...
        {else}
            <code style="color: lightgreen;display:none;">{$lockseat|json_encode}</code>
