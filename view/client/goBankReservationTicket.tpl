...
{if $smarty.post.bankType neq 'credit'}
    {$objBook->setPortBankForReservationTicket($smarty.post.bankType, $smarty.post.factorNumber)}
{/if}
{if $redirectBank}
{literal}
...
{$objBank->calculateAmount('ReservationTicket', $smarty.post.factorNumber, $smarty.post.paymentPrice)}
{$objBank->executeBank('go')}

{if $objBank->failMessage eq ''}
    {$objBook->sendUserToBankForReservationTicket($smarty.post.factorNumber)}
{else}
