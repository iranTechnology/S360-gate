{load_presentation_object filename="bookingInsurance" assign="objBookingLocal"}
...
{if $smarty.post.flag eq 'credit'}		{*پرداخت از طریق اعتبار*}
	...
	{if $smarty.post.factorNumber neq ''}
        {$successPayment = 'true'}
		{$objBookingLocal->insuranceBook($smarty.post.factorNumber, 'credit')} {*در جدول book مشخص میکند که پرداخت از اعتبار صورت گرفته*}
    {else}
...
{elseif $smarty.post.flag eq 'currencyPayment'}     {* پرداخت ارزی *}
    ...
    {if $smarty.post.trackingCode neq ''}
        ...
        {$objBookingLocal->insuranceBook($smarty.post.factorNumber)}

        {if $objBookingLocal->okInsurance eq true}
...
{elseif $PaymentSamanInsurance && !isset($smarty.post.flag)}
	{if $smarty.post.status eq '3'}
		{$successPayment = 'true'}
		{$objBookingLocal->samanInsuranceBook($smarty.post.bimehNo)} {*در جدول book مشخص میکند که پرداخت از اعتبار صورت گرفته*}
	{else}
		{$successPayment = 'false'}
		{$errorPaymentMessage = '##Sorrypayment##'}
	{/if}
{else}		{*پرداخت از طریق بانکها*}
	...
	{if $objBank->transactionStatus neq 'failed' && $objBank->trackingCode neq ''}
		...
		{$objBookingLocal->insuranceBook($objBank->factorNumber)}
			...
			<div class="main-bank-box">
				...
				<div class="s-u-update-popup-change">
					<div class="s-u-bank-item s-u-bank-item-tiket-print">
						<a  id="myBtn" onclick="modalListInsurance('{$objBookingLocal->factorNumber}');" class="btn btn-primary-green fa fa-eye site-bg-tsxt-color-b site-bg-color-border-right site-border-main-color">
							<p class="s-u-bank-title"> ##Showinsurance##</p>
						</a>
					</div>
					<div class="s-u-bank-item s-u-bank-item-tiket-print">
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padr0 padl0 marb40 Clr">
			<div class="main-bank-box">
				...
				<h4 class="mbb-bank-title mbb-bank-title-red">
					<span>##ProblemreservationInsurance##</span>
				</h4>
				<div class="mbb-detail">
					<p class="clearfix txtCenter">
						<span class="txtCenter">
							            ...
							            {if $paymentType eq 'credit'}
                                            {functions::StrReplaceInXml(["@@PaymentType@@"=>"##Validity##","@@typeReserve@@"=>"##ReserveInsurance##"],"ErrorReserveReturnBank")}
                                        {else}
                                            {functions::StrReplaceInXml(["@@PaymentType@@"=>"##Vajh##","@@typeReserve@@"=>"##ReserveInsurance##"],"ErrorReserveReturnBank")}
...
<div id="ModalSendEmail" class="modal" >
    ...
    <div class="modal-content" style="width:56%">
        ...
        <div class="modal-body">
            <div class="row">
                <div class="col-md-12">
                    <label>##SendInsuranceToAnother##</label>
                </div>
        ...
        <div class="modal-footer site-bg-main-color">
            <div class="col-md-12 text-left" >
                ...
                <input type="button" class="btn btn-success margin-10" value="##Send##"  onclick="SendInsuranceEmailForOther();" id="SendInsuranceEmailForOther">
