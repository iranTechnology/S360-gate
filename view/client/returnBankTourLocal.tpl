{load_presentation_object filename="BookingTourLocal" assign="objBookingLocal"}
...
{if $smarty.post.flag eq 'credit'}		{*پرداخت از طریق اعتبار*}
    ...
    {if $smarty.post.factorNumber neq ''}
        {$successPayment = 'true'}
        {$objBookingLocal->tourBook($smarty.post.factorNumber, 'credit')}{*در جدول book مشخص میکند که پرداخت از اعتبار صورت گرفته*}
    {else}
...
{elseif $smarty.post.flag eq 'currencyPayment'}     {* پرداخت ارزی *}
    ...
    {if $smarty.post.trackingCode neq ''}
        ...
        {$objBookingLocal->tourBook($smarty.post.factorNumber, 'cash')}
...
{else}  {*پرداخت از طریق بانکها*}
    ...
    {if $objBank->transactionStatus neq 'failed' && $objBank->trackingCode neq ''}
        ...
        {$objBookingLocal->tourBook($objBank->factorNumber, 'cash')}
...
{if $successPayment eq 'true'}
    ...
    {if $objBookingLocal->okBook eq true}
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padr0 padl0 marb40 Clr">
			<div class="main-bank-box">
                ...
                {if $objBookingLocal->status eq 'PreReserve'}
					<div class="mbb-detail">
						<div class="message-box-car">
                            {assign var="factorNumber" value="<i>`$objBookingLocal->factorNumber`</i>"}
                            {functions::StrReplaceInXml(["@@factorNumber@@"=>$factorNumber],"MessageFactorNumberReturnToure")}
						</div>
					</div>
                {/if}

                {if $objBookingLocal->tourInfo['tour_total_price_a'] gt 0}
					<div class="mbb-detail">
						<div class="message-box-car">
                            {assign var="TourTotalPrice" value="<i>`$objBookingLocal->tourInfo['tour_total_price_a']`</i>"}
                            {assign var="CurrencyTitle" value=$objBookingLocal->tourInfo['currency_title_fa']}
                            {assign var="ClientName" value={$smarty.const.CLIENT_NAME}}
                            {assign var="ClientName" value={$smarty.const.CLIENT_NAME}}


                            {functions::StrReplaceInXml(["@@TourTotalPrice@@"=>$TourTotalPrice,"@@CurrencyTitle@@"=>$CurrencyTitle,"@@ClientName@@"=>"##ClientName##"],"MessageCurrencyReturnTour")}
						</div>
					</div>
                {/if}
				...
				<div class="s-u-update-popup-change">
                    {*<div class="s-u-bank-item s-u-bank-item-tiket-print">
                        <a  id="myBtn" onclick="modalListForTour('{$objBookingLocal->factorNumber}');" class="btn btn-primary-green fa fa-eye site-border-main-color">
                            <p class="s-u-bank-title">مشاهده رزرو </p>
                        </a>
                    </div>*}

                    {*<div class="s-u-bank-item s-u-bank-item-tiket-print">
                        <a href="{$smarty.const.ROOT_ADDRESS}/eTourReservation&num={$objBookingLocal->factorNumber}"  class="btn btn-primary-green fa fa-print site-border-main-color" target="_blank">
                            <p class="s-u-bank-title">چاپ رزرو  </p>
                        </a>
                    </div>*}

                    {if $objBookingLocal->status eq 'BookedSuccessfully' || $objBookingLocal->status eq 'PreReserve'}
						<div class="s-u-bank-item s-u-bank-item-tiket-print">
							<a href="{$smarty.const.ROOT_ADDRESS}/pdf&target=BookingTourLocal&id={$objBookingLocal->factorNumber}" class="btn btn-primary-green fa fa-file-pdf-o site-border-main-color" target="_blank">
								<p class="s-u-bank-title"> ##PdfFile## </p>
							</a>
						</div>
                    {/if}
                    ...
                    {*{$offCode = $objOffCode->offCodeUse($objBookingLocal->tourInfo['factor_number'], $objBookingLocal->tourInfo['serviceTitle'], $objBookingLocal->tourInfo['destination_iata'])}
                    {if $offCode neq ''}
    ...
    {if $paymentType eq 'credit'}
        {functions::StrReplaceInXml(["@@PaymentType@@"=>"##Validity##","@@typeReserve@@"=>"##Tourreservation##"],"ErrorReserveReturnBank")}
    {else}
        {functions::StrReplaceInXml(["@@PaymentType@@"=>"##Vajh##","@@typeReserve@@"=>"##Tourreservation##"],"ErrorReserveReturnBank")}
...
{else}
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padr0 padl0 marb40 Clr">
		<div class="main-bank-box">
			...
			<div class="mbb-detail">
				<p class="clearfix txtCenter">
					<span class="txtCenter">##Sorrypayment##</span>
					<br>
...
<div id="ModalSendEmail" class="modal" >
	...
	<div class="modal-content" style="width:56%">
		...
		<div class="modal-body">
			<div class="row">
				<div class="col-md-12">
					<label>##Alreadyrequestcarforuser##</label>
				</div>
		...
		<div class="modal-footer">
			<div class="col-md-12 text-left" >
				...
				<input type="hidden" id="typeApplication" name="typeApplication" value="tour">
