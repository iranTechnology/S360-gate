...
{$TotalNumberRoom = $TotalNumberRoom + 1}
    ...
    {*                                   {if $currency_price.AmountCurrency gt 0}*}
                                        ...
                                        <span class="title_price">##PriceforanynightDbl##</span>
            ...
            {* نمایش قیمت تخت اضافی کودک (ECHD) *}
            {if isset($objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]) && $objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['PriceOnline'] > 0}
                <!-- دکمه نمایش/مخفی کردن بخش تخت اضافی کودک -->
                <div class="divided-list-item">
                    <div class="DetailRoom showECHDPrice" id="btnECHDPrice-{$room.id_room}" data-roomindex="{$room.id_room}" onclick="toggleECHDPriceSection(event.currentTarget)">
                        <span>##Pricechild##</span>
                        <i class="fa fa-angle-down"></i>
                    </div>
                </div>
                            ...
                            <div class="parent-col--new">
                                <div class="hotel-rooms-price-col">
                                    <div class="divided-list">
                                        ...
                                        {assign var="mainCurrencyECHD" value=$objFunctions->CurrencyCalculate($objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['PriceOnlineForView'], $CurrencyCode, $CurrencyEquivalent)}
                                        <span class="price_number">
                                <i class="site-main-text-color">{$objFunctions->numberFormat($mainCurrencyECHD.AmountCurrency)}</i>
                                {$mainCurrencyECHD.TypeCurrency}
                            </span>
                                        <input type="hidden" name="priceRoomECHD{$room.id_room}" id="priceRoomECHD{$room.id_room}" value="{$objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['PriceOnline']}">

                                        <!-- اضافه کردن data attributes برای قیمت کودک -->
                                        <input type="hidden" id="CostkolHotelRoom_ECHD{$room.id_room}"
                                               value="{$objResult->RoomPrices[$room.id_room]['ECHD'][$startDate]['PriceOnline']}"
                                ...
                                {/if}
                                        ...
                                        {* انتخاب تعداد تخت اضافی کودک *}
                                        <div class="selsect-room-reserve s-u-bank-title">{if $smarty.const.SOFTWARE_LANG neq 'en' } ##CountExtraChildBed##{/if}</div>
                                        <div class="number_room number_room-reservation">
                                            <i class="plus_room site-bg-main-color" data-type_application="reservation" data-room_token="{$room.id_room}" data-bed_type="echd" onclick="changeECHDCount({$room.id_room}, 1)"> + </i>
                                            ...
                                            <i class="minus_room site-bg-main-color" data-type_application="reservation" data-room_token="{$room.id_room}" data-bed_type="echd" onclick="changeECHDCount({$room.id_room}, -1)"> - </i>
                                        </div>
                                    </div>
                                </div>
                            </div>
