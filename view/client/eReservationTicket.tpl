...
{load_presentation_object filename="resultReservationTicket" assign="objResultTicket"}

{assign var='infoTicket' value=$objResultTicket->getInfoTicketReservation($smarty.get.num)}
{assign var='list' value=$infoTicket['infoTicket']}
{*$typeReservation = $objResultTicket->getTypeVehicleByTicketId($list[0]['fk_id_ticket'])*}
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padr0 padl0 s-u-passenger-wrapper-change"
        ...
        <div class="s-u-result-wrapper">
                <span class="s-u-result-item-change direcR iranR txt12 txtRed">
                    ##PleaseClickForPrintReservation##
                    <a class="eticketLocaltext" target="_blank"
                       href="{$smarty.const.ROOT_ADDRESS}/pdf&target=BookingReservationTicket&id={$smarty.get.num}"> ##Here##</a>
                    ##DoClick##
                </span>
        </div>
    </div>
...
<div class="row">
    ...
    <div class="header-col-15 pull-right">
        <img src="project_files/images/logo.png" style="max-height: 50px;">
    </div>

    <div class="header-col-50 pull-right " style="margin-right: 250px;">

        <div class="header-col-50 pull-right font-12 marginInfo">
            <span style="text-align: left">##Invoicenumber## : </span><span>{$list[0]['request_number']}</span>
        </div>
    ...
    <h2 style="font-size: 15px;display: block;text-align: right;margin: 20px 0 10px;font-weight:bold">
        <span>##Inofrmationreservation## </span>
    </h2>
    ...
    <table width="100%" style="border-collapse: collapse;  margin:0px auto">
        ...
        <tr class="trbgColor">
            ...
            {if $list[0]['type_app'] eq 'reservation'}
                <td class="thead-td header-col-7">##Airline##</td>
                <td class="thead-td header-col-7"> ##Numflight##</td>
            {elseif $list[0]['type_app'] eq 'reservationBus'}
                <td class="thead-td header-col-7">##Nametour##</td>
            {else}
                <td class="thead-td header-col-7">##Reservation##</td>
            {/if}
        ...
        <tr>
            ...
            <td class="tbody-td">{$objBookingLocal->set_date_reserve($list[0]['date_flight'])}</td>
            <td class="tbody-td">{$objResultLocal->format_hour($list[0]['time_flight'])}</td>
            <td class="tbody-td">{$infoTicket['totalPrice']|number_format} ##Rial## </td>
        </tr>
    </table>
    <div class="clear-both"></div>
    <br>
    ...
    <table width="100%" style="border-collapse: collapse; margin:0 auto;">
        ...
        {foreach key=key item=item from=$list}
            ...
            <tr>
                ...
                <td class="tbody-td">
                    {if !empty($item['passenger_birthday'])}
                        {$item['passenger_birthday']}
                    {else}
                        {$item['passenger_birthday_en']}
                    {/if}
                </td>
                <td class="tbody-td">
                    {if $item['passenger_age'] =='Adt'} ##Adult##
                    {elseif $item['passenger_age'] =='Chd'}##Child##
                    {elseif $item['passenger_age'] =='Inf'}##Baby##
                    {/if}
                </td>
                <td class="tbody-td">{$Country['titleFa']}</td>
                <td class="tbody-td">
                    {if $item['passenger_age'] =='Adt'}
                        {$item['adt_price']|number_format}
                    {elseif $item['passenger_age'] =='Chd'}
                        {$item['chd_price']|number_format}
                    {elseif $item['passenger_age'] =='Inf'}
                        {$item['inf_price']|number_format}
                    {/if}
                </td>
                <td class="tbody-td">{$item['percent_discount']} %</td>
                <td class="tbody-td">
                    {if $item['percent_discount'] gt 0}
                        {if $item['passenger_age'] =='Adt'}
                            {$item['discount_adt_price']|number_format}
                        {elseif $item['passenger_age'] =='Chd'}
                            {$item['discount_chd_price']|number_format}
                        {elseif $item['passenger_age'] =='Inf'}
                            {$item['discount_inf_price']|number_format}
                        {/if}
                    {else}
                        {if $item['passenger_age'] =='Adt'}
                            {$item['adt_price']|number_format}
