...
{$objBank->calculateAmount('package', $smarty.post.factorNumberHotel, $smarty.post.paymentPrice)}
{$objBank->executeBank('go')}
{load_presentation_object filename="parvazBookingLocal" assign="objBook"}
{load_presentation_object filename="BookingHotelNew" assign="objBookHotel"}
{if $objBank->failMessage eq ''}
    {$objBookHotel->sendUserToBankForHotel($smarty.post.factorNumberHotel)}
    {$objBook->sendUserToBank($smarty.post.factorNumberHotel)}
{else}
