...
{if $smarty.post.bankType neq 'credit'}
    {$objBook->setPortBankVisa($smarty.post.bankType,$smarty.post.factorNumber)}
{/if}
{if $redirectBank}
{literal}
...
{$objBank->calculateAmount('reservationVisa', $smarty.post.factorNumber)}
{$objBank->executeBank('go')}

{if $objBank->failMessage eq ''}
    {$objBook->sendUserToBankForVisa($smarty.post.factorNumber)}
{else}
