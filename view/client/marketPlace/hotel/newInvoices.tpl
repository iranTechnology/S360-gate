...
{if $objSession->IsLogin()}
    ...
    <div class="box-style">
        <div>
            <input type='hidden' id='preview_factor_number' name='preview_factor_number' value=''>
            <div class='parent-text-invoice'>
                <div class='text-danger'>
                    <span class='selected_reserve_count'>0</span>
                    از
                    <span class='all_reserve_count'>0</span>
                    رزرو
                </div>
                برای صدور فاکتور انتخاب شده است. فقط رزروهایی قابلیت صدور فاکتور دارند که وضعیت مالی آنها قابل درخواست باشد.
                <button type='button' class='preview-invoice' onclick='setPreviewDataAndRedirectPdf()' disabled >پیش نمایش</button>
            </div>
        </div>
        <div class="box-style-padding">
            ...
            <form id="FormUserDataSearchFilter" name="FormUserDataSearchFilter" method='post'
                ...
                <input type='hidden' name='statusGroup' id='statusGroup' value='receivable'>
    ...
    <div id="memberResultSearch" class="memberResultSearch">
        ...
        <div class="table-responsive d-none" style="width: 100%;">
            <table id="booking" class="table table-striped text-center">
                ...
                <tr>
                    <th>
                        <input onclick='checkInputAllNewInvoice()' class='input-newInvoice-title' id='input-newInvoice-title' type='checkbox' name='factor_number_list'>
                        <label  class='label-newInvoice-title' for="input-newInvoice-title"></label>
                    </th>
                    <th>شناسه رزرو</th>
                    <th>شماره فاکتور</th>
    ...
    <div class="invoicePopup-overlay" id="popupOverlay">
        <div class="invoicePopup-content">
            <div class='parent-btn-invoicePopup-content'>
                <span id="closePopupButton" class="invoicePopup-close" onclick='closepopupOverlay()'>بازگشت</span>
                <button type='button' onclick="createFacor()">ایجاد فاکتور</button>
            </div>
            <div class='parent-iframe-invoicePopup'>
...
{literal}
    <script>
      $(document).ready(function () {
        setTimeout(function () {
          getFinancialReport('new');
        }, 100);
      });
      function getFinancialReport(type) {
        getHotelFinancialReport(type);
      }
    </script>
{/literal}
{else}
