...
{if $smarty.get.bankType neq 'credit'}
    {$objBook->setPortBankForHotel($smarty.get.bankType, $smarty.get.factorNumber)}
{/if}
...
{$objBank->calculateAmount('hotelLocal', $smarty.get.factorNumber, $smarty.get.paymentPrice)}
{$objBank->executeBank('go')}

{if $objBank->failMessage eq ''}
    {$objBook->sendUserToBankForHotel($smarty.get.factorNumber)}
{else}
