...
{if $smarty.get.bankType neq 'credit'}
    {$objBook->setPortBankForTour($smarty.get.bankType, $smarty.get.factorNumber)}
{/if}
...
{$objBank->calculateAmount('reservationTourLocal', $smarty.get.factorNumber)}
{$objBank->executeBank('go')}

{if $objBank->failMessage eq ''}
    {$objBook->sendUserToBankForTour($smarty.get.factorNumber)}
{else}
