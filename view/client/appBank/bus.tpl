...
{if $smarty.get.bankType neq 'credit'}
    {$objBookingBus->setPortBank($smarty.get.bankType, $smarty.get.factorNumber)}
{/if}
...
{$objBank->calculateAmount('busTicket', $smarty.get.factorNumber, $smarty.get.paymentPrice)}
{$objBank->executeBank('go')}

{if $objBank->failMessage eq ''}
    {$objBookingBus->sendUserToBank($smarty.get.factorNumber)}
{else}
