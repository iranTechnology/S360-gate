...
{assign var="params_credit" value=[
    'factor_number' => $smarty.post.factorNumber,
    'price' => $smarty.post.price,
    'reason'=> $smarty.post.type_pay
]}

{$objCreditDetail->addIncreaseCreditByAgency($params_credit)}

{load_presentation_object filename="bank" assign="objBank"}
{$objBank->initBankParams($smarty.post.bankType)}
{$objBank->calculateAmount('charge_credit_sign', $smarty.post.factorNumber,$smarty.post.price,$smarty.post.additionalData)}
