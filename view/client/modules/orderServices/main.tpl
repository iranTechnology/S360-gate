{load_presentation_object filename="country" assign="objCountry"}
{load_presentation_object filename="orderServicesKind" assign="objKindService"}
{if $smarty.const.SOFTWARE_LANG eq 'en'}
        <link rel='stylesheet' href='assets/styles/css/modules-en/orderServices-en.css'>
{else}
        <link rel='stylesheet' href='assets/modules/css/orderServices.css'>
{/if}



<main>
        <section class="banner-order">
                <div class="container">
                        <div class="row">
                                <div class="col-lg-6 banner_text_parent">
                                        {if $smarty.const.SOFTWARE_LANG == 'fa'}
                                                <div class="banner_text">
                                                        <h2>ما همواره سعی داریم</h2>
                                                        <h3>خدمات متفاوتی را به شما ارائه دهیم</h3>
                                                        <h4>این خدمات از درب منزل شما شروع و تا بازگشت به منزل ادامه دارد</h4>
                                                        <h5>شما می توانید درخواستهای خاص خود اعم از ترانسفر، لیدر، بیمه مسافرتی، کشتی کروز، تورهای لوکس، پروازهای داخلی کشورها و غیره را از طریق فرم زیر از ما بخواهید.</h5>
                                                </div>
                                        {elseif $smarty.const.SOFTWARE_LANG == 'ar'}
{*                                                <div class="banner_text">*}
{*                                                        <h2>نحن نحاول دائما</h2>*}
{*                                                        <h3>نحن نقدم لك خدمات مختلفة</h3>*}
{*                                                        <h4>تبدأ هذه الخدمة عند باب منزلك وتستمر حتى عودتك إلى المنزل</h4>*}
{*                                                        <h5>يمكنكم أن تطلبوا منا طلباتكم الخاصة مثل الإنتقالات والقادة وتأمين السفر والسفن السياحية والجولات الفاخرة والرحلات الداخلية وغيرها من خلال النموذج أدناه.</h5>*}
{*                                                </div>*}
                                        {else}
                                                <div class="banner_text text-left">
                                                        <h2>We always try</h2>
                                                        <h3>We offer you different services</h3>
                                                        <h4>This service starts at your door and continues until you return home</h4>
                                                        <h5>You can ask us for your special requests, such as transfers, leaders, travel insurance, cruise ships, luxury tours, domestic flights, etc. through the form below.</h5>
                                                </div>
                                        {/if}
                                </div>
                                <div class="col-lg-6 d-lg-flex d-none parent-img-banner-order-services">
                                        <img class="banner_img" src="assets/images/banner_img.png" alt="img-banner">
                                </div>
                        </div>
                </div>
        </section>
        <section class="travel_services">
                <div class="">
                        <form data-toggle="validator" id="orderServicesAdd" method="post">
                                <input type="hidden" name="flag" value="orderServicesAdd">
                        <div class="parent-form-private ">
                                <div class="item-input ">
                                        <label for="name">
                                                  <span>
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M476.8 384C472.3 391.7 464.2 396 455.1 396c-4.094 0-8.234-1.031-12.03-3.25L280 297.7V488c0 13.25-10.75 24-24 24s-24-10.75-24-24V297.7l-163.1 95.09C64.25 394.1 60.11 396 56.02 396c-8.266 0-16.33-4.281-20.78-11.97c-6.641-11.47-2.734-26.16 8.719-32.78L208.2 256l-164.2-95.25C32.5 154.1 28.6 139.4 35.24 127.1c6.641-11.5 21.34-15.41 32.81-8.719L232 214.3V24C232 10.75 242.8 0 256 0s24 10.75 24 24v190.3l163.1-95.09c11.48-6.688 26.16-2.781 32.81 8.719c6.641 11.47 2.734 26.16-8.719 32.78L303.8 256l164.2 95.25C479.5 357.9 483.4 372.6 476.8 384z"/></svg>
                                                  </span>
                                                ##Namefamily##

                                        </label>
                                        <input type="text"  id="name" name="name" placeholder="##YourNameFamily##...">
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M224 256c70.7 0 128-57.31 128-128s-57.3-128-128-128C153.3 0 96 57.31 96 128S153.3 256 224 256zM224 32c52.94 0 96 43.06 96 96c0 52.93-43.06 96-96 96S128 180.9 128 128C128 75.06 171.1 32 224 32zM274.7 304H173.3C77.61 304 0 381.6 0 477.3c0 19.14 15.52 34.67 34.66 34.67h378.7C432.5 512 448 496.5 448 477.3C448 381.6 370.4 304 274.7 304zM413.3 480H34.66C33.2 480 32 478.8 32 477.3C32 399.4 95.4 336 173.3 336h101.3C352.6 336 416 399.4 416 477.3C416 478.8 414.8 480 413.3 480z"/></svg></i>
                                </div>
                                <div class="item-input">
                                        <label for="age">##Age##</label>
                                        <input type="text"  id="age" name="age" placeholder="##Age##...">
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M112 0C120.8 0 128 7.164 128 16V64H320V16C320 7.164 327.2 0 336 0C344.8 0 352 7.164 352 16V64H384C419.3 64 448 92.65 448 128V448C448 483.3 419.3 512 384 512H64C28.65 512 0 483.3 0 448V128C0 92.65 28.65 64 64 64H96V16C96 7.164 103.2 0 112 0zM416 192H312V264H416V192zM416 296H312V376H416V296zM416 408H312V480H384C401.7 480 416 465.7 416 448V408zM280 376V296H168V376H280zM168 480H280V408H168V480zM136 376V296H32V376H136zM32 408V448C32 465.7 46.33 480 64 480H136V408H32zM32 264H136V192H32V264zM168 264H280V192H168V264zM384 96H64C46.33 96 32 110.3 32 128V160H416V128C416 110.3 401.7 96 384 96z"/></svg></i>
                                </div>
                                <div class="item-input">
                                        <label for="numberOfRequests">
                                                <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M476.8 384C472.3 391.7 464.2 396 455.1 396c-4.094 0-8.234-1.031-12.03-3.25L280 297.7V488c0 13.25-10.75 24-24 24s-24-10.75-24-24V297.7l-163.1 95.09C64.25 394.1 60.11 396 56.02 396c-8.266 0-16.33-4.281-20.78-11.97c-6.641-11.47-2.734-26.16 8.719-32.78L208.2 256l-164.2-95.25C32.5 154.1 28.6 139.4 35.24 127.1c6.641-11.5 21.34-15.41 32.81-8.719L232 214.3V24C232 10.75 242.8 0 256 0s24 10.75 24 24v190.3l163.1-95.09c11.48-6.688 26.16-2.781 32.81 8.719c6.641 11.47 2.734 26.16-8.719 32.78L303.8 256l164.2 95.25C479.5 357.9 483.4 372.6 476.8 384z"/></svg></span>
                                                ##NumberRequests##
                                        </label>
                                        <input type="text" id="numberOfRequests" name="number_requests" placeholder="##NumberRequests##...">
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M127.1 32C136.8 32 143.1 39.16 143.1 48V297.8L239.1 265.8V48C239.1 39.16 247.2 32 255.1 32C264.8 32 271.1 39.16 271.1 48V255.1L368 223.1V48C368 39.16 375.2 32 384 32C392.8 32 400 39.16 400 48V212.5L496 180.5V48C496 39.16 503.2 32 512 32C520.8 32 528 39.16 528 48V169.8L602.9 144.8C611.3 142 620.4 146.6 623.2 154.9C625.1 163.3 621.4 172.4 613.1 175.2L528 203.5V464C528 472.8 520.8 480 512 480C503.2 480 496 472.8 496 464V214.2L400 246.2V464C400 472.8 392.8 480 384 480C375.2 480 368 472.8 368 464V256.9L271.1 288.9V464C271.1 472.8 264.8 480 255.1 480C247.2 480 239.1 472.8 239.1 464V299.5L143.1 331.5V464C143.1 472.8 136.8 480 127.1 480C119.2 480 111.1 472.8 111.1 464V342.2L37.06 367.2C28.68 369.1 19.62 365.4 16.82 357.1C14.03 348.7 18.56 339.6 26.94 336.8L111.1 308.5V48C111.1 39.16 119.2 32 127.1 32V32z"/></svg></i>
                                </div>
                                <div class="item-input">
                                        <label for="mobile">
                                                <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M476.8 384C472.3 391.7 464.2 396 455.1 396c-4.094 0-8.234-1.031-12.03-3.25L280 297.7V488c0 13.25-10.75 24-24 24s-24-10.75-24-24V297.7l-163.1 95.09C64.25 394.1 60.11 396 56.02 396c-8.266 0-16.33-4.281-20.78-11.97c-6.641-11.47-2.734-26.16 8.719-32.78L208.2 256l-164.2-95.25C32.5 154.1 28.6 139.4 35.24 127.1c6.641-11.5 21.34-15.41 32.81-8.719L232 214.3V24C232 10.75 242.8 0 256 0s24 10.75 24 24v190.3l163.1-95.09c11.48-6.688 26.16-2.781 32.81 8.719c6.641 11.47 2.734 26.16-8.719 32.78L303.8 256l164.2 95.25C479.5 357.9 483.4 372.6 476.8 384z"/></svg></span>
                                                ##Mobile##
                                        </label>
                                        <input type="text"  id="mobile" name="mobile" placeholder="##YourMobileNumber##...">
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M288 0H96C60.65 0 32 28.65 32 64v384c0 35.35 28.65 64 64 64h192c35.35 0 64-28.65 64-64V64C352 28.65 323.3 0 288 0zM320 448c0 17.64-14.36 32-32 32H96c-17.64 0-32-14.36-32-32V64c0-17.64 14.36-32 32-32h192c17.64 0 32 14.36 32 32V448zM224 400H160c-8.836 0-16 7.162-16 16c0 8.836 7.164 16 16 16h64c8.836 0 16-7.164 16-16C240 407.2 232.8 400 224 400z"/></svg></i>
                                </div>
                                <div class="item-input">
                                        <label for="email">
                                                <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M476.8 384C472.3 391.7 464.2 396 455.1 396c-4.094 0-8.234-1.031-12.03-3.25L280 297.7V488c0 13.25-10.75 24-24 24s-24-10.75-24-24V297.7l-163.1 95.09C64.25 394.1 60.11 396 56.02 396c-8.266 0-16.33-4.281-20.78-11.97c-6.641-11.47-2.734-26.16 8.719-32.78L208.2 256l-164.2-95.25C32.5 154.1 28.6 139.4 35.24 127.1c6.641-11.5 21.34-15.41 32.81-8.719L232 214.3V24C232 10.75 242.8 0 256 0s24 10.75 24 24v190.3l163.1-95.09c11.48-6.688 26.16-2.781 32.81 8.719c6.641 11.47 2.734 26.16-8.719 32.78L303.8 256l164.2 95.25C479.5 357.9 483.4 372.6 476.8 384z"/></svg></span>
                                                ##Email##
                                        </label>
                                        <input type="text" id="email" name="email"  placeholder="example@site.com">
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M0 128C0 92.65 28.65 64 64 64H448C483.3 64 512 92.65 512 128V384C512 419.3 483.3 448 448 448H64C28.65 448 0 419.3 0 384V128zM32 128V167.9L227.6 311.3C244.5 323.7 267.5 323.7 284.4 311.3L480 167.9V128C480 110.3 465.7 96 448 96H63.1C46.33 96 31.1 110.3 31.1 128H32zM32 207.6V384C32 401.7 46.33 416 64 416H448C465.7 416 480 401.7 480 384V207.6L303.3 337.1C275.1 357.8 236.9 357.8 208.7 337.1L32 207.6z"/></svg></i>
                                </div>
                                <div class="item-input">
                                        <label for="address">##Address##</label>
                                        <input type="text" id="address" name="address" placeholder="##YourAddress##...">
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M272 192C272 236.2 236.2 272 192 272C147.8 272 112 236.2 112 192C112 147.8 147.8 112 192 112C236.2 112 272 147.8 272 192zM192 240C218.5 240 240 218.5 240 192C240 165.5 218.5 144 192 144C165.5 144 144 165.5 144 192C144 218.5 165.5 240 192 240zM384 192C384 279.4 267 435 215.7 499.2C203.4 514.5 180.6 514.5 168.3 499.2C116.1 435 0 279.4 0 192C0 85.96 85.96 0 192 0C298 0 384 85.96 384 192H384zM192 32C103.6 32 32 103.6 32 192C32 207.6 37.43 229 48.56 255.4C59.47 281.3 74.8 309.4 92.14 337.5C126.2 392.8 166.6 445.7 192 477.6C217.4 445.7 257.8 392.8 291.9 337.5C309.2 309.4 324.5 281.3 335.4 255.4C346.6 229 352 207.6 352 192C352 103.6 280.4 32 192 32z"/></svg></i>
                                </div>
                                <div class="item-input state">
                                        <label>
                                                <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M476.8 384C472.3 391.7 464.2 396 455.1 396c-4.094 0-8.234-1.031-12.03-3.25L280 297.7V488c0 13.25-10.75 24-24 24s-24-10.75-24-24V297.7l-163.1 95.09C64.25 394.1 60.11 396 56.02 396c-8.266 0-16.33-4.281-20.78-11.97c-6.641-11.47-2.734-26.16 8.719-32.78L208.2 256l-164.2-95.25C32.5 154.1 28.6 139.4 35.24 127.1c6.641-11.5 21.34-15.41 32.81-8.719L232 214.3V24C232 10.75 242.8 0 256 0s24 10.75 24 24v190.3l163.1-95.09c11.48-6.688 26.16-2.781 32.81 8.719c6.641 11.47 2.734 26.16-8.719 32.78L303.8 256l164.2 95.25C479.5 357.9 483.4 372.6 476.8 384z"/></svg></span>
                                                ##Country##
                                        </label>
                                        <select class="select2 selectCountry" name="country" id="country">
                                                <option value="" disabled selected>
                                                        ##ChoseOption##
                                                </option>

                                                {foreach $objCountry->getAllCountries() as $key => $country}

                                                        <option value="{$country['id']}">
                                                                {if $smarty.const.SOFTWARE_LANG eq 'fa'}
                                                                        {$country['titleFa']}
                                                                {else}
                                                                        {$country['titleEn']}
                                                                {/if}
                                                        </option>
                                                {/foreach}
                                        </select>
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M195.9 352.5C204.5 354.6 209.7 363.3 207.5 371.9C205.4 380.5 196.7 385.7 188.1 383.5L156.1 375.5C147.5 373.4 142.3 364.7 144.5 356.1C146.6 347.5 155.3 342.3 163.9 344.5L195.9 352.5zM248.1 347.5C239.5 345.4 234.3 336.7 236.5 328.1L244.5 296.1C246.6 287.5 255.3 282.3 263.9 284.5C272.5 286.6 277.7 295.3 275.5 303.9L267.5 335.9C265.4 344.5 256.7 349.7 248.1 347.5zM350.3 175.2C346.4 183.1 336.7 186.3 328.8 182.3C320.9 178.4 317.7 168.7 321.7 160.8L337.7 128.8C341.6 120.9 351.3 117.7 359.2 121.7C367.1 125.6 370.3 135.3 366.3 143.2L350.3 175.2zM512 256C512 397.4 397.4 512 256 512C114.6 512 0 397.4 0 256C0 114.6 114.6 0 256 0C397.4 0 512 114.6 512 256zM345.8 50.7L323.4 106.5C318.6 118.5 308.4 127.4 295.1 130.5L278.9 134.8C273.4 136.1 271.1 142.7 274.5 147.3L290.6 168.8C300.8 182.4 301.3 200.9 291.9 215L281.2 231.1C270.8 246.6 253.3 256 234.6 256H232.3C226.9 256 223.1 261.2 224.6 266.3L230.6 286.2C238 311 219.4 336 193.5 336C176.7 336 161.4 326.5 153.9 311.5L144.6 292.9C142.1 288.1 135.6 286.1 131.8 290.9L116.4 306.2C106.9 315.7 93.38 319.8 80.27 317.2L38.25 308.8C61.97 407 150.5 480 255.1 480C303.4 480 347.4 465.3 383.6 440.1L375.1 433.1L364.5 437.9C336.9 448.2 305.1 435.3 293.9 408.3L288.4 395.1C276.5 369.5 286.8 338.3 312.2 324.2L348.4 303.1L354.5 294.5C363.4 280.5 378.9 272 395.5 272C412.1 272 427.6 280.5 436.6 294.5L438.5 297.4L454.1 293C461.6 290.9 469.5 291.1 476.9 293.4C478.9 281.2 480 268.7 480 256C480 164.2 424.8 85.31 345.8 50.71L345.8 50.7zM462.7 323.9L446.9 328.3C433.4 332 419.1 326.6 411.6 314.8L409.6 311.7C406.5 306.9 401.2 304 395.5 304C389.8 304 384.5 306.9 381.4 311.7L375.3 321.2C372.5 325.7 368.6 329.4 363.1 331.9L327.8 352.1C316.9 358.2 312.5 371.5 317.6 382.9L323.1 395.3C328.3 406.8 341.5 412.3 353.4 407.9L372 401C377 399.1 382.7 399.9 386.1 403L409.4 419.3C436.6 393.6 457.5 361.3 469.2 324.8L468.4 324.4C466.6 323.5 464.6 323.3 462.7 323.9V323.9zM315.6 40.01C296.6 34.79 276.6 32 256 32C132.3 32 32 132.3 32 256C32 262.4 32.27 268.8 32.8 275L86.54 285.8C89.17 286.3 91.88 285.5 93.77 283.6L109.1 268.2C128.4 248.9 160.1 254.2 173.2 278.6L182.5 297.2C184.6 301.4 188.8 304 193.5 304C197.1 304 201.2 299.7 199.9 295.4L193.9 275.5C186.2 249.8 205.5 224 232.3 224H234.6C242.6 224 250.1 219.1 254.6 213.3L265.3 197.3C267.1 194.4 267 190.7 265 188L248.9 166.5C231.7 143.6 243.4 110.6 271.2 103.7L288.2 99.45C290.7 98.83 292.7 97.05 293.7 94.66L315.6 40.01z"/></svg></i>
                                </div>
                                <div class="item-input">
                                        <label for="field-Study">
                                                <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M476.8 384C472.3 391.7 464.2 396 455.1 396c-4.094 0-8.234-1.031-12.03-3.25L280 297.7V488c0 13.25-10.75 24-24 24s-24-10.75-24-24V297.7l-163.1 95.09C64.25 394.1 60.11 396 56.02 396c-8.266 0-16.33-4.281-20.78-11.97c-6.641-11.47-2.734-26.16 8.719-32.78L208.2 256l-164.2-95.25C32.5 154.1 28.6 139.4 35.24 127.1c6.641-11.5 21.34-15.41 32.81-8.719L232 214.3V24C232 10.75 242.8 0 256 0s24 10.75 24 24v190.3l163.1-95.09c11.48-6.688 26.16-2.781 32.81 8.719c6.641 11.47 2.734 26.16-8.719 32.78L303.8 256l164.2 95.25C479.5 357.9 483.4 372.6 476.8 384z"/></svg></span>
                                                ##RequestedService##
                                        </label>
                                        <select class="select2" name="kind_service" id="kind_service">
                                                <option value="" disabled selected>
                                                        ##ChoseOption##
                                                </option>
                                                {foreach $objKindService->getAllServicesKind() as $key => $kind}
                                                        <option value="{$kind['id']}">
                                                                {if $smarty.const.SOFTWARE_LANG eq 'en'}
                                                                        {$kind['titleEn']}
                                                                {else}
                                                                        {$kind['titleFa']}
                                                                {/if}
                                                        </option>
                                                {/foreach}
                                        </select>
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M160 256C160 202.1 202.1 160 256 160C309 160 352 202.1 352 256C352 309 309 352 256 352C202.1 352 160 309 160 256zM256 208C229.5 208 208 229.5 208 256C208 282.5 229.5 304 256 304C282.5 304 304 282.5 304 256C304 229.5 282.5 208 256 208zM293.1 .0003C315.3 .0003 334.6 15.19 339.8 36.74L347.6 69.21C356.1 73.36 364.2 78.07 371.9 83.28L404 73.83C425.3 67.56 448.1 76.67 459.2 95.87L496.3 160.1C507.3 179.3 503.8 203.6 487.8 218.9L463.5 241.1C463.8 246.6 464 251.3 464 256C464 260.7 463.8 265.4 463.5 270L487.8 293.1C503.8 308.4 507.3 332.7 496.3 351.9L459.2 416.1C448.1 435.3 425.3 444.4 404 438.2L371.9 428.7C364.2 433.9 356.1 438.6 347.6 442.8L339.8 475.3C334.6 496.8 315.3 512 293.1 512H218.9C196.7 512 177.4 496.8 172.2 475.3L164.4 442.8C155.9 438.6 147.8 433.9 140.1 428.7L107.1 438.2C86.73 444.4 63.94 435.3 52.85 416.1L15.75 351.9C4.66 332.7 8.168 308.4 24.23 293.1L48.47 270C48.16 265.4 48 260.7 48 255.1C48 251.3 48.16 246.6 48.47 241.1L24.23 218.9C8.167 203.6 4.66 179.3 15.75 160.1L52.85 95.87C63.94 76.67 86.73 67.56 107.1 73.83L140.1 83.28C147.8 78.07 155.9 73.36 164.4 69.21L172.2 36.74C177.4 15.18 196.7 0 218.9 0L293.1 .0003zM205.5 103.6L194.3 108.3C181.6 113.6 169.8 120.5 159.1 128.7L149.4 136.1L94.42 119.9L57.31 184.1L98.81 223.6L97.28 235.6C96.44 242.3 96 249.1 96 256C96 262.9 96.44 269.7 97.28 276.4L98.81 288.4L57.32 327.9L94.42 392.1L149.4 375.9L159.1 383.3C169.8 391.5 181.6 398.4 194.3 403.7L205.5 408.4L218.9 464H293.1L306.5 408.4L317.7 403.7C330.4 398.4 342.2 391.5 352.9 383.3L362.6 375.9L417.6 392.1L454.7 327.9L413.2 288.4L414.7 276.4C415.6 269.7 416 262.9 416 256C416 249.1 415.6 242.3 414.7 235.6L413.2 223.6L454.7 184.1L417.6 119.9L362.6 136.1L352.9 128.7C342.2 120.5 330.4 113.6 317.7 108.3L306.5 103.6L293.1 48H218.9L205.5 103.6z"/></svg></i>
                                </div>
                                <div class="item-input">
                                        <label for="field-Study">##Description##</label>
                                        <input type="text"  id="comment" name="comment"  placeholder="##Description##...">
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M448 56v80C448 149.3 437.3 160 424 160S400 149.3 400 136V80h-152v352h48c13.25 0 24 10.75 24 24S309.3 480 296 480h-144C138.8 480 128 469.3 128 456s10.75-24 24-24h48v-352H48v56C48 149.3 37.25 160 24 160S0 149.3 0 136v-80C0 42.75 10.75 32 24 32h400C437.3 32 448 42.75 448 56z"/></svg></i>
                                </div>
                                <div class="item-input">
                                        <label for="request-rights">
                                                <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M476.8 384C472.3 391.7 464.2 396 455.1 396c-4.094 0-8.234-1.031-12.03-3.25L280 297.7V488c0 13.25-10.75 24-24 24s-24-10.75-24-24V297.7l-163.1 95.09C64.25 394.1 60.11 396 56.02 396c-8.266 0-16.33-4.281-20.78-11.97c-6.641-11.47-2.734-26.16 8.719-32.78L208.2 256l-164.2-95.25C32.5 154.1 28.6 139.4 35.24 127.1c6.641-11.5 21.34-15.41 32.81-8.719L232 214.3V24C232 10.75 242.8 0 256 0s24 10.75 24 24v190.3l163.1-95.09c11.48-6.688 26.16-2.781 32.81 8.719c6.641 11.47 2.734 26.16-8.719 32.78L303.8 256l164.2 95.25C479.5 357.9 483.4 372.6 476.8 384z"/></svg></span>
                                                ##DateStart##
                                        </label>
                                        <input type="text"  id="date_start" name="date_start" placeholder="##Enterdate##...">
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M112 0C120.8 0 128 7.164 128 16V64H320V16C320 7.164 327.2 0 336 0C344.8 0 352 7.164 352 16V64H384C419.3 64 448 92.65 448 128V448C448 483.3 419.3 512 384 512H64C28.65 512 0 483.3 0 448V128C0 92.65 28.65 64 64 64H96V16C96 7.164 103.2 0 112 0zM416 192H312V264H416V192zM416 296H312V376H416V296zM416 408H312V480H384C401.7 480 416 465.7 416 448V408zM280 376V296H168V376H280zM168 480H280V408H168V480zM136 376V296H32V376H136zM32 408V448C32 465.7 46.33 480 64 480H136V408H32zM32 264H136V192H32V264zM168 264H280V192H168V264zM384 96H64C46.33 96 32 110.3 32 128V160H416V128C416 110.3 401.7 96 384 96z"></path></svg></i>
                                </div>
                                <div class="item-input">
                                        <label for="request2-rights">
                                                <span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M476.8 384C472.3 391.7 464.2 396 455.1 396c-4.094 0-8.234-1.031-12.03-3.25L280 297.7V488c0 13.25-10.75 24-24 24s-24-10.75-24-24V297.7l-163.1 95.09C64.25 394.1 60.11 396 56.02 396c-8.266 0-16.33-4.281-20.78-11.97c-6.641-11.47-2.734-26.16 8.719-32.78L208.2 256l-164.2-95.25C32.5 154.1 28.6 139.4 35.24 127.1c6.641-11.5 21.34-15.41 32.81-8.719L232 214.3V24C232 10.75 242.8 0 256 0s24 10.75 24 24v190.3l163.1-95.09c11.48-6.688 26.16-2.781 32.81 8.719c6.641 11.47 2.734 26.16-8.719 32.78L303.8 256l164.2 95.25C479.5 357.9 483.4 372.6 476.8 384z"/></svg></span>
                                                ##DateEnd##
                                        </label>
                                        <input type="text"  id="date_end" name="date_end" placeholder="##Exitdate##...">
                                        <i class="icon-input-item"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M112 0C120.8 0 128 7.164 128 16V64H320V16C320 7.164 327.2 0 336 0C344.8 0 352 7.164 352 16V64H384C419.3 64 448 92.65 448 128V448C448 483.3 419.3 512 384 512H64C28.65 512 0 483.3 0 448V128C0 92.65 28.65 64 64 64H96V16C96 7.164 103.2 0 112 0zM416 192H312V264H416V192zM416 296H312V376H416V296zM416 408H312V480H384C401.7 480 416 465.7 416 448V408zM280 376V296H168V376H280zM168 480H280V408H168V480zM136 376V296H32V376H136zM32 408V448C32 465.7 46.33 480 64 480H136V408H32zM32 264H136V192H32V264zM168 264H280V192H168V264zM384 96H64C46.33 96 32 110.3 32 128V160H416V128C416 110.3 401.7 96 384 96z"></path></svg></i>
                                </div>
                                <div class="item-input">
                                        <label for="request2-rights">##AttachedFile##
                                        </label>
                                        <input type="text" name="fileText" id="fileText" class="btn-upload-fara file_input_replacement" placeholder="##AttachedFile##">
                                        <input type="file" name="file" id="file" class="file_input_with_replacement">
                                        <i class="icon-input-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M272.1 191c-9.375-9.375-24.56-9.375-33.94 0l-80 80c-9.375 9.375-9.375 24.56 0 33.94s24.56 9.375 33.94 0L232 265.9V368c0 13.25 10.75 24 24 24s24-10.75 24-24V265.9l39.03 39.03C323.7 309.7 329.8 312 336 312s12.28-2.344 16.97-7.031c9.375-9.375 9.375-24.56 0-33.94L272.1 191zM448 96h-172.1L226.7 50.75C214.7 38.74 198.5 32 181.5 32H64C28.65 32 0 60.66 0 96v320c0 35.34 28.65 64 64 64h384c35.35 0 64-28.66 64-64V160C512 124.7 483.3 96 448 96zM464 416c0 8.824-7.178 16-16 16H64c-8.822 0-16-7.176-16-16V96c0-8.824 7.178-16 16-16h117.5c4.273 0 8.293 1.664 11.31 4.688L256 144h192c8.822 0 16 7.176 16 16V416z"/></svg>
                                        </i>
                                </div>
                        </div>
                        <div class="col-12 mt-5 d-flex justify-content-center align-items-center">
                                <button type="submit" class="btn-submit" id='orderServiceButton'>
                                        ##Submitapplication##
                                </button>
                        </div>
                        </form>
                </div>
        </section>

</main>
{literal}
        <script src="assets/modules/js/orderServices.js"></script>
{/literal}