{load_presentation_object filename="bookingVisa" assign="objEntertainment"}
...
{if $smarty.post.flag eq 'credit'}		{*پرداخت از طریق اعتبار*}
	...
	{if $smarty.post.factorNumber neq ''}
        {$successPayment = 'true'}
		{$objEntertainment->visaBook($smarty.post.factorNumber, 'credit')} {*در جدول book مشخص میکند که پرداخت از اعتبار صورت گرفته*}
    {else}
...
{elseif $smarty.post.flag eq 'currencyPayment'}     {* پرداخت ارزی *}
    ...
    {if $smarty.post.trackingCode neq ''}
		...
		{$objEntertainment->updateBank($smarty.post.trackingCode, $smarty.post.factorNumber)}
        {$objEntertainment->visaBook($smarty.post.factorNumber)}

        {if $objEntertainment->okBook eq true}
    ...
    {else}
        ...
        {$objEntertainment->delete_transaction_current($smarty.post.factorNumber)}
    {/if}
...
{else}		{*پرداخت از طریق بانکها*}
	...
	{if $objBank->transactionStatus neq 'failed' && $objBank->trackingCode neq ''}
		...
		{$objEntertainment->updateBank($objBank->trackingCode,$objBank->factorNumber)}
		{$objEntertainment->visaBook($objBank->factorNumber)}
	...
	{else}
		...
		{$objEntertainment->delete_transaction_current($objBank->factorNumber)}
	{/if}
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padr0 padl0 marb40 Clr">
			<div class="main-bank-box">
				...
				<div class="mbb-detail">
					<div class="message-box-car">##uploadingEntertainmentDocumentsDiscriptions##</div>
				</div>
				...
				<div class="mbb-detail">
					...
					<p class="clearfix">
						...
						<span class="pull-left">{$objFunctions->set_date_payment($objEntertainment->payment_date)}</span>
					</p>
					<p class="clearfix">
						<span class="pull-right"> ##Invoicenumber##</span>
						<span class="pull-left">{$objEntertainment->factorNumber}</span>
					</p>
				</div>

				<div class="s-u-update-popup-change">
					<div class="s-u-bank-item s-u-bank-item-tiket-print">
						<a  id="myBtn" href="{$smarty.const.ROOT_ADDRESS}/pdf&target=entertainment&id={$objEntertainment->factorNumber}" class="btn btn-primary-green fa fa-eye site-bg-tsxt-color-b site-bg-color-border-right site-border-main-color">
							<p class="s-u-bank-title"> ##ShowEntertainment##</p>
						</a>
					</div>
					<div class="s-u-bank-item s-u-bank-item-tiket-print">
						<a onclick="modalEmail('{$objEntertainment->factorNumber}');"  class="btn btn-primary-green fa fa-envelope-o site-bg-tsxt-color-b site-bg-color-border-right site-border-main-color" target="_blank">
							<p class="s-u-bank-title">##Sendemail## </p>
						</a>
					</div>

					<div class="s-u-bank-item s-u-bank-item-tiket-print">
						<a href="{$smarty.const.ROOT_ADDRESS}/UserTracking&type=entertainment&id={$objEntertainment->factorNumber}"
						   class="btn btn-primary-green fa fa-envelope-o site-bg-tsxt-color-b site-bg-color-border-right site-border-main-color"
						   target="_blank">
							<p class="s-u-bank-title">##TrackOrder##</p>
						</a>
					</div>

                    {* نمایش کد ترانسفر پس از خرید موفق *}
                    {*{$offCode = $objOffCode->offCodeUse($objEntertainment->visaInfo['factor_number'], $objEntertainment->visaInfo['serviceTitle'], $objEntertainment->visaInfo['visa_destination_code'])}
                    {if $offCode neq ''}
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padr0 padl0 marb40 Clr">
			<div class="main-bank-box">
				...
				<h4 class="mbb-bank-title mbb-bank-title-red">
					<span>##Errorreservationvisa##</span>
				</h4>
				<div class="mbb-detail">
					<p class="clearfix txtCenter">
						<span class="txtCenter">
							    {if $paymentType eq 'credit'}
                                    {functions::StrReplaceInXml(["@@PaymentType@@"=>"##Validity##","@@typeReserve@@"=>"##BookTicket##"],"Reservationvisa")}
                                {else}
                                    {functions::StrReplaceInXml(["@@PaymentType@@"=>"##Vajh##","@@typeReserve@@"=>"##BookTicket##"],"Reservationvisa")}
