        ...
        <div class="main-bank-box">
            ...
            {if $smarty.post.bankType neq 'credit'}
                ...
                <div class="txtAlertBank">
                    ##Hotelwaderissuedpaymentbankredirectedagain##
                </div>
            {/if}
        </div>
    </div>
...
{load_presentation_object filename="BookingHotelLocal" assign="objBook"}
{if $smarty.post.bankType neq 'credit'}
    {$objBook->setPortBankForHotel($smarty.post.bankType, $smarty.post.factorNumber , $smarty.post.paymentStatus)}
{/if}
{if $redirectBank}
{literal}
...
{$objBank->calculateAmount('hotelLocal', $smarty.post.factorNumber, $smarty.post.paymentPrice)}
{$objBank->executeBank('go')}

{if $objBank->failMessage eq ''}
    {$objBook->sendUserToBankForHotel($smarty.post.factorNumber)}
{else}
