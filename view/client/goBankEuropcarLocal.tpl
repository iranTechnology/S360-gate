...
{load_presentation_object filename="BookingEuropcarLocal" assign="objBook"}
{if $smarty.post.bankType neq 'credit'}
    {$objBook->setPortBankForCar($smarty.post.bankType, $smarty.post.factorNumber)}
{/if}
...
{$objBank->calculateAmount('europcarLocal', $smarty.post.factorNumber)}
{$objBank->executeBank('go')}

{if $objBank->failMessage eq ''}
    {$objBook->sendUserToBankForCar($smarty.post.factorNumber)}
{else}
