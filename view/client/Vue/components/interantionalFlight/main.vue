<template>
  ...
  <div class="row">
    ...
    <sidebar :dataSearch="setDataSearch" :price="price" :interrupt="interrupt" :timeFilter="timeFilter" :timeArrivalFilter="timeArrivalFilter"
             :typeFlightFilter="typeFlightFilter" :seatClassFilter="seatClassFilter"  :transitAirports="transitAirports"
             :minPriceAirline="minPriceAirline" :baggageFilter="baggageFilter" :countFlights="count_flights"
             @filterFlights="filterFlightsFinally" @filterTransitDuration="filterTransitDuration"
...
export default {
  ...
  data() {
    return {
      ...
      arrival_time_filter: ['all_arrival_time'],
      airline_filter: ['all_airline'],
      seat_class_filter: ['all_seat_class'],
      transit_airports_filter: ['all_transit_airports'],
      transit_duration_filter: [0, 24],
      price_filter_flight: [],
  ...
  methods: {
    ...
    filterTransitDuration(value) {
      this.transit_duration_filter = value;
    },
    filterFlightsFinally(value, type) {
      ...
      switch (type) {
        ...
        case 'time':
          ...
          break;
        case 'arrival_time':
          if (this.arrival_time_filter.indexOf(value) !== -1) {
            if (value.indexOf('all') === -1) {
              document.getElementsByClassName(value)[0].classList.remove("checked");
            }
            this.arrival_time_filter = this.deleteByValue(this.arrival_time_filter, value);
          } else {
            document.getElementsByClassName(value)[0].classList.add("checked");
            this.arrival_time_filter.push(value);
          }
          if (this.arrival_time_filter.length === 1 && this.arrival_time_filter.indexOf('all') === -1) {
            document.getElementsByClassName('all_arrival_time')[0].classList.add("checked");
          }
          break;
        case 'transit_airports':

          if (this.transit_airports_filter.indexOf(value) !== -1) {
            if (value.indexOf('all') === -1) {
              let elements = document.getElementsByClassName(value);
              if (elements && elements.length > 0) {
                for (let i = 0; i < elements.length; i++) {
                  if (elements[i] && elements[i].classList.contains('transit_airport')) {
                    elements[i].classList.remove("checked");
                    break;
                  }
                }
              } else {
                console.warn('⚠️ No elements found with class:', value);
              }
            }
            this.transit_airports_filter = this.deleteByValue(this.transit_airports_filter, value);
          } else {
            let elements = document.getElementsByClassName(value);
            if (elements && elements.length > 0) {
              for (let i = 0; i < elements.length; i++) {
            ...
            this.transit_airports_filter.push(value);
          }

          // اگر فقط یک گزینه غیر از "همه" انتخاب شده، "همه" را غیرفعال کن
          if (this.transit_airports_filter.length === 1 && this.transit_airports_filter.indexOf('all') === -1) {
            let allElements = document.getElementsByClassName('all_transit_airports');
            if (allElements && allElements.length > 0) {
              for (let i = 0; i < allElements.length; i++) {
                if (allElements[i]) {
                  allElements[i].classList.remove("checked");
                }
              }
            }
          }

          // اگر "همه" انتخاب شده، همه گزینه‌ها را غیرفعال کن و "همه" را فعال کن
          if (value === 'all_transit_airports') {
            // غیرفعال کردن همه گزینه‌ها
            let allOptions = document.getElementsByClassName('transit_airport');
            if (allOptions && allOptions.length > 0) {
              for (let i = 0; i < allOptions.length; i++) {
                if (allOptions[i]) {
                  allOptions[i].classList.remove("checked");
                }
              }
            }
            // فعال کردن "همه"
            ...
            if (allElements && allElements.length > 0) {
              for (let i = 0; i < allElements.length; i++) {
                if (allElements[i]) {
                  allElements[i].classList.add("checked");
                }
              }
            }
          }
    ...
    getFlightTransitAirports(flight) {
      const transitAirports = [];

      // ============================================
      // 🔍 بررسی output_routes_detail برای پیدا کردن is_transit = true
      // ============================================
      if (flight.output_routes_detail && Array.isArray(flight.output_routes_detail)) {
        flight.output_routes_detail.forEach(route => {

          // ✅ فقط مسیرهایی که is_transit = true هستند
          if (route.is_transit === true) {
            // بررسی وجود arrival در route
            if (route.arrival && route.arrival.arrival_code) {
              transitAirports.push(route.arrival.arrival_code);
            }
          }
          // ✅ یا اگر transit زمان دارد (روش جایگزین)
          else if (route.transit && route.transit !== '00:00' && route.transit !== '') {
            if (route.arrival && route.arrival.arrival_code) {
              transitAirports.push(route.arrival.arrival_code);
            }
          }
    ...
    getFlightTransitDuration(flight) {
      let totalTransit = 0;
      let transitCount = 0;
      ...
      if (flight.output_routes_detail && Array.isArray(flight.output_routes_detail)) {
        flight.output_routes_detail.forEach(route => {
          // اگر is_transit = true باشد، یعنی توقف دارد
          if (route.is_transit === true && route.transit) {
            // تبدیل زمان توقف به ساعت (مثلاً "02:25" -> 2.42)
            const duration = this.convertTransitToHours(route.transit);
            if (duration !== null) {
              totalTransit += duration;
              transitCount++;
            }
          }
        });
      ...
      // اگر هیچ توقفی وجود نداشت
      if (transitCount === 0) {
        return null;
      }

      // بازگشت میانگین یا مجموع مدت توقف
      return Math.round(totalTransit * 100) / 100; // گرد کردن به 2 رقم اعشار
    },

    // ✅ تبدیل زمان توقف به ساعت
    convertTransitToHours(transitTime) {
      if (!transitTime || transitTime === '00:00') {
        return null;
      }

      // فرمت: "02:25" یا "2:25"
      const parts = transitTime.split(':');
      if (parts.length === 2) {
  ...
  computed: {
    transitAirports() {
      return this.$store.state.transitAirports;
    },
    count_flights() {
    ...
    ,
    timeArrivalFilter() {
      return this.$store.state.timeArrivalFilter;
    },
    interrupt() {
    ...
    baggageFilter() {
      return this.$store.state.baggageFilter;
    }
    ,
    price() {
    ...
    filteredFlights() {
      if (this.flights != null) {
        const filtered = this.flights.filter(flight => {
          ...
          return (this.arrival_time_filter.length == 1 && this.arrival_time_filter[0] == 'all_arrival_time') ? flight : this.arrival_time_filter.includes(flight.time_flight_arrival_name)
        }).filter(flight => {
          const minDuration = this.transit_duration_filter[0];
          const maxDuration = this.transit_duration_filter[1];

          // اگر محدوده کامل است (0 تا 24)، همه را نمایش بده
          if (minDuration === 0 && maxDuration === 24) {
            return flight;
          }

          // استخراج مدت توقف از پرواز
          const transitDuration = this.getFlightTransitDuration(flight);

          // اگر پرواز توقف ندارد
          if (transitDuration === null) {
            return minDuration === 0; // فقط اگر حداقل 0 باشد
          }

          // بررسی اینکه مدت توقف در محدوده است
          return transitDuration >= minDuration && transitDuration <= maxDuration;
        })

      .filter(flight => {
              if (this.transit_airports_filter.length == 1 && this.transit_airports_filter[0] == 'all_transit_airports') {
                return flight;
              }

              // ✅ استفاده از متد getFlightTransitAirports
              const transitAirports = this.getFlightTransitAirports(flight);

              // اگر پرواز توقف ندارد
              if (transitAirports.length === 0) {
                return false;
              }

              // بررسی اینکه آیا حداقل یکی از فرودگاه‌های توقف در فیلتر انتخاب شده است
              return transitAirports.some(airport =>
                  this.transit_airports_filter.includes(airport)
              );
            })
            .filter(flight => {
