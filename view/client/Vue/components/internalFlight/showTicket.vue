  ...
  <div id="result" class="col-lg-9 col-md-12 col-sm-12 col-xs-12 col-padding-5">
    ...
    <div class=" s-u-ajax-container foreign">
      <div class="s-u-result-wrapper">
        <header-flight-internal :dataSearch="dataSearch" :flights="flights" @updateFlights="applyUniqueFlights"
                                @sortByTime="timeSort" @sortByPrice="priceSort"
                                @updateShowOfferFlights="updateShowOfferFlights"></header-flight-internal>
        <!--    TODO: advertisement-->
        <div class='fullCapacity_div mt-4' v-if='!is_show_loader && (flights.length === 0 || this.$store.state.isComplete)'>
          ...
          <template v-else-if='flights.length === 0'>
            <h2 class="mb-0">{{ useXmltag('Noflight') }}</h2>

            <navigation-days
                :dataSearch="dataSearch"
            />
          </template>
          <h2 v-else-if='this.$store.state.isComplete' class="mt-4">{{ useXmltag('FullCapacityRequestOffline') }}</h2>
        ...
        <ul id="s-u-result-wrapper-ul" v-if="!is_show_loader && flights.length > 0">
          ...
          <div class="items item_flight"><!---->
            ...
            <div class="showListSort" v-for="(flight,key_flight) in twoWayFlight" :key="key_flight"
                 :id="flight.flight_id">
              <div data-typeappticket="noReservation" class="international-available-box"
                   :class="[flight.time_flight_name ,flight.flight_type_li ]">
                              <span class="hidden-data">
                                 {{ key_flight }}
                              </span>
                <input type="hidden" value="noReservation" name="typeAppTicket" id="typeAppTicket">
                <each-twoWay-flight :each_flight="flight" :data_search="dataSearch" :key_flight="key_flight"
                                    :key="`twoWay_${key_flight}`"></each-twoWay-flight>
              </div>
            </div>
            <div class="showListSort" v-for="(flight,key_flight) in flights" :key="key_flight" :id="flight.flight_id">

              <div data-typeappticket="noReservation" class="international-available-box"
                   :class="[flight.time_flight_name , flight.airline ,flight.flight_type_li]">
                                  <span class="hidden-data">
                                 {{ key_flight }}
                              </span>
                <input type="hidden" value="noReservation" name="typeAppTicket" id="typeAppTicket">
                <each-flight :flight="flight" :data_search="dataSearch" :key_flight="key_flight" :key='key_flight'
                             :showOfferFlights="showOfferFlights"></each-flight>
              </div>
            </div>
          </div>
        </ul>
...
import NavigationDays from "./navigationDays.vue";
...
export default {
  ...
  components: {
    ...
    'navigation-days': NavigationDays
  },
  data() {
    return {
      is_show_loader: true,
