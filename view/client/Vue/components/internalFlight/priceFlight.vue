            ...
            <div class="SelectTicket">
               <template>
                  <a
                     ...
                     @click="openSidebarFromPriceInternal">
                    {{ flightTitleBtn }}
                    <svg
...
export default {
   name: "priceFlight",
   props: ["data_search", "flight", "type" ,  "open-sidebar" , 'isProcessingFlightSelection'],
   data() {
      return {
   ...
   computed: {
     flightTitleBtn() {
    if( this.data_search.MultiWay === "TwoWay"){
      return this.$store.state.typeTripFlight === 'dept'
          ? useXmltag('TicketFlightWent')
          : useXmltag('TicketFlightReturn');
    }else{
      return useXmltag('Ticketselect')
    }
