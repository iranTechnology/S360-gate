      ...
      <div class="parent-data-tour">
        <div class="parent-origin-destination">
          ...
          <div class="tour-went-return">
            ...
            <span>مقصد:</span>
            <h6>
              <template v-for="(destination, index) in tour.destinations">
                <template v-if="tour.night != 0">
      <span v-if="destination.night != 0" :key="index">
        {{ destination.city_name }}
      </span>
                </template>

                <template v-else>
      <span v-if="index !== tour.destinations.length - 1" :key="index">
        {{ destination.city_name }}
      </span>
                </template>
              </template>
