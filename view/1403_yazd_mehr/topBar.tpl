...
{if $objSession->IsLogin() && $objSession->getTypeUser() eq 'counter'}
...
<path d="M55,27.5C55,12.337,42.663,0,27.5,0S0,12.337,0,27.5c0,8.009,3.444,15.228,8.926,20.258l-0.026,0.023l0.892,0.752
	...
	c-0.252,0.137-0.502,0.297-0.752,0.476C5.276,41.792,2,35.022,2,27.5z"/>
            ...
            <div class="hello-login"><span>{$objSession->getNameUser()}</span> <span>##Welcomeing##</span></div>
            <div class="etebar-login"><span>##YourCredit##</span>
                {assign var="typeMember" value=$objFunctions->TypeUser($objSession->getUserId())}
                {if $typeMember eq 'Counter'}
                    <span><i> {$objFunctions->CalculateCredit($objSession->getUserId())}</i> <i>##PriceRial##</i></span>
                {elseif $typeMember eq 'Ponline'}
                    ...
                    {if $infoMember.is_member eq '1' && $infoMember.fk_counter_type_id eq '5'}
                        <span><i>  {$objFunctions->getOnlineMemberCredit()|number_format}</i> <i>##PriceRial##</i></span>
                    {/if}
                {/if}
            </div>
            {if $smarty.const.IS_ENABLE_CLUB eq 1}

                <a target="_parent" class="club_" href="https://{$smarty.const.CLIENT_MAIN_DOMAIN}/fa/user/login.php?clubID={$hashedPass}">##Passengerclub##</a>
            {/if}
        </div>
...
{elseif $objSession->IsLogin() && $objSession->getTypeUser() eq 'agency'}
    <div class="sup-menu-flex sup-menu-flex-login">
        <div class="sup-menu-flex-r">
            <div class="login-head-icon">
                <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
<path d="M55,27.5C55,12.337,42.663,0,27.5,0S0,12.337,0,27.5c0,8.009,3.444,15.228,8.926,20.258l-0.026,0.023l0.892,0.752
	...
	c-0.252,0.137-0.502,0.297-0.752,0.476C5.276,41.792,2,35.022,2,27.5z"/>
            ...
            <div class="etebar-login"><span>اعتبار شما</span>
                {assign var="typeMember" value=$objFunctions->TypeUser($objSession->getUserId())}
                <span><i> {$objFunctions->CalculateCredit($smarty.const.SUB_AGENCY_ID,'agency')}</i> <i>ریال</i></span>
            </div>
        </div>
        <div class="sup-menu-flex-l ">
            <ul class="support-menu__links">
                <li class="support-menu__link">
                    <a target="_parent"  href="{$smarty.const.ROOT_ADDRESS}/agencyProfile" >
                        اطلاعات کاربری
                    </a>
                </li>
                <li class="support-menu__link">
                    <a target="_parent" href="{$smarty.const.ROOT_ADDRESS}/reportAgency" >
                        سوابق خرید
                    </a>
                </li>

                <li class="support-menu__link">
                    <a target="_parent" href="{$smarty.const.ROOT_ADDRESS}/membersAgency">
                        لیست کاربران
                    </a>
                </li>
                <li class="support-menu__link">
                    <a target="_parent" href="{$smarty.const.ROOT_ADDRESS}/cancelAgency">
                        سوابق کنسلی
                    </a>
                </li>
                {*   <li class="support-menu__link"><a target="_parent" href="{$smarty.const.ROOT_ADDRESS}/UserPass">
