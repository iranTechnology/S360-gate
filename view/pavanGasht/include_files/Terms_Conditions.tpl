                    ...
                    {foreach $getServices as  $key => $position}
                        {if $position.countFaq>0}
                            <li class="nav-item" role="presentation">
                                <button class="nav-link {if $key=='Flight'} active {/if}" id="Domestic_{$position.MainService}" data-toggle="pill" data-target="#Domestic_{$position.MainService}_body" type="button" role="tab" aria-controls="Domestic_{$position.MainService}_body" aria-selected="false" autocomplete="off">
            ...
            <div class="tab_Terms_Conditions">
                <div class="tab-content" id="Terms_Conditions">
                    {foreach $faqs as  $key=>$position}
                        {if count($position) > 0}
                            <div class="tab-pane fade {if $key=='Flight'} active show {/if}" id="Domestic_{$key}_body" role="tabpanel" aria-labelledby="Domestic_{$key}">
