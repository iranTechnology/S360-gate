        ...
        <div class="sup-menu-flex-r">
            ...
            {elseif $typeMember eq 'Ponline'}
                <div class="etebar-login"><span>امتیاز شما</span>
                    <span><i>{$objFunctions->getOnlineMemberCredit()|number_format} </i> </span>
                </div>
            {/if}

            {if $smarty.const.IS_ENABLE_CLUB eq 1}

                <a class="club_" target="_parent" href="https://search.{$smarty.const.CLIENT_MAIN_DOMAIN}/fa/user/login.php?clubID={$hashedPass}">باشگاه مسافران</a>
            {/if}
        </div>
