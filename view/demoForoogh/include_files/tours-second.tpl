...
<h5>تور های خارجی</h5>
<span>  تورهای  خارجی فرصتی بی‌نظیر برای کشف فرهنگ‌ها، مناظر طبیعی و آثار تاریخی در  خارج از کشور فراهم می‌کنند.  </span>
</div>
</div>
...
<div class="desktop parent-grid-box __tour__external__">
    {foreach $tour_external as $item}
<a class="tour_div __link__ __i_modular_nc_item_class_0" href="{$smarty.const.ROOT_ADDRESS}/detailTour/{$item['id_same']}/{$item['tour_slug']}">
...
<h6 class="tour_title_price ___price_class__">{$item['min_price']['discountedMinPriceR']|number_format}</h6>
    ...
    <div class="mobile owl-carousel owl-theme owl-work-tour parent-grid-box d-block __tour__external__">
        {foreach $tour_external as $item}
            <a class="item tour_div __link__ __i_modular_nc_item_class_0" href="{$smarty.const.ROOT_ADDRESS}/detailTour/{$item['id_same']}/{$item['tour_slug']}">
                <img alt="{$item['tour_name']}" class="__image_class__" src="{$smarty.const.ROOT_ADDRESS_WITHOUT_LANG}/pic/reservationTour/{$item['tour_pic']}"/>
                <div class="tour_text">
                    <h6 class="tour_title __title_class__">{$item['tour_name']}</h6>
                    <p class="tour_dis __description_class__">{$item['destination_city_name']}</p>
