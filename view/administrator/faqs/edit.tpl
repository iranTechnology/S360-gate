...
{assign var="faq" value=$objFaqs->getFaq($smarty.get.id)}
    ...
    <div class="row bg-title">
        <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
            <ol class="breadcrumb FloatRight">
                ...
                <li><a href="{$smarty.const.ROOT_ADDRESS_WITHOUT_LANG}/itadmin/faqs/list">سوالات متداول</a></li>
                <li class='active'>
                    ویرایش
                    <span class='font-bold underdash'>{$faq['title']}</span>
                </li>
            </ol>
        </div>
    </div>
    <div class="row">
        <form data-toggle="validator" method="post" id="editFaq" enctype='multipart/form-data'>
            <input type="hidden" name="className" value="faqs">
            <input type="hidden" name="method" value="editFaq">
            <input type="hidden" name="faq_id" value="{$faq.id}">
            <input type="hidden" name="order_number" value="{$faq.order_number}">
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <div class="form-group">
                                ...
                                <textarea name="content" id="content" class="form-control"
                                          placeholder="محتوای مطلب">{$faq.content}</textarea>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="language" class="form-control" id="language">
                                    {foreach $languages as $value=>$title}
                                        <option value="{$value}" {if $faq.language eq $value} selected {/if}>{$title}</option>
                                    {/foreach}
                                </select>
                            </div>
                        </div>
