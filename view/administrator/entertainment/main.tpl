            ...
            <div class="white-box">
                ...
                <div class="table-responsive">
                    <table id="myTable" class="table table-striped ">
                        ...
                        {if $EntertainmentGetSubDataByLevel != ''}
                            {foreach key=key item=item from=$EntertainmentGetSubDataByLevel}
                                {$number=$number+1}

                                <tr id="del-{$item.CategoryId}">
                                    ...
                                    <td>
                                        <a href="#">
                                            <div style='float: right;'
                                                 onclick="entertainmentCategoryApproval('{$item.CategoryId}'); return false;">
                                                <input type="checkbox" class="js-switch" data-color="#99d683"
                                                       data-secondary-color="#f96262" data-size="small"
                                                       {if $item.CategoryApproval eq 'granted'}checked="checked"{/if} />
                                            </div>
                                        </a>
                                    </td>
                                    ...
                                    {if $CorrectDeep >= 1}
                                        <td class="align-middle">
                                            <a href="{$smarty.const.ROOT_ADDRESS_WITHOUT_LANG}/itadmin/entertainment/ManageEntertainmentList&level={$item.CategoryId}">لیست تفریحات</a>
                                        </td>
                                    {else}
                                        <td class="align-middle">
                                    ...
                                    <td class="align-middle" >
                                         {if $objEntertainment->GetData($item.CategoryId) neq null && $objEntertainment->GetEntertainmentData('',$item.CategoryId) neq null}
                                ...
                                {assign var="subCategoryList" value=$objEntertainment->GetData($item['CategoryId'])}
                                {$sub_number = 0}
                                {foreach $subCategoryList as $sub_item}
                                    {$sub_number = $sub_number + 1}
                                    <tr style='box-shadow: 1px 0px 0px 0px black;' id="del-{$sub_item.CategoryId}">

                                        <td class="align-middle">
                                            <span class="badge badge-info">{$number} - {$sub_number}</span>
                                         </td>
                                        <td class="align-middle">{$sub_item.CategoryTitle}</td>

                                        <td>
                                            <a href="#">
                                                <div style='float: right;'
                                                     onclick="entertainmentCategoryApproval('{$sub_item.CategoryId}'); return false;">
                                                    <input type="checkbox" class="js-switch" data-color="#99d683"
                                                           data-secondary-color="#f96262" data-size="small"
                                                           {if $sub_item.CategoryApproval eq 'granted'}checked="checked"{/if} />
                                                </div>
                                            </a>
                                        </td>
                                        ...
                                        <td class="align-middle">
                                            <a href="{$smarty.const.ROOT_ADDRESS_WITHOUT_LANG}/itadmin/entertainment/EntertainmentList&level={$sub_item.CategoryId}">لیست تفریحات</a>
                                        </td>
                                        <td class="align-middle">
                                            <a onclick="EntertainmentEditCategory({$sub_item.CategoryId})" data-toggle="modal" data-target="#ModalPublic" >
                                                <i  class="fcbtn btn btn-outline btn-primary btn-1e fa fa-pencil tooltip-primary"
                                                    data-toggle="tooltip" data-placement="top" title=""
                                                    data-original-title="ویرایش">
                                        ...
                                        <td class="align-middle" >
                                            {if $objEntertainment->GetData($sub_item.CategoryId) neq null && $objEntertainment->GetEntertainmentData('',$sub_item.CategoryId) neq null}
                                                <a class="cursor-default  popoverBox  popover-default"  data-toggle="popover" title="حذف سردسته" data-placement="right"
                                                   data-content="امکان حذف وجود ندارد زیرا هنوز زیر دسته دارد"> <i class="fcbtn btn btn-outline btn-default btn-1c fa fa-ban " ></i>
                                                </a>
                                            {else}
                                                <a id="DelChangePrice-{$sub_item.CategoryId}" onclick="SubmitRemoveEntertainmentCategory('{$sub_item.CategoryId}'); return false" class="popoverBox  popover-danger" data-toggle="popover" title="حذف سردسته" data-placement="right"
                                                   data-content="حذف"> <i class="fcbtn btn btn-outline btn-danger btn-1c fa fa-trash "></i>
                                                </a>
                                            {/if}
                                        </td>
