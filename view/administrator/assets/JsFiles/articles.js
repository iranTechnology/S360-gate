...
function GetArticleGalleryData(article_id) {
  $.ajax({
    type: 'post',
    url: amadeusPath + 'ajax',
    dataType: 'json',
...
function UpdateArticleGalleryData(id,_this,article_id){
  $.ajax({
    type: 'post',
    url: amadeusPath + 'ajax',
    dataType: 'json',
    ...
    success: function(data) {
      ...
      if (data.success) {
        ...
        GetArticleGalleryData(article_id)
        $.toast({
          heading: 'ثبت تغییرات',
          text: data.message,
          position: 'top-right',
...
async function createVisaTypeSelectBox(positions, _this) {
  await positions.find('select').each(async function() {
