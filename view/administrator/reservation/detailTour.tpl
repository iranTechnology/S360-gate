...
{assign var="isAccessToOrganizationalTour" value=$objFunctions->checkClientConfigurationAccess('organizational_tour')}
...
{/if}
{if $isAccessToOrganizationalTour}
    <div class="row">
        <div class="col-sm-12">
            <div class="white-box">
                <p class="text-muted m-b-10">نمایش تور به همکاران</p>
                <div class="width100">
                    <label for="colleagues" class="flr">##ListColleagues##</label>
                    {assign var="selectedColleagues" value=[]}
                    {if isset($infoTour['colleagues']) && $infoTour['colleagues']}
                        {if is_array($infoTour['colleagues'])}
                            {assign var="selectedColleagues" value=$infoTour['colleagues']}
                        {elseif $infoTour['colleagues']|substr:0:1 == '['}
                            {assign var="selectedColleagues" value=$infoTour['colleagues']|json_decode:true}
                        {else}
                            {assign var="selectedColleagues" value=","|explode:$infoTour['colleagues']}
                        {/if}
                    {/if}
                    <select name="colleagues[]" id="colleagues" class="select2 form-control" multiple="multiple">
                        {assign var="typeMember" value=$objFunctions->infoMember($infoTour['user_id'])}
                        {foreach key=key item=item from=$objAgency->list}
                            {if $item['isColleague'] == 1}
                                <option value="{$item['id']}"
                                        {if in_array($item['id'], $selectedColleagues)}selected{/if}
                                        {if $typeMember['fk_agency_id'] == $item['id']}selected{/if}
                                >
                                    {$item['name_fa']}
                                </option>
                            {/if}
                        {/foreach}
                        <option value="onlinePassengers" {if in_array('onlinePassengers', $selectedColleagues)}selected{/if}>
                            مسافر آنلاین
                        </option>
                    </select>
                </div>
