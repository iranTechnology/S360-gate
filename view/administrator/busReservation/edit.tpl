...
{assign var="bus_condition" value=['id'=>$smarty.get.id]}
{assign var="bus_data" value=$objResult->getBusReservationData($bus_condition)}
{assign var="stations" value=$objResult->getStations()}
{assign var="bus_data" value=$bus_data[0]}
    ...
    <div class="row bg-title">
        <div class="col-lg-6 col-sm-3 col-md-6 col-xs-12">
            ...
            <ol class="breadcrumb FloatRight">
                ...
                <li class="active">ویرایش اتوبوس</li>
            </ol>
        </div>
    </div>
        ...
        <div class="col-sm-12">
            <div class="white-box">
                ...
                <form class='d-flex w-100 flex-wrap' id="form_reservation_bus" method="post" action="">
                    <input type="hidden" name="id" value="{$bus_data['id']}">
                    ...
                    <div class="form-group col-sm-3">
                        ...
                        <select onchange='getStations($(this).val(),$("#origin_terminal"))'
                            ...
                            {foreach $objResult->getCities() as $city}
                                <option {if $bus_data['origin_city'] eq $city['id']} selected {/if} value="{$city['id']}">{$city['name_fa']}</option>
                            {/foreach}
                        </select>
                    </div>
                    ...
                    <div class="form-group col-sm-3">
                        ...
                        <select onchange='getStations($(this).val(),$("#destination_terminal"))' name="destination_city"
                            ...
                            <option value="">انتخاب کنید....</option>
                            {foreach $objResult->getCities() as $city}
                                <option {if $bus_data['destination_city'] eq $city['id']} selected {/if} value="{$city['id']}">{$city['name_fa']}</option>
                            {/foreach}
                        </select>
                    </div>
                    ...
                    <div class="form-group col-sm-3">
                        ...
                        <select name="origin_terminal"
                            ...
                            {foreach $stations as $item}
                                <option {if $bus_data['origin_terminal'] eq $item['id']} selected {/if} value="{$item['id']}">{$item['station_name']}</option>
                            {/foreach}
                    ...
                    <div class="form-group col-sm-3">
                        ...
                        <select name="destination_terminal" id="destination_terminal"
                            ...
                            <option value="">انتخاب کنید....</option>
                            {foreach $stations as $item}
                                <option {if $bus_data['destination_terminal'] eq $item['id']} selected {/if} value="{$item['id']}">{$item['station_name']}</option>
                            {/foreach}
                        </select>
                        <a class='mt-1' href='stations'>تعریف پایانه</a>
                    </div>
                    ...
                    <div class="form-group col-sm-12 mb-3">
                        ...
                        <div class="row">
                            <div class="form-group m-0 col-sm-12 DynamicExtraStationData">
                                ...
                                {assign var="ExtraStationData" value=$bus_data['dropping_points']}
                                {if $ExtraStationData eq ''}
                                    {assign var="ExtraStationData" value='[{"body":""}]'}
                                {/if}
                                {assign var="counter" value='0'}
                                {foreach key=key item=item from=$ExtraStationData|json_decode:true}
                                ...
                                <div data-target="BaseExtraStationDataDiv" class="col-sm-12 p-0 form-group">
                                    <div class="col-md-11 p-0">
                                        ...
                                        <input data-parent="ExtraStationDataValues" data-target="body" name="ExtraStationData[{$counter}][body]" placeholder="نام ایستگاه"
                                               data-action='required'
                                               value="{$item}"
                                               class="form-control"
                                               type="text">
                    ...
                    <div class="form-group col-sm-3">
                        ...
                        <select name="company" id="company"
                            ...
                            {foreach $objResult->getBaseCompanyBus() as $item}
                                <option {if $bus_data['company_id'] eq $item['id']} selected {/if}  value="{$item['id']}">{$item['name_fa']}</option>
                            {/foreach}
                        </select>
                    </div>
                    ...
                    <div class="form-group col-sm-3">
                        ...
                        <select name="chairs_count"
                            ...
                            {foreach $objResult->getChairsArray() as $item}
                                <option {if $bus_data['chairs_count'] eq $item} selected {/if} value="{$item}">{$item}</option>
                            {/foreach}
                        </select>
                    </div>
                    ...
                    <div class="form-group col-sm-12">
                        ...
                        <textarea class="form-control"
                                  data-action='required'
                                  name="description"
                                  id="description" placeholder=" توضیحات را وارد نمائید">{$bus_data['description']}</textarea>
                    </div>


                    <div class="form-group col-sm-3">

                        <button onclick='submitUpdateReservationBus($(this),$("#form_reservation_bus"))'
                                type='button'
                                class='btn btn-primary'>ویرایش اتوبوس</button>
                    </div>
