    ...
    <div class="row bg-title">
        <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
            ...
            <ol class="breadcrumb FloatRight">
                ...
                <li>کانتر</li>
                <li>مسافران</li>
                <li class="active">سوابق خرید مسافر</li>
            </ol>
        </div>
                ...
                <div class="table-responsive">
                    <table id="ticketHistory" class="table table-striped text-center">
                        ...
                        <tr>
                            ...
                            <th style="text-align: center;">شماره رزرو<br />شماره فاکتور</th>
                            ...
                            <tbody>
                            {$objbook->CountTicketCounter}
                            {assign var="bookList" value=$objbook->PassengerTicketHistory($smarty.get.Code,$AgencyId)}
                           {foreach  $bookList as $key=>$item}
                                ...
                                <tr>
                                    ...
                                    <td> ({$objbook->format_hour($item.time_flight)})&nbsp;&nbsp; {$objbook->DateJalali($item.date_flight)}  </td>
                                    <td>{if $item.flight_type eq 'charter'}چارتری{else} سیستمی{/if}</td>
                                    <td>{if $item.payment_type eq 'cash'}نقدی{else}اعتباری{/if}</td>
                                    <td>{$item.request_number}<br />{$item.factor_number}</td>
                                    <td>{$objFunctions->CalculateDiscount($item.request_number,'yes')|number_format} ریال</td>
                                    <td>
                                        <a href="{$smarty.const.SERVER_HTTP}{$objbook->ShowAddressClient($smarty.const.CLIENT_ID)}/gds/pdf&target=parvazBookingLocal&id={$item.request_number}" target="_blank"
                                        ...
                                        <a href="{$smarty.const.SERVER_HTTP}{$objbook->ShowAddressClient($smarty.const.CLIENT_ID)}/gds/pdf&target=bookshow&id={$item.request_number}" target="_blank"
                                           title="دانلود بلیط(pdf)" style="margin: 5px 0;">
