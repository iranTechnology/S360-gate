...
{assign var="info" value=$objCode->getGroupByID($smarty.get.id)}
...
<div class="container-fluid">
    <div class="row bg-title">
        <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
            ...
            <ol class="breadcrumb FloatRight">
                ...
                <li>خدمات ویژه</li>
                <li class="active">ویرایش کدهای تخفیف</li>
            </ol>
        </div>
        <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="white-box">
                <h3 class="box-title m-b-0">ویرایش کدهای تخفیف</h3>
                <p class="text-muted m-b-30"> شما با استفاده از فرم زیر می توانید کدهای تخفیف را از طریق فایل اکسل در سیستم ثبت نمائید</p>

                <form data-toggle="validator" id="interactiveOffCodesEdit" method="post">
                    <input type="hidden" name="flag" id="flag" value="interactiveOffCodesEdit">
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <input type="text" class="form-control datepickerOfToday" id="expireDate" name="expireDate" value="{$objDate->jdate('Y-m-d', $info['expireDateInt'])}"
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <select class="form-control select2" id="origin" name="origin[]" multiple="multiple">
                            <option value="">خدمات مورد نظر را انتخاب نمائید</option>
                            {foreach $allCities as $city}
                                <option value="{$city.city_iata}" {if in_array($city.city_iata, $info['originIata']|json_decode)}selected="selected"{/if}>{$city.city_name}</option>
                            {/foreach}
                        </select>
                    </div>
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <select class="form-control select2" id="destination" name="destination[]" multiple="multiple">
                            <option value="">خدمات مورد نظر را انتخاب نمائید</option>
                            {foreach $allCities as $city}
                                <option value="{$city.city_iata}" {if in_array($city.city_iata, $info['destinationIata']|json_decode)}selected="selected"{/if}>{$city.city_name}</option>
                            {/foreach}
                        </select>
                    </div>
                    ...
                    <div class="form-group col-sm-6 ">
                        ...
                        <input type="text" class="form-control" id="priority" name="priority" value="{$info['priority']}" />
                    </div>
                    ...
                    <div class="form-group col-sm-12">
                        ...
                        {foreach key=keyService item=itemService from=$objServicesDiscount->services}
                            <div class="col-sm-3">
                                <input type="checkbox" name="Check{$itemService.TitleEn}" id="Check{$itemService.TitleEn}" value="1" {if $objCode->HasThisService($info['id'], $itemService.TitleEn)} checked="checked" {/if} />
