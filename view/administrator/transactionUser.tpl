        ...
        {if $smarty.const.TYPE_ADMIN eq '2' }
            ...
            <div class='white-box'>
                <p style='font-weight:bold'>
                    ...
                    شماره کارت
                    <br>
                    <code style='display: inline-flex;font-size: 12px;'>5041721205849384</code>
                    <br>
                    شماره شبا
                    <br>
                    <code style='display: inline-flex;font-size: 12px;'>790700010001108551633001</code>
                    <br>
                    ...
                    شماره تلگرام:
                    <br>
                    <code style='display: inline-flex;font-size: 12px;'>09057078341</code>
                </p>
            </div>
            {*<div class='white-box'>
        ...
        <div class="col-sm-12">
            <div class="white-box">
                ...
                <form id="SearchTransaction" method="post"
                    ...
                    <div class="form-group col-sm-6 ">
                        <label for="Reason" class="control-label">دلیل تراکنش (چند انتخابی)</label>
                        <select name="Reason[]" id="Reason" class="form-control select2" multiple="multiple" style="width: 100%;">
                            <option value="buy" {if is_array($smarty.post.Reason) && in_array('buy', $smarty.post.Reason)}selected="selected"{/if}>خرید</option>
                            <option value="charge" {if is_array($smarty.post.Reason) && in_array('charge', $smarty.post.Reason)}selected="selected"{/if}>شارژ حساب</option>
                            <option value="decrease" {if is_array($smarty.post.Reason) && in_array('decrease', $smarty.post.Reason)}selected="selected"{/if}>کسر از حساب</option>
                            <option value="increase" {if is_array($smarty.post.Reason) && in_array('increase', $smarty.post.Reason)}selected="selected"{/if}>واریز به حساب</option>
                            <option value="indemnity_cancel" {if is_array($smarty.post.Reason) && in_array('indemnity_cancel', $smarty.post.Reason)}selected="selected"{/if}>واریز کنسلی</option>
                            {if $smarty.const.TYPE_ADMIN == '1'}
                                <option value="temporary_deduction"  {if is_array($smarty.post.Reason) && in_array('temporary_deduction', $smarty.post.Reason)}selected="selected"{/if}>کسر موقت</option>
                            {/if}
                        </select>
                    </div>
                    {*                        <div class="form-group col-sm-6 ">*}
