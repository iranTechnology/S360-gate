                    ...
                    $data_change_price = array(
                        ...
                        'price' => array(
                            ...
                            'infant' => array(
                                ...
                                'CommisionPrice' => $passengerInfant['CommisionPrice']
                            ),
                        )
                    );
...
//                                'transit' => functions::duration_time_source($flight['SourceId'], $details_return['transit'], $translateVariable),
            ...
            functions::insertLog('$this->tickets $resFilterInternationalFlight: ' . json_encode($this->tickets), '000shojaee');
...
// ===================================
                    ...
                    if (isset($route['Baggage']) && is_array($route['Baggage'])) {
                        foreach ($route['Baggage'] as $bag) {
                            ...
                            if ($result !== null) {
                                ...
                                if (strlen($normalized) >= 1 && preg_match('/\d+/', $normalized)) {
                                    // ✅ name_fa از baggageTitle گرفته می‌شود
                                    $title = $this->baggageTitle($flight['SourceId'], $route, $translateVariable);
            ...
            if (isset($flight['Baggage']) && is_array($flight['Baggage'])) {
                foreach ($flight['Baggage'] as $bag) {
                    ...
                    if ($result !== null) {
                        ...
                        if (strlen($normalized) >= 1 && preg_match('/\d+/', $normalized)) {
                            $title = $this->getDefaultBaggageTitle($this->extractNumberFromString($normalized), $translateVariable);
    ...
    private function normalizeBaggageDisplay($display)
    {
        ...
        // ============================================
        // 1️⃣ حذف کاراکترهای اضافی مانند "P" قبل از KG
        // ============================================
        // "0P KG" -> "0 KG"
        // "0P KG" -> "0 KG"
        // "0P" -> "0 KG"
        ...
        if (empty($datas['return'])) {
            ...
            foreach ($datas['dept'] as $key => $flight) {
                ...
                $this->tickets['flights'][$key] = array(
                    ...
                    'time_flight_arrival_name' => functions::classTimeLOCAL(functions::format_hour($flight['ArrivalTime']), false),
...
//        $user_names = ['ozhangasht','flymurshid','versagasht','myna','safarcenter'];
        if (empty($dataCacheAverage) || ($dataCacheAverage['creation_date_int'] < (time() - 3600))) {
            ...
            if (in_array($params['origin'], $cities) && in_array($params['destination'], $cities)) {
                ...
                $url = /*$this->apiAddress .*/
                    'https://safar360.com/Core/V-1/Flight/getAveragePriceParto/flymurshid';//. $user_names[$user_name_selected]
                ...
                $url = /*$this->apiAddress .*/
                    "http://safar360.com/Core/V-1/Flight/flightFifteenDay";
    ...
    public function configHotelatoFlightInternational($params)
    {//برای هتلاتو فقط انجام میشه و برای بقیه همون روند عادیه
