...
//            $price_check = $this->getController('checkLimitPrice')->getChecKLimits($data_check_limit);
            ...
            foreach ($prices_limit_route as $row) {
                $limits[$row['airline_uniqe_iata']][$row['fare_class']] = $row['ceiling_price'];
            }

            functions::insertLog('$limits: ' . json_encode($limits) , '0abbasi');
                ...
                if ( (Session::IsLogin() && Session::getCounterTypeId() !='1') ||  !(Session::IsLogin())) {
                    if ($flight_type == 'charter') {
                        if (isset($limits[$airlineId][$fareClass])) {
                            functions::insertLog('isset: ' . json_encode($limits[$airlineId][$fareClass]) , '0abbasi');
                            functions::insertLog('$price: ' . json_encode($price) , '0abbasi');
