...
//    }
        ...
        if (!empty($info_ticket)) {
            ...
            //            QRcode::png($qrUrl, false, QR_ECLEVEL_L, 4, 2);
            ...
            functions::insertLog('$Fee: ' . json_encode($Fee) , 'shojaee');
            $Price = '0';
...
//        $cancelTicketPrice = $priceTotal - (($priceTotal * $info['cancelTicketPercent']) / 100);
            ...
            <div  style='margin-top: 1000px;font-family: "yekanbakh"'>
                ...
                if ($info['flight_type'] == 'system' && $info['successfull'] == 'private_reserve') {
                    ...
                    $DataFlightitAgencyCommission ='<span class="font-bold rounded-xl d-inline-block" style="direction:ltr;">'.number_format($info['system_flight_commission']).'</span>';//پرداخت مسافر - خرید از ما
                }
                ...
                if ( $info['flight_type'] != 'charterPrivate' ) {
                    ...
                    $DataFlightFare = $info['flight_type'] == 'system' ? number_format($info['adt_fare'] + $info['chd_fare'] + $info['inf_fare']) : '0';
                }
                else {
                    $DataFlightTotal = '0';
                    $DataFlightFare = '0';
...
//                  }
                ...
                if ($info['request_cancel'] != 'confirm' && ($info['successfull'] == 'book' || $info['successfull'] == 'private_reserve') && CLIENT_ID == 408){ ?>
                    ...
                    <div class="" style="margin: 10px 100px ;border:1px solid #ccc">
                        ...
                        <table width="100%" align="center" cellpadding="0" cellspacing="0">
                            ...
                            <tr>
                                <td style="padding:20px">
                                    <ul>
                                        <li>1- جریمه استرداد تا 30 دقیقه بعد از رزرو (فقط تا 16 ساعت قبل از پرواز) 0 درصد مي باشد.</li>
                                        <li>2- جریمه استرداد از زمان صدور بلیت تا ساعت 12 ظهر 3 روز قبل از پرواز 30 درصد مي باشد.</li>
                                        <li>3- جریمه استرداد از ساعت 12 ظهر 3 روز قبل از پرواز تا ساعت 12 ظهر 2 روز قبل از پرواز 40 درصد مي باشد.</li>
                                        <li>4- جریمه استرداد از ساعت 12 ظهر 2 روز قبل از پرواز تا ساعت 12 ظهر 1 روز قبل از پرواز 70 درصد مي باشد.</li>
                                        <li>5- جریمه استرداد از ساعت 12 ظهر 1 روز قبل از پرواز تا 24 ساعت قبل از پرواز 80 درصد مي باشد.</li>
                                        <li>6- جریمه استرداد از 24 ساعت قبل از پرواز به بعد 100 درصد مي باشد.</li>
                                        <li>7- تمامی درصد جریمه های پروازی بر روی سیستم کنسلی و استرداد آنلاین تعریف شده است و مسافرین محترم میتوانند از طریق لینک زیر اقدام به ثبت کنسلی و استرداد بلیت های خود نمایند. ضمنا در صورت خرید نرخ های دو مسیره قوانین استرداد متفاوت بوده و بر اساس کل رفرنس محاسبه میگردد با تشکر</li>
                    ...
                    <?php
                }else if($info['request_cancel'] != 'confirm' && ($info['successfull'] == 'book' || $info['successfull'] == 'private_reserve') && CLIENT_ID != 408){
                    ?>
                    <div class="" style="margin: 10px 100px ;border:1px solid #ccc">
                        ...
                        <table width="100%" align="center" cellpadding="0" cellspacing="0">
                            ...
                            <tr>
                                <td style="padding:20px">
                                    <ul>
                                        <li>1- طبق مقررات سازمان هواپیمایی کشوری، مسئولیت کنسلی، تاخیر یا تعجیل در زمان انجام پرواز، بر عهده‌ی شرکت هواپیمایی برگزار کننده‌ی پرواز است. </li>
                                        <li>2- مسئولیت درج صحیح کلیه اطلاعات مسافران و مسیر و ... با کاربر میباشد و در صورت درج اشتباه اطلاعات اصلاح نام مسافر، تاریخ و ... پس از خرید نهایی بلیط وجود ندارد و برای اصلاح اطلاعات، مسافر می‌بایست بلیط را کنسل و مجدداً خریداری کند یا مشمول جریمه می گردد. </li>
                                        <li>3- کاربر باید یک شماره موبایل معتبر که پیامک تبلیغاتی آن باز باشد در سیستم درج نماید تا در صورت نیاز پیامک های تاخیر، تعجیل و لغو پرواز برای او ارسال شود </li>
                                        <li>4- طبق قوانین هواپیمایی کشوری، نوزاد ( بین ۱۰ روز تا دو سال ) ، کودک ( بین دو تا دوازده سال )و بزرگسال ( بیش از دوازده سال ) سن دارند. </li>
                                        <li>5- بلیط نوزاد یا کودک را به هیچ وجه نمی‌توان به تنهایی و جدا از بلیط بزرگسال به صورت آنلاین خریداری نمود. </li>
                                        <li>6- نوع هواپیما بر اساس اعلام ایرلاین‌ها و چارترکنندگان در هنگام خرید و بلیط صادر شده مشخص می باشد. هر نوع تغییر در نوع هواپیما بر عهدۀ شرکت هواپیمایی برگزارکنندۀ پرواز است. </li>
                                        <li>7- حضور مسافر در پرواز های داخلی حداقل یک ساعت قبل از زمان پرواز و در پرواز های خارجی حداقل 3 ساعت قبل از زمان پرواز در فرودگاه و ترمینال پرواز به همراه مدارک مورد نیاز الزامی است </li>
                                        <li>8- جدول جریمه به شکل زیر است، اما امکان دارد در مواقعی خاص و بر اساس سیاست تامین کننده، تغییراتی داشته باشد. </li>
                                    </ul>
