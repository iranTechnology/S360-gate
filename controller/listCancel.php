...
//                $data['PriceIndemnity'] = $busRefundCheck['data']['totalRefundableAmount'];
            ...
            if (!empty($result)) {
                ...
                elseif ($InfoCancel['TypeCancel'] == 'cip') {
                    $d['Price'] = $Param['PriceIndemnity'];
                    $InfoBook['factor_number'] = $InfoCancel['FactorNumber'];
                    $titleType = 'cip';
                }
                ...
                foreach ($InfoCancelNationalCodes as $key => $NationalCode) {
                    ...
                    elseif ($InfoCancel['TypeCancel'] == 'cip') {
                        ...
                        $this->admin->ConectDbClient('', $Param['ClientId'], "Update", $book, "book_cip_tb", $Condition);
                        $this->ModelBase->setTable('report_cip_tb');
                    }
    ...
    protected function getCipForCancel($params) {

        $sql = " SELECT *,  "
            . " (Select COUNT(passenger_age) FROM book_cip_tb WHERE passenger_age='Adt' AND request_number='{$params['RequestNumber']}' AND request_cancel <> 'confirm') AS Adt_passenger ,"
            . " (Select COUNT(passenger_age) FROM book_cip_tb WHERE passenger_age='Chd' AND request_number='{$params['RequestNumber']}' AND request_cancel <> 'confirm') AS Chd_passenger ,"
            . " (Select COUNT(passenger_age) FROM book_cip_tb WHERE passenger_age='Inf' AND request_number='{$params['RequestNumber']}' AND request_cancel <> 'confirm') AS Inf_passenger "
            . " FROM book_cip_tb WHERE request_number='{$params['RequestNumber']}' AND successfull='book' AND request_cancel <> 'confirm' ";
        return $this->Model->load($sql);
    }
...
//        die;
    ...
    private function getInfoCancelWithTypeService($params, $apiCancel) {
        ...
        if ($params['typeService'] == 'cip') {
            $BookResult = $this->getCipForCancel($params);
        }
