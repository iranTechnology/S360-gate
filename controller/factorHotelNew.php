                    ...
                    if ($typeApplication == 'externalApi' || ($typeApplication == 'api' && ($sourceId == '17' || $sourceId == '29' || $sourceId == '46'))) {
                        ...
                        foreach ($price_rooms as $price_room) {
                            ...
                            if (!empty($price_room) && $price_room['price_current_sum'] > 0) {
                                ...
                                $adult[$i]['is_transfer'] = $_POST['is_transfer'];
                                $adult[$i]['transfer_hotel'] = [
                                    // خروج
                                    'type_vehicle' => $_POST['type_vehicle'] ?? '',
                                    'number_vehicle' => $_POST['number_vehicle'] ?? '',
                                    'time_vehicle' => $_POST['time_vehicle'] ?? '',
                                    'carrier_company' => $_POST['carrier_company'] ?? '',
                                    'origin_city' => $_POST['origin_city'] ?? '',
                                    // ورود
                                    'type_vehicle_arrival' => $_POST['type_vehicle_arrival'] ?? '',
                                    'number_vehicle_arrival' => $_POST['number_vehicle_arrival'] ?? '',
                                    'time_vehicle_arrival' => $_POST['time_vehicle_arrival'] ?? '',
                                    'carrier_company_arrival' => $_POST['carrier_company_arrival'] ?? '',
                                    'origin_city_arrival' => $_POST['origin_city_arrival'] ?? '',
...
//						}
                        ...
                        if (!empty($price_room) && $price_room['price_current_sum'] > 0) {
                            ...
                            $adult_arr['is_transfer'] = $_POST['is_transfer'];
                            $adult_arr['transfer_hotel'] = [
                                // خروج
                                'type_vehicle' => $_POST['type_vehicle'] ?? '',
                                'number_vehicle' => $_POST['number_vehicle'] ?? '',
                                'time_vehicle' => $_POST['time_vehicle'] ?? '',
                ...
                do {
                    ...
                    if (
                        ...
                        if ($sourceId == '46') {
                            ...
                            $Id_Select_Room_Child = $_POST['Id_Select_Room' . $room_count_number];
