...
class logErrorBuses extends clientAuth{
    ...
    public function insertLogErrorBuses($data_log) {

        return $this->getModel('logErrorBusesModel')->insertLocal($data_log);
    }

    public function getErrorMessage($request_number,$client_id) {
        $result_error =  $this->getModel('logErrorBusesModel')->get()->where('request_number',$request_number)->where('client_id',$client_id)->orderBy()->all();
