...
class api extends clientAuth
{
    ...
    public function search($param, $dataInfo) {
        ...
        if(!$param['IsInternal'] && $param['Origin']=='THR'){
            $infoCode = array(
                'Result' => array(
                    'Value' => ''
                ),
...
// این کد مربوط به حذف پرواز های بالاتر از نرخح بود که کامنت شد
//        if ($param['IsInternal']) {
//
            ...
            if (isset($BookFlight['Result']['ProviderStatus']) && $BookFlight['Result']['ProviderStatus'] != "Error") {
                ...
                foreach ($param['Books'] as $key => $passengers) {
                    ...
                    foreach ($BookFlight['Result']['Request']['RequestFares'] as $RequestFares) {
                        if ($d['api_id'] == '10' || $d['api_id'] == '15' || $d['api_id'] == '17' || $d['api_id'] == '18' || $d['api_id'] == '21' || $d['api_id'] == '43') {
