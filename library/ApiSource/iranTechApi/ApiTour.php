...
class ApiTour extends clientAuth
{
    ...
    public function fetchAllNewTours($content = []) {
        ...
        // تعریف reservation_tour_controller خارج از حلقه
        ...
        foreach ($this->providers as $provider) {
            ...
            $tourIdsStr = implode("','", $tourIds);

            // دریافت قیمت‌ها به صورت دسته‌ای
            $prices = [];
            $priceSql = "SELECT 
                    T.id as tour_id,
                    T.change_price,
                    P.id AS package_id,
                    CASE 
                        WHEN P.double_room_price_r > 0 THEN P.double_room_price_r 
                        WHEN P.single_room_price_r > 0 THEN P.single_room_price_r 
                        ELSE P.three_room_price_r 
                    END AS min_price_r,
                    CASE 
                ...
                LEFT JOIN reservation_tour_package_tb AS P ON T.id = P.fk_tour_id AND P.is_del = 'no'
                WHERE T.id IN ('{$tourIdsStr}')
                ORDER BY min_price_r ASC";

            $priceResults = $this->admin_controller->ConectDbClient($priceSql, $provider, "SelectAll", "", "", "");
            ...
            if (!empty($priceResults)) {
                ...
                foreach ($priceResults as $pr) {
                    if (!empty($pr['package_id'])) {
                        $packageIds[] = $pr['package_id'];
                    }
                }

                if (!empty($packageIds)) {
                    $packageIdsStr = implode("','", array_unique($packageIds));
                    $discountSql = "SELECT tour_id, adult_amount, tour_package_id 
                               FROM reservation_tour_discount_tb 
                               WHERE tour_package_id IN ('{$packageIdsStr}') AND counter_type_id='5'";
                    $discountResults = $this->admin_controller->ConectDbClient($discountSql, $provider, "SelectAll", "", "", "");

                    foreach ($discountResults as $disc) {
                        $discountMap[$disc['tour_id'] . '_' . $disc['tour_package_id']] = $disc['adult_amount'];
                    }
                }
                ...
                foreach ($priceResults as $pr) {
                    ...
                    // فقط ارزان‌ترین قیمت را نگه دار
                    if (isset($prices[$tourId])) {
                        continue;
                    }

                    $discountAmount = $discountMap[$tourId . '_' . $pr['package_id']] ?? 0;
                    $changedPrice = $reservation_tour_controller->doPriceChange($pr['min_price_r'] ?? 0, $pr['change_price'] ?? 0);
                    $finalPrice = $changedPrice - $discountAmount;

                    if ($finalPrice > 0) {
                        $prices[$tourId] = [
                            'price' => round($finalPrice),
                            'price_currency' => $pr['min_price_a'] ?? 0,
                            'Currency_title' => $pr['currency_type'] ?? ''
                        ];
                    }
                }
            }

            // دریافت route ها به صورت دسته‌ای
            $routes = [];
            $routeSql = "SELECT TR.*, RC.name, RC.name_en, 
                    RCOUNTRY.name as country_name, RCOUNTRY.name_en as country_name_en 
                    FROM reservation_tour_rout_tb TR
                    ...
                    WHERE TR.fk_tour_id IN ('{$tourIdsStr}') AND TR.is_del = 'no' 
                    ORDER BY TR.tour_title";

            $routeResults = $this->admin_controller->ConectDbClient($routeSql, $provider, "SelectAll", "", "", "");
            foreach ($routeResults as $route) {
                $routes[$route['fk_tour_id']][] = $route;
            }

            // دریافت vehicle ها به صورت دسته‌ای
            $vehicles = [];
            $vehicleSql = "SELECT 
                        R.fk_tour_id, R.type_vehicle_name, R.airline_name, R.tour_title
                    FROM reservation_tour_tb T
                    LEFT JOIN reservation_tour_rout_tb R ON R.fk_tour_id = T.id 
                    WHERE T.id IN ('{$tourIdsStr}')
                    GROUP BY R.fk_tour_id, R.tour_title
                    ORDER BY R.id, R.tour_title = 'dept' DESC";

            $vehicleResults = $this->admin_controller->ConectDbClient($vehicleSql, $provider, "SelectAll", "", "", "");
            foreach ($vehicleResults as $veh) {
                $vid = $veh['fk_tour_id'];
                $vehicles[$vid][$veh['tour_title']] = [
                    'type_vehicle_name' => $veh['type_vehicle_name'],
                    'airline_name' => $veh['airline_name'],
                    'tour_title' => ($veh['tour_title'] == 'dept') ? 'رفت' : 'برگشت'
                ];
            }
            ...
            foreach ($tours as $tour) {
                ...
                $all_final_tours[] = [
                    ...
                    'vehicle_id' => $tour['airline_id'],
                    'type_vehicle_id' => $tour['type_vehicle_id'],
                    'start_price' => $prices[$tourId],
                    'vehicles' => $vehicles[$tourId] ?? [],
                    'info_tour_rout' => $routes[$tourId] ?? [],
                    'agency_info' => [
                        'AgencyName' => $agencyInfo['AgencyName'] ?? '',
                        'Address' => $agencyInfo['Address'] ?? '',
                        'Phone' => $agencyInfo['Phone'] ?? '',
